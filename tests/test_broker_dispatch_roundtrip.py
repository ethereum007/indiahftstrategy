...
from reports.operational_lineage import empty_broker_dispatch_ack_lineage
...
def add_ack_complete_final_target_application_lineage(config, vendor, **overrides):
    ...
    ] = ack_view_56_target_application_lineage_comparison(vendor)
    config[
    ...
    ] = ack_view_64_target_application_lineage_comparison(vendor)
    config[
    ...
    ] = ack_view_72_target_application_lineage_comparison(vendor)
...
def ack_view_56_target_application_lineage_comparison(
    ...
    **overrides,
    ...
    comparison = ack_view_48_target_application_lineage_comparison(
        vendor,
        lineage_sha256=lineage_sha256,
    )
...
def ack_view_64_target_application_lineage_comparison(
    ...
    **overrides,
    ...
    comparison = ack_view_56_target_application_lineage_comparison(
        vendor,
        lineage_sha256=lineage_sha256,
    )
    comparison.pop(
...
def ack_view_72_target_application_lineage_comparison(
    ...
    **overrides,
    ...
    comparison = ack_view_64_target_application_lineage_comparison(
        vendor,
        lineage_sha256=lineage_sha256,
    )
    comparison.update(
...
def roundtrip_view_73_target_application_lineage_comparison(
    ...
    *,
    lineage_sha256=None,
    roundtrip_lineage_sha256=None,
    **overrides,
):
    lineage_sha256 = lineage_sha256 or target_application_lineage_sha256(
        vendor["datasets"]
    )
    roundtrip_lineage_sha256 = roundtrip_lineage_sha256 or lineage_sha256
    comparison = ack_view_72_target_application_lineage_comparison(
        vendor,
        lineage_sha256=lineage_sha256,
    )
    comparison.update(
        {
            "roundtrip_confirmed_verified_reconciled_current_latest_extended_complete_final_review_carried_application_lineage_sha256": roundtrip_lineage_sha256,
...
def with_broker_vendor_batch_summary(summary, vendor, *, prefix):
    ...
    if (
        ...
        current_latest_extended_complete_final_lineage_48 = (
            ack_view_48_target_application_lineage_comparison(vendor)
        )
        reconciled_current_latest_extended_complete_final_lineage_56 = (
            ack_view_56_target_application_lineage_comparison(vendor)
        )
        verified_reconciled_current_latest_extended_complete_final_lineage_64 = (
            ack_view_64_target_application_lineage_comparison(vendor)
        )
        confirmed_verified_reconciled_current_latest_extended_complete_final_lineage_72 = (
            ack_view_72_target_application_lineage_comparison(vendor)
        )
        ...
        result.loc[
            0,
            f"{current_latest_extended_complete_final_48_prefix}_lineage_match_required",
        ] = current_latest_extended_complete_final_lineage_48["required"]
        result.loc[
            0,
            f"{current_latest_extended_complete_final_48_prefix}_lineage_matches",
        ] = current_latest_extended_complete_final_lineage_48["matches"]
        for field, value in current_latest_extended_complete_final_lineage_48.items():
            if field == "carried_application_lineage_sha256":
                result.loc[
                    0,
                    f"{current_latest_extended_complete_final_48_prefix}_ack_current_latest_extended_complete_final_review_carried_application_lineage_sha256",
                ] = value
            elif field not in {"required", "matches"}:
                result.loc[
                    0,
                    f"{current_latest_extended_complete_final_48_prefix}_{field}",
                ] = value
        result.loc[
            0,
            f"{reconciled_current_latest_extended_complete_final_56_prefix}_lineage_match_required",
        ] = reconciled_current_latest_extended_complete_final_lineage_56["required"]
        result.loc[
            0,
            f"{reconciled_current_latest_extended_complete_final_56_prefix}_lineage_matches",
        ] = reconciled_current_latest_extended_complete_final_lineage_56["matches"]
        for field, value in reconciled_current_latest_extended_complete_final_lineage_56.items():
            if field == "carried_application_lineage_sha256":
                result.loc[
                    0,
                    f"{reconciled_current_latest_extended_complete_final_56_prefix}_ack_reconciled_current_latest_extended_complete_final_review_carried_application_lineage_sha256",
                ] = value
            elif field not in {"required", "matches"}:
                result.loc[
                    0,
                    f"{reconciled_current_latest_extended_complete_final_56_prefix}_{field}",
                ] = value
        result.loc[
            0,
            f"{verified_reconciled_current_latest_extended_complete_final_64_prefix}_lineage_match_required",
        ] = verified_reconciled_current_latest_extended_complete_final_lineage_64[
            "required"
        ]
        result.loc[
            0,
            f"{verified_reconciled_current_latest_extended_complete_final_64_prefix}_lineage_matches",
        ] = verified_reconciled_current_latest_extended_complete_final_lineage_64[
            "matches"
        ]
        for field, value in (
            verified_reconciled_current_latest_extended_complete_final_lineage_64.items()
        ):
            if field == "carried_application_lineage_sha256":
                result.loc[
                    0,
                    f"{verified_reconciled_current_latest_extended_complete_final_64_prefix}_ack_verified_reconciled_current_latest_extended_complete_final_review_carried_application_lineage_sha256",
                ] = value
            elif field not in {"required", "matches"}:
                result.loc[
                    0,
                    f"{verified_reconciled_current_latest_extended_complete_final_64_prefix}_{field}",
                ] = value
        result.loc[
            0,
            f"{confirmed_verified_reconciled_current_latest_extended_complete_final_72_prefix}_lineage_match_required",
        ] = confirmed_verified_reconciled_current_latest_extended_complete_final_lineage_72[
            "required"
        ]
        result.loc[
            0,
            f"{confirmed_verified_reconciled_current_latest_extended_complete_final_72_prefix}_lineage_matches",
        ] = confirmed_verified_reconciled_current_latest_extended_complete_final_lineage_72[
            "matches"
        ]
        for field, value in (
            confirmed_verified_reconciled_current_latest_extended_complete_final_lineage_72.items()
        ):
            if field == "carried_application_lineage_sha256":
                result.loc[
                    0,
                    f"{confirmed_verified_reconciled_current_latest_extended_complete_final_72_prefix}_ack_confirmed_verified_reconciled_current_latest_extended_complete_final_review_carried_application_lineage_sha256",
                ] = value
            elif field not in {"required", "matches"}:
                result.loc[
                    0,
                    f"{confirmed_verified_reconciled_current_latest_extended_complete_final_72_prefix}_{field}",
                ] = value
    return result
...
def broker_bound_ack_lineage(**overrides):
    lineage = empty_broker_dispatch_ack_lineage(required=True)
    lineage.update(
        {
            "provided": True,
            "manifest_current": True,
            "manifest_run_type": "broker_dispatch_ack_reconciliation",
            "manifest_path": "ack/manifest.json",
            "manifest_sha256": "a" * 64,
            "contract_consistent": True,
            "non_authorizing": True,
            "send_lineage_gate_passed": True,
            "send_matches_current": True,
            "expected_send_matches_current": True,
            "gate_passed": True,
            "broker_dispatch_send_lineage_gate_passed": True,
            "broker_dispatch_send_broker_dispatch_route_enable_cutover_broker_readiness_required": True,
            "broker_dispatch_send_broker_dispatch_route_enable_cutover_runtime_lineage_source_bound": True,
            "broker_dispatch_send_broker_dispatch_route_enable_cutover_current_runtime_session_manifest_sha256": "b" * 64,
            "broker_dispatch_send_broker_dispatch_route_enable_cutover_runtime_lineage_matches_current": True,
            "broker_dispatch_send_broker_dispatch_route_enable_cutover_broker_readiness_source_matches_scaleup": True,
            "broker_dispatch_send_broker_dispatch_route_enable_cutover_current_broker_readiness_manifest_sha256": "c" * 64,
            "broker_dispatch_send_broker_dispatch_route_enable_cutover_broker_readiness_matches_current": True,
        }
    )
...
def test_broker_dispatch_roundtrip_carries_broker_bound_lineage_assertions():
    report = evaluate_broker_dispatch_roundtrip(
        dispatch_summary=dispatch_summary(),
        dispatch_orders=dispatch_orders(),
        send_summary=send_summary(),
        ...
        ack_summary=ack_summary(),
        acknowledgements=acknowledgements(),
        broker_dispatch_ack_lineage=broker_bound_ack_lineage(),
    )
    ...
    assert (
        summary[
            "broker_dispatch_ack_broker_dispatch_send_broker_dispatch_route_enable_cutover_current_broker_readiness_manifest_sha256"
        ]
        == "c" * 64
    )
...
def test_broker_dispatch_roundtrip_blocks_stale_broker_lineage_with_green_ack_gate(
    lineage_field,
    check,
):
    report = evaluate_broker_dispatch_roundtrip(
    ...
    assert bool(report.summary.iloc[0]["broker_dispatch_ack_lineage_gate_passed"])
    ...
    assert action["recommendation"] == (
        "rebuild_broker_readiness_lineage_before_roundtrip_review"
    )
...
def test_broker_dispatch_roundtrip_carries_target_application_vendor_batch():
    ...
    ack_current_48_prefix = (
        "ack_current_latest_extended_complete_final_"
        "broker_dispatch_roundtrip_vendor_market_data_batch"
    )
    ack_view_48 = ack_view_48_target_application_lineage_comparison(vendor)
    assert summary[f"{ack_current_48_prefix}_lineage_match_required"]
    assert summary[f"{ack_current_48_prefix}_lineage_matches"]
    compatibility_only_fields = {
        "broker_readiness_extended_complete_final_review_carried_application_lineage_sha256",
        "scaleup_extended_complete_final_review_carried_application_lineage_sha256",
        "cutover_extended_complete_final_review_carried_application_lineage_sha256",
    }
    for field, expected in ack_view_48.items():
        ...
        assert summary[f"{ack_current_48_prefix}_{summary_field}"] == expected
    assert (
        summary[
            f"{ack_current_48_prefix}_roundtrip_current_latest_extended_complete_final_review_carried_application_lineage_sha256"
        ]
        == lineage_sha256
    )
    roundtrip_view_49 = report.config[
        "roundtrip_current_latest_extended_complete_final_broker_dispatch_roundtrip_vendor_market_data_batch_lineage_comparison"
    ]
    assert roundtrip_view_49["required"]
    assert roundtrip_view_49["matches"]
    for field, expected in ack_view_48.items():
        if field in {
            "required",
            "matches",
            ...
            *compatibility_only_fields,
            ...
            continue
        assert roundtrip_view_49[field] == expected
    assert (
        roundtrip_view_49[
            "ack_current_latest_extended_complete_final_review_carried_application_lineage_sha256"
        ]
    ...
    assert roundtrip_view_49["carried_application_lineage_sha256"] == lineage_sha256
    ack_reconciled_56_prefix = (
        "ack_reconciled_current_latest_extended_complete_final_"
        "broker_dispatch_roundtrip_vendor_market_data_batch"
    )
    ack_view_56 = ack_view_56_target_application_lineage_comparison(vendor)
    assert summary[f"{ack_reconciled_56_prefix}_lineage_match_required"]
    assert summary[f"{ack_reconciled_56_prefix}_lineage_matches"]
    for field, expected in ack_view_56.items():
        if field in {"required", "matches", "carried_application_lineage_sha256"}:
            continue
        assert summary[f"{ack_reconciled_56_prefix}_{field}"] == expected
    assert (
        summary[
            f"{ack_reconciled_56_prefix}_roundtrip_reconciled_current_latest_extended_complete_final_review_carried_application_lineage_sha256"
        ]
        == lineage_sha256
    )
    roundtrip_view_57 = report.config[
    ...
    assert len(roundtrip_view_57) == 56
    assert roundtrip_view_57 == roundtrip_view_57_target_application_lineage_comparison(
        vendor
    )
    ack_verified_reconciled_64_prefix = (
        "ack_verified_reconciled_current_latest_extended_complete_final_"
        "broker_dispatch_roundtrip_vendor_market_data_batch"
    )
    ack_view_64 = ack_view_64_target_application_lineage_comparison(vendor)
    assert len(ack_view_64) == 63
    assert summary[f"{ack_verified_reconciled_64_prefix}_lineage_match_required"]
    assert summary[f"{ack_verified_reconciled_64_prefix}_lineage_matches"]
    for field, expected in ack_view_64.items():
        if field in {"required", "matches", "carried_application_lineage_sha256"}:
            continue
        assert summary[f"{ack_verified_reconciled_64_prefix}_{field}"] == expected
    assert (
        summary[
            f"{ack_verified_reconciled_64_prefix}_roundtrip_verified_reconciled_current_latest_extended_complete_final_review_carried_application_lineage_sha256"
        ]
        == lineage_sha256
    )
    roundtrip_view_65 = report.config[
    ...
    assert len(roundtrip_view_65) == 64
    assert roundtrip_view_65 == roundtrip_view_65_target_application_lineage_comparison(
        vendor
    )
    ack_confirmed_verified_reconciled_72_prefix = (
        "ack_confirmed_verified_reconciled_current_latest_extended_complete_final_"
        "broker_dispatch_roundtrip_vendor_market_data_batch"
    )
    ack_view_72 = ack_view_72_target_application_lineage_comparison(vendor)
    assert len(ack_view_72) == 71
    assert summary[f"{ack_confirmed_verified_reconciled_72_prefix}_lineage_match_required"]
    assert summary[f"{ack_confirmed_verified_reconciled_72_prefix}_lineage_matches"]
    for field, expected in ack_view_72.items():
        if field in {"required", "matches", "carried_application_lineage_sha256"}:
            continue
        assert summary[f"{ack_confirmed_verified_reconciled_72_prefix}_{field}"] == expected
    assert (
        summary[
            f"{ack_confirmed_verified_reconciled_72_prefix}_roundtrip_confirmed_verified_reconciled_current_latest_extended_complete_final_review_carried_application_lineage_sha256"
        ]
        == lineage_sha256
    )
    roundtrip_view_73 = report.config[
    ...
    assert len(roundtrip_view_73) == 72
    assert roundtrip_view_73 == roundtrip_view_73_target_application_lineage_comparison(
        vendor
    )
    ...
    view_48_check_prefix = (
        "broker_dispatch_roundtrip_vendor_market_data_batch_"
        "ack_current_latest_extended_complete_final"
    )
    expected_checks.update(
        {
            f"{view_48_check_prefix}_lineage_match_required",
            f"{view_48_check_prefix}_lineage_matches",
            f"{view_48_check_prefix}_source_lineage_sha256_matches",
            f"{view_48_check_prefix}_compatibility_broker_lineage_sha256_matches",
            f"{view_48_check_prefix}_compatibility_ack_latest_extended_complete_final_review_carried_lineage_sha256_matches",
            f"{view_48_check_prefix}_broker_readiness_latest_extended_complete_final_review_carried_lineage_sha256_matches",
            f"{view_48_check_prefix}_scaleup_latest_extended_complete_final_review_carried_lineage_sha256_matches",
            f"{view_48_check_prefix}_cutover_latest_extended_complete_final_review_carried_lineage_sha256_matches",
            f"{view_48_check_prefix}_route_latest_extended_complete_final_review_carried_lineage_sha256_matches",
            f"{view_48_check_prefix}_dispatch_latest_extended_complete_final_review_carried_lineage_sha256_matches",
            f"{view_48_check_prefix}_send_latest_extended_complete_final_review_carried_lineage_sha256_matches",
            f"{view_48_check_prefix}_ack_current_latest_extended_complete_final_review_carried_lineage_sha256_matches",
            f"{view_48_check_prefix}_roundtrip_current_latest_extended_complete_final_review_carried_lineage_sha256_matches",
        }
    )
    for stage in (
        "prior_scaleup",
        ...
        expected_checks.add(
            f"{view_48_check_prefix}_{stage}_carried_lineage_sha256_matches"
        )
    view_56_check_prefix = (
        "broker_dispatch_roundtrip_vendor_market_data_batch_"
        "ack_reconciled_current_latest_extended_complete_final"
    )
    expected_checks.update(
        {
            f"{view_56_check_prefix}_lineage_match_required",
            f"{view_56_check_prefix}_lineage_matches",
            f"{view_56_check_prefix}_source_lineage_sha256_matches",
            f"{view_56_check_prefix}_compatibility_broker_lineage_sha256_matches",
            f"{view_56_check_prefix}_compatibility_roundtrip_current_latest_extended_complete_final_review_carried_lineage_sha256_matches",
        }
    )
    expected_checks.update(
        f"{view_56_check_prefix}_{stage}_carried_lineage_sha256_matches"
        for stage in (
    ...
    view_64_check_prefix = (
        "broker_dispatch_roundtrip_vendor_market_data_batch_"
        "ack_verified_reconciled_current_latest_extended_complete_final"
    )
    expected_checks.update(
        {
            f"{view_64_check_prefix}_lineage_match_required",
            f"{view_64_check_prefix}_lineage_matches",
            f"{view_64_check_prefix}_source_lineage_sha256_matches",
            f"{view_64_check_prefix}_compatibility_broker_lineage_sha256_matches",
            f"{view_64_check_prefix}_compatibility_roundtrip_verified_reconciled_current_latest_extended_complete_final_review_carried_lineage_sha256_matches",
        }
    )
    source_view_64_fields = set(ack_view_64) - {
        "required",
        "matches",
        "current_application_lineage_sha256",
        "broker_application_lineage_sha256",
    ...
    for field in source_view_64_fields:
        ...
        if stage == "scaleup":
            ...
            stage = "prior_cutover"
        expected_checks.add(
            f"{view_64_check_prefix}_{stage}_carried_lineage_sha256_matches"
        )
    expected_checks.update(
        {
            f"{view_64_check_prefix}_ack_verified_reconciled_current_latest_extended_complete_final_review_generic_carried_lineage_sha256_matches",
            f"{view_64_check_prefix}_roundtrip_verified_reconciled_current_latest_extended_complete_final_review_carried_lineage_sha256_matches",
        }
    )
    view_72_check_prefix = (
        "broker_dispatch_roundtrip_vendor_market_data_batch_"
        "ack_confirmed_verified_reconciled_current_latest_extended_complete_final"
    )
    expected_checks.update(
        {
            f"{view_72_check_prefix}_lineage_match_required",
            f"{view_72_check_prefix}_lineage_matches",
            f"{view_72_check_prefix}_source_lineage_sha256_matches",
            f"{view_72_check_prefix}_compatibility_broker_lineage_sha256_matches",
            f"{view_72_check_prefix}_compatibility_roundtrip_verified_reconciled_current_latest_extended_complete_final_review_carried_lineage_sha256_matches",
        }
    )
    source_view_72_fields = set(ack_view_72) - {
        "required",
        "matches",
        "current_application_lineage_sha256",
        "broker_application_lineage_sha256",
    ...
    for field in source_view_72_fields:
        stage = field.removesuffix("_carried_application_lineage_sha256")
        if stage == "scaleup":
            stage = "prior_scaleup"
        elif stage == "cutover":
            stage = "prior_cutover"
        expected_checks.add(
            f"{view_72_check_prefix}_{stage}_carried_lineage_sha256_matches"
        )
    expected_checks.update(
        {
            f"{view_72_check_prefix}_ack_confirmed_verified_reconciled_current_latest_extended_complete_final_review_generic_carried_lineage_sha256_matches",
            f"{view_72_check_prefix}_roundtrip_confirmed_verified_reconciled_current_latest_extended_complete_final_review_carried_lineage_sha256_matches",
        }
    )
...
def test_broker_dispatch_roundtrip_blocks_ack_view_48_drift_while_preserving_view_41():
    vendor = target_application_vendor_market_data_batch_config()
    lineage_sha256 = target_application_lineage_sha256(vendor["datasets"])
    dispatch_config = route_enable_config()
    dispatch_config["route_broker_dispatch_roundtrip_vendor_market_data_batch"] = vendor
    ...
    assert roundtrip_view_49["broker_application_lineage_sha256"] == "f" * 64
    assert (
        roundtrip_view_49[
            "ack_current_latest_extended_complete_final_review_carried_application_lineage_sha256"
        ]
...
def test_broker_dispatch_roundtrip_blocks_ack_view_56_drift_while_preserving_view_49():
    vendor = target_application_vendor_market_data_batch_config()
    lineage_sha256 = target_application_lineage_sha256(vendor["datasets"])
    dispatch_config = route_enable_config()
    dispatch_config["route_broker_dispatch_roundtrip_vendor_market_data_batch"] = vendor
    ...
    ack_config[
        "ack_reconciled_current_latest_extended_complete_final_broker_dispatch_roundtrip_vendor_market_data_batch_lineage_comparison"
    ] = ack_view_56_target_application_lineage_comparison(
        vendor,
        lineage_sha256="f" * 64,
    )
    ...
    assert {
        ...
        f"{check_prefix}_roundtrip_reconciled_current_latest_extended_complete_final_review_carried_lineage_sha256_matches",
    } <= failed
    roundtrip_view_49 = report.config[
        "roundtrip_current_latest_extended_complete_final_broker_dispatch_roundtrip_vendor_market_data_batch_lineage_comparison"
    ]
    assert roundtrip_view_49["required"]
    assert roundtrip_view_49["matches"]
    assert roundtrip_view_49["broker_application_lineage_sha256"] == lineage_sha256
    assert (
        roundtrip_view_49[
            "ack_current_latest_extended_complete_final_review_carried_application_lineage_sha256"
        ]
    ...
    assert roundtrip_view_57["broker_application_lineage_sha256"] == "f" * 64
    assert (
        roundtrip_view_57[
    ...
    assert roundtrip_view_57["carried_application_lineage_sha256"] == lineage_sha256


def test_broker_dispatch_roundtrip_blocks_ack_view_64_drift_while_preserving_view_57():
    vendor = target_application_vendor_market_data_batch_config()
    lineage_sha256 = target_application_lineage_sha256(vendor["datasets"])
    ...
    ack_config[
        "ack_verified_reconciled_current_latest_extended_complete_final_broker_dispatch_roundtrip_vendor_market_data_batch_lineage_comparison"
    ] = ack_view_64_target_application_lineage_comparison(
        vendor,
        lineage_sha256=drifted_lineage_sha256,
    )
    ack_summary_input = with_broker_vendor_batch_summary(
        ack_summary(),
        vendor,
        prefix="ack_broker_dispatch_roundtrip_vendor_market_data_batch",
    )
    ...
    drifted_view_64 = ack_view_64_target_application_lineage_comparison(
        vendor,
        lineage_sha256=drifted_lineage_sha256,
    )
    for field, value in drifted_view_64.items():
        if field == "required":
            ack_summary_input.loc[
                0,
                f"{summary_prefix}_lineage_match_required",
            ] = value
        elif field == "matches":
            ack_summary_input.loc[0, f"{summary_prefix}_lineage_matches"] = value
        elif field == "carried_application_lineage_sha256":
            ack_summary_input.loc[
                0,
                f"{summary_prefix}_ack_verified_reconciled_current_latest_extended_complete_final_review_carried_application_lineage_sha256",
            ] = value
        else:
            ack_summary_input.loc[0, f"{summary_prefix}_{field}"] = value
    ...
    assert roundtrip_view_57["broker_application_lineage_sha256"] == lineage_sha256
    assert roundtrip_view_57["broker_application_lineage_sha256"] != (
        drifted_lineage_sha256
    )
    roundtrip_view_65 = report.config[
        "roundtrip_verified_reconciled_current_latest_extended_complete_final_broker_dispatch_roundtrip_vendor_market_data_batch_lineage_comparison"
    ]
    assert roundtrip_view_65["broker_application_lineage_sha256"] == (
        drifted_lineage_sha256
    )
    assert (
        roundtrip_view_65[
    ...
    assert roundtrip_view_65["carried_application_lineage_sha256"] == lineage_sha256


def test_broker_dispatch_roundtrip_blocks_ack_view_72_drift_while_preserving_view_65():
    vendor = target_application_vendor_market_data_batch_config()
    lineage_sha256 = target_application_lineage_sha256(vendor["datasets"])
    drifted_lineage_sha256 = "f" * 64
    dispatch_config = route_enable_config()
    ...
    add_ack_complete_final_target_application_lineage(ack_config, vendor)
    drifted_view_72 = ack_view_72_target_application_lineage_comparison(
        vendor,
        lineage_sha256=drifted_lineage_sha256,
    )
    assert len(drifted_view_72) == 71
    ack_config[
        "ack_confirmed_verified_reconciled_current_latest_extended_complete_final_broker_dispatch_roundtrip_vendor_market_data_batch_lineage_comparison"
    ] = drifted_view_72
    ...
    assert roundtrip_view_65["broker_application_lineage_sha256"] == lineage_sha256
    assert roundtrip_view_65["broker_application_lineage_sha256"] != (
        drifted_lineage_sha256
    )
    roundtrip_view_73 = report.config[
    ...
    assert roundtrip_view_73 == roundtrip_view_73_target_application_lineage_comparison(
        vendor,
        lineage_sha256=drifted_lineage_sha256,
        roundtrip_lineage_sha256=lineage_sha256,
    )


def test_broker_dispatch_roundtrip_requires_ack_view_72_lineage():
    vendor = target_application_vendor_market_data_batch_config()
    dispatch_config = route_enable_config()
    dispatch_config["route_broker_dispatch_roundtrip_vendor_market_data_batch"] = vendor
    send_config = route_enable_config()
    ...
    assert not roundtrip_view_73["required"]
    assert not roundtrip_view_73["matches"]
...
@pytest.mark.parametrize(
    ...
    [
        (
            "required",
            False,
            "broker_dispatch_roundtrip_vendor_market_data_batch_ack_confirmed_verified_reconciled_current_latest_extended_complete_final_lineage_match_required",
        ),
        (
            "matches",
            False,
            "broker_dispatch_roundtrip_vendor_market_data_batch_ack_confirmed_verified_reconciled_current_latest_extended_complete_final_lineage_matches",
        ),
        (
            "current_application_lineage_sha256",
            "f" * 64,
            "broker_dispatch_roundtrip_vendor_market_data_batch_ack_confirmed_verified_reconciled_current_latest_extended_complete_final_source_lineage_sha256_matches",
        ),
        (
            "ack_confirmed_verified_reconciled_current_latest_extended_complete_final_review_carried_application_lineage_sha256",
            "f" * 64,
            "broker_dispatch_roundtrip_vendor_market_data_batch_ack_confirmed_verified_reconciled_current_latest_extended_complete_final_ack_confirmed_verified_reconciled_current_latest_extended_complete_final_review_carried_lineage_sha256_matches",
        ),
        (
            "carried_application_lineage_sha256",
            "f" * 64,
            "broker_dispatch_roundtrip_vendor_market_data_batch_ack_confirmed_verified_reconciled_current_latest_extended_complete_final_ack_confirmed_verified_reconciled_current_latest_extended_complete_final_review_generic_carried_lineage_sha256_matches",
        ),
    ],
)
def test_broker_dispatch_roundtrip_blocks_invalid_ack_view_72_lineage(
    field,
    value,
    ...
    add_ack_complete_final_target_application_lineage(ack_config, vendor)
    ack_view_72 = ack_config[
        "ack_confirmed_verified_reconciled_current_latest_extended_complete_final_broker_dispatch_roundtrip_vendor_market_data_batch_lineage_comparison"
    ]
    ack_view_72[field] = value
...
def test_broker_dispatch_roundtrip_requires_ack_view_64_lineage():
    vendor = target_application_vendor_market_data_batch_config()
    dispatch_config = route_enable_config()
    dispatch_config["route_broker_dispatch_roundtrip_vendor_market_data_batch"] = vendor
    send_config = route_enable_config()
    ...
    assert not roundtrip_view_65["required"]
    assert not roundtrip_view_65["matches"]
...
@pytest.mark.parametrize(
    ...
    [
        (
            "required",
            False,
            "broker_dispatch_roundtrip_vendor_market_data_batch_ack_verified_reconciled_current_latest_extended_complete_final_lineage_match_required",
        ),
        (
            "matches",
            False,
            "broker_dispatch_roundtrip_vendor_market_data_batch_ack_verified_reconciled_current_latest_extended_complete_final_lineage_matches",
        ),
        (
            "current_application_lineage_sha256",
            "f" * 64,
            "broker_dispatch_roundtrip_vendor_market_data_batch_ack_verified_reconciled_current_latest_extended_complete_final_source_lineage_sha256_matches",
        ),
        (
            "ack_verified_reconciled_current_latest_extended_complete_final_review_carried_application_lineage_sha256",
            "f" * 64,
            "broker_dispatch_roundtrip_vendor_market_data_batch_ack_verified_reconciled_current_latest_extended_complete_final_ack_verified_reconciled_current_latest_extended_complete_final_review_carried_lineage_sha256_matches",
        ),
        (
            "carried_application_lineage_sha256",
            "f" * 64,
            "broker_dispatch_roundtrip_vendor_market_data_batch_ack_verified_reconciled_current_latest_extended_complete_final_ack_verified_reconciled_current_latest_extended_complete_final_review_generic_carried_lineage_sha256_matches",
        ),
    ],
)
def test_broker_dispatch_roundtrip_blocks_invalid_ack_view_64_lineage(
    field,
    value,
    expected_failed_check,
):
    ...
    add_ack_complete_final_target_application_lineage(ack_config, vendor)
    ack_view_64 = ack_config[
        "ack_verified_reconciled_current_latest_extended_complete_final_broker_dispatch_roundtrip_vendor_market_data_batch_lineage_comparison"
    ]
    ack_view_64[field] = value
...
def test_broker_dispatch_roundtrip_requires_ack_view_56_lineage():
    vendor = target_application_vendor_market_data_batch_config()
    dispatch_config = route_enable_config()
    dispatch_config["route_broker_dispatch_roundtrip_vendor_market_data_batch"] = vendor
    send_config = route_enable_config()
    ...
    assert not roundtrip_view_57["required"]
    assert not roundtrip_view_57["matches"]
...
@pytest.mark.parametrize(
    ...
    [
        (
            "required",
            False,
            "broker_dispatch_roundtrip_vendor_market_data_batch_ack_reconciled_current_latest_extended_complete_final_lineage_match_required",
        ),
        (
            "matches",
            False,
            "broker_dispatch_roundtrip_vendor_market_data_batch_ack_reconciled_current_latest_extended_complete_final_lineage_matches",
        ),
        (
            "current_application_lineage_sha256",
            "f" * 64,
            "broker_dispatch_roundtrip_vendor_market_data_batch_ack_reconciled_current_latest_extended_complete_final_source_lineage_sha256_matches",
        ),
        (
            "broker_readiness_current_latest_extended_complete_final_review_carried_application_lineage_sha256",
            "f" * 64,
            "broker_dispatch_roundtrip_vendor_market_data_batch_ack_reconciled_current_latest_extended_complete_final_broker_readiness_current_latest_extended_complete_final_review_carried_lineage_sha256_matches",
        ),
        (
            "carried_application_lineage_sha256",
            "f" * 64,
            "broker_dispatch_roundtrip_vendor_market_data_batch_ack_reconciled_current_latest_extended_complete_final_ack_reconciled_current_latest_extended_complete_final_review_generic_carried_lineage_sha256_matches",
        ),
    ],
)
def test_broker_dispatch_roundtrip_blocks_invalid_ack_view_56_lineage(
    field,
    value,
    expected_failed_check,
):
    ...
    add_ack_complete_final_target_application_lineage(ack_config, vendor)
    ack_view_56 = ack_config[
        "ack_reconciled_current_latest_extended_complete_final_broker_dispatch_roundtrip_vendor_market_data_batch_lineage_comparison"
    ]
    ack_view_56[field] = value
...
def test_broker_dispatch_roundtrip_requires_ack_view_48_lineage():
    vendor = target_application_vendor_market_data_batch_config()
    dispatch_config = route_enable_config()
    dispatch_config["route_broker_dispatch_roundtrip_vendor_market_data_batch"] = vendor
    send_config = route_enable_config()
    ...
    assert not roundtrip_view_49["required"]
    assert not roundtrip_view_49["matches"]
...
@pytest.mark.parametrize(
    ...
    [
        (
            "required",
            False,
            "broker_dispatch_roundtrip_vendor_market_data_batch_ack_current_latest_extended_complete_final_lineage_match_required",
        ),
        (
            "matches",
            False,
            "broker_dispatch_roundtrip_vendor_market_data_batch_ack_current_latest_extended_complete_final_lineage_matches",
        ),
        (
            "current_application_lineage_sha256",
            "f" * 64,
            "broker_dispatch_roundtrip_vendor_market_data_batch_ack_current_latest_extended_complete_final_source_lineage_sha256_matches",
        ),
        (
            "send_extended_complete_final_review_carried_application_lineage_sha256",
            "f" * 64,
            "broker_dispatch_roundtrip_vendor_market_data_batch_ack_current_latest_extended_complete_final_send_extended_complete_final_review_carried_lineage_sha256_matches",
        ),
        (
            "carried_application_lineage_sha256",
            "f" * 64,
            "broker_dispatch_roundtrip_vendor_market_data_batch_ack_current_latest_extended_complete_final_ack_current_latest_extended_complete_final_review_carried_lineage_sha256_matches",
        ),
    ],
)
def test_broker_dispatch_roundtrip_blocks_invalid_ack_view_48_lineage(
    field,
    value,
    expected_failed_check,
):
    ...
    add_ack_complete_final_target_application_lineage(ack_config, vendor)
    ack_view_48 = ack_config[
        "ack_current_latest_extended_complete_final_broker_dispatch_roundtrip_vendor_market_data_batch_lineage_comparison"
    ]
    ack_view_48[field] = value
...
def test_broker_dispatch_roundtrip_carries_target_application_batch_from_component_summaries():
    ...
    assert (
        ...
        == roundtrip_view_49["broker_application_lineage_sha256"]
    )
    assert (
        roundtrip_view_49[
            "roundtrip_current_latest_extended_complete_final_review_carried_application_lineage_sha256"
    ...
    assert roundtrip_view_49["carried_application_lineage_sha256"] == (
        roundtrip_view_49["broker_application_lineage_sha256"]
    )
    roundtrip_view_57 = report.config[
        "roundtrip_reconciled_current_latest_extended_complete_final_broker_dispatch_roundtrip_vendor_market_data_batch_lineage_comparison"
