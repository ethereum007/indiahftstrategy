...
def test_run_imbalance_sweep_writes_runs_summary_and_proof(tmp_path):
    ...
    assert int(
        sweep.summary.iloc[0]["total_terminal_liquidation_shortfall_qty"]
    ) == 0
    assert int(
        sweep.summary.iloc[0]["total_terminal_residual_position_qty"]
    ) == 0
    ...
    assert sweep.runs["terminal_liquidation_complete"].all()
