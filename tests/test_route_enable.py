...
def route_contract_identity_route_enable_inputs(tmp_path_factory):
    ...
    return cutover, upload, export, broker_fields


@pytest.fixture(scope="module")
def route_enable_route_contract_identity_route_enable_inputs(
    tmp_path_factory,
):
    from reports.cutover import write_cutover_gate_report
    from tests.test_cutover_gate import (
    ...
    root = tmp_path_factory.mktemp("rei")
    (
        scaleup,
        broker,
        runtime,
...
@pytest.fixture(scope="module")
def route_enable_route_enable_route_contract_identity_route_enable_inputs(
    tmp_path_factory,
):
    from reports.cutover import write_cutover_gate_report
    from tests.test_cutover_gate import (
    ...
    if os.name == "nt":
        ...
        root = Path(tempfile.mkdtemp(prefix="rei_", dir=short_root))
    else:
        root = tmp_path_factory.mktemp("route_enable_terminal_identity")
    try:
        (
            scaleup,
            broker,
        ...
        yield cutover, upload, export, broker_fields
    finally:
        if os.name == "nt":
...
@pytest.fixture(scope="module")
def route_enable_route_enable_route_enable_route_contract_identity_route_enable_inputs(
    tmp_path_factory,
):
    from reports.cutover import write_cutover_gate_report
    from tests.test_cutover_gate import (
    ...
    if os.name == "nt":
        short_root = Path(Path.cwd().anchor) / ".hft_test_tmp"
        short_root.mkdir(exist_ok=True)
        root = Path(tempfile.mkdtemp(prefix="re5_", dir=short_root))
    else:
        root = tmp_path_factory.mktemp("route_enable_terminal_identity_fifth")
    try:
        (
            scaleup,
            broker,
...
def forge_cutover_route_contract_identity(cutover, forged_sha256):
    ...
    current_sha256 = str(summary.loc[0, fields[-1]])
    ...
    for name in ("cutover_authorization.csv", "cutover_summary.csv"):
        ...
        for field in fields:
            ...
            frame.loc[0, field] = forged_sha256
        frame.to_csv(path, index=False)

    config_path = cutover / "cutover_config.json"
    config = json.loads(config_path.read_text(encoding="utf-8"))
    runtime_lineage = config["runtime_lineage"]
    for field in fields:
        assert field in runtime_lineage
        runtime_lineage[field] = forged_sha256
    config_path.write_text(
        json.dumps(config, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
...
def forge_cutover_route_enable_route_contract_identity(
    cutover,
    forged_sha256,
):
    from tests.data_readiness_helpers import reseal_experiment_manifest
...
def forge_cutover_route_enable_route_enable_route_contract_identity(
    cutover,
    forged_sha256,
):
    from tests.data_readiness_helpers import reseal_experiment_manifest
...
def forge_cutover_route_enable_route_enable_route_enable_route_contract_identity(
    cutover,
    forged_sha256,
):
    from tests.data_readiness_helpers import reseal_experiment_manifest
...
def test_route_enable_verifies_cutover_broker_route_contract_identity(
    tmp_path,
    route_contract_identity_route_enable_inputs,
):
    (
        cutover,
        upload,
    ...
    runbook = (out_dir / "route_enable_runbook.md").read_text(
        encoding="utf-8"
    )
    assert (
    ...
    lineage = load_route_enable_lineage(
        out_dir / "route_enable_config.json"
    )
    assert lineage["gate_passed"]
    assert lineage["cutover_route_contract_identity_active"]
    assert (
        lineage["current_cutover_route_contract_identity_sha256"]
        == identity_sha256
    )
    assert lineage["cutover_route_contract_identity_matches_current"]


def test_route_enable_blocks_remanifested_cutover_route_identity_forgery(
    tmp_path,
    route_contract_identity_route_enable_inputs,
):
    (
    ...
    cutover = tmp_path / "forged_cutover"
    shutil.copytree(cutover_source, cutover)
    forged_sha256 = "e" * 64
    current_sha256 = forge_cutover_route_contract_identity(
        cutover,
        forged_sha256,
    )
    manifest_path = cutover / "manifest.json"
    ...
    assert not lineage["cutover_matches_current"]
    assert not lineage["cutover_route_contract_identity_matches_current"]
    assert not lineage["gate_passed"]


def test_route_enable_verifies_cutover_broker_route_enable_route_identity(
    tmp_path,
    route_enable_route_contract_identity_route_enable_inputs,
):
    (
        cutover,
        upload,
    ...
    assert (
        (
            "Broker route-enable route contract identity digest: "
            f"`{identity_sha256}`"
        )
        in runbook
    )
    assert (
        "Broker route-enable route contract identity matches current: yes"
        in runbook
    )
...
def test_route_enable_blocks_remanifested_cutover_route_enable_forgery(
    tmp_path,
    route_enable_route_contract_identity_route_enable_inputs,
):
    (
    ...
    shutil.copytree(cutover_source, cutover)
    forged_sha256 = "e" * 64
    current_sha256 = forge_cutover_route_enable_route_contract_identity(
        cutover,
        forged_sha256,
    )
    manifest_path = cutover / "manifest.json"
...
def test_route_enable_verifies_cutover_broker_route_enable_route_enable_identity(
    tmp_path,
    route_enable_route_enable_route_contract_identity_route_enable_inputs,
):
    (
        cutover,
        upload,
    ...
    assert summary[verdict_field]
    assert report.config["cutover_lineage"][verdict_field]
    ...
    downstream = route_enable_lineage_fields(lineage)
    assert downstream[
        (
...
def test_route_enable_blocks_remanifested_cutover_route_enable_route_enable_forgery(
    tmp_path,
    route_enable_route_enable_route_contract_identity_route_enable_inputs,
):
    (
    ...
    forged_sha256 = "c" * 64
    current_sha256 = (
        forge_cutover_route_enable_route_enable_route_contract_identity(
            cutover,
            forged_sha256,
        )
    )
    ...
    assert not summary[verdict_field]
    action = report.action_queue.loc[
        report.action_queue["check"] == verdict_field
    ].iloc[0]
    assert action["component"] == "broker_readiness"
    assert action["next_gate"] == "review-broker-readiness"
    assert verify_experiment_manifest(
...
def test_route_enable_verifies_cutover_broker_route_enable_route_enable_route_enable_identity(
    tmp_path,
    route_enable_route_enable_route_enable_route_contract_identity_route_enable_inputs,
):
    (
        cutover,
        upload,
    ...
    assert (
        (
            "Broker route-enable route-enable route-enable route contract "
            f"identity digest: `{identity_sha256}`"
        )
        in runbook
    )
    assert (
    ...
    assert lineage["gate_passed"]
    assert lineage[f"{aggregate_prefix}active"]
    assert lineage[
        (
    ...
    assert lineage[f"{aggregate_prefix}matches_current"]
    downstream = route_enable_lineage_fields(lineage)
    downstream_prefix = f"route_enable_{aggregate_prefix}"
    assert downstream[f"{downstream_prefix}active"]
    assert downstream[
        (
            "route_enable_current_cutover_route_enable_route_enable_"
            "route_enable_route_contract_identity_sha256"
    ...
    assert downstream[f"{downstream_prefix}matches_current"]
    assert downstream["route_enable_lineage_gate_passed"]


def test_route_enable_blocks_remanifested_cutover_route_enable_route_enable_route_enable_forgery(
    tmp_path,
    route_enable_route_enable_route_enable_route_contract_identity_route_enable_inputs,
):
    (
    ...
    forged_sha256 = "d" * 64
    current_sha256 = (
        forge_cutover_route_enable_route_enable_route_enable_route_contract_identity(
            cutover,
            forged_sha256,
        )
    )
    ...
    assert "cutover_manifest_current" not in failed
    assert "cutover_lineage_contract_consistent" not in failed
    assert f"{runtime_field}_present" not in failed
    assert {
        f"{runtime_field}_matches_current",
        verdict_field,
        "cutover_runtime_lineage_matches_current",
        "cutover_lineage_gate_passed",
    } <= failed
    ...
    assert not lineage["cutover_matches_current"]
    assert not lineage[f"{aggregate_prefix}matches_current"]
    assert not lineage["gate_passed"]
    downstream = route_enable_lineage_fields(lineage)
    downstream_prefix = f"route_enable_{aggregate_prefix}"
    assert downstream[f"{downstream_prefix}active"]
    assert not downstream[f"{downstream_prefix}matches_current"]
    assert not downstream["route_enable_lineage_gate_passed"]
