...
from tests.test_broker_dispatch_send import (
    ...
    _write_route_identity_ack_chain,
...
def write_broker_readiness_input_dirs(
    ...
    *,
    ...
    if verified_roundtrip:
        if route_enable_route_enable_route_enable_route_contract_identity:
            if adapter != "arrow_money":
                raise ValueError(
                    "route-enable route-enable route contract identity "
                    "readiness fixture requires arrow_money"
                )
            route_identity_root = root / "t"
            route_identity_root.mkdir()
            dispatch, send, ack, _broker_fields = (
                _write_route_identity_ack_chain(
                    route_identity_root,
                    contract_identity=False,
        ...
        elif route_enable_route_enable_route_contract_identity:
            if adapter != "arrow_money":
                raise ValueError(
                    "route-enable route contract identity readiness fixture "
                    "requires arrow_money"
                )
            route_identity_root = root / "r"
            route_identity_root.mkdir()
            dispatch, send, ack, _broker_fields = (
                _write_route_identity_ack_chain(
                    route_identity_root,
        ...
        elif route_enable_route_contract_identity:
            if adapter != "arrow_money":
                raise ValueError(
                    "route-enable contract identity readiness fixture "
                    "requires arrow_money"
                )
            route_identity_root = root / "route_enable_identity"
            route_identity_root.mkdir()
            dispatch, send, ack, _broker_fields = (
                _write_route_identity_ack_chain(
                    route_identity_root,
        ...
        elif route_contract_identity:
            if adapter != "arrow_money":
                raise ValueError(
                    "route contract identity readiness fixture requires arrow_money"
                )
            route_identity_root = root / "route_identity_chain"
            route_identity_root.mkdir()
            dispatch, send, ack, _broker_fields = (
                _write_route_identity_ack_chain(route_identity_root)
            )
        elif contract_identity:
            if adapter != "arrow_money":
                raise ValueError(
...
def route_enable_route_identity_readiness_inputs(tmp_path_factory):
    root = tmp_path_factory.mktemp(
        "broker_route_identity"
    )
    ...
    return root, schema_dir, export_dir, upload_dir, roundtrip_dir


@pytest.fixture(scope="module")
def route_enable_route_enable_identity_readiness_inputs(
    tmp_path_factory,
):
...
@pytest.fixture(scope="module")
def route_enable_route_enable_route_enable_identity_readiness_inputs(
    tmp_path_factory,
):
    if os.name == "nt":
        short_root = Path(Path.cwd().anchor) / ".hft_test_tmp"
        short_root.mkdir(exist_ok=True)
        root = Path(tempfile.mkdtemp(prefix="brr_", dir=short_root))
    else:
        root = tmp_path_factory.mktemp("broker_readiness_terminal_identity")
    try:
        schema_dir, export_dir, upload_dir, roundtrip_dir = (
            write_broker_readiness_input_dirs(
                root,
        ...
        yield root, schema_dir, export_dir, upload_dir, roundtrip_dir
    finally:
        if os.name == "nt":
...
def rewrite_roundtrip_ack_lineage_field(roundtrip, column, value):
    ...
    orders[column] = value
    config["broker_dispatch_ack_lineage"][column] = value
    ...
    refresh_dispatch_manifest(
        roundtrip / "manifest.json",
        extra_updates={column: value},
    )


def forge_roundtrip_route_enable_route_contract_identity(
    roundtrip,
    forged_sha256,
):
    digest_fields = (
        ...
        (
            "broker_dispatch_ack_broker_dispatch_send_current_dispatch_"
        ...
        (
            "broker_dispatch_ack_broker_dispatch_send_broker_dispatch_"
            "route_enable_cutover_runtime_lineage_"
            "current_broker_readiness_route_contract_identity_sha256"
        ),
        (
            "broker_dispatch_ack_broker_dispatch_send_broker_dispatch_"
...
def forge_roundtrip_route_enable_route_enable_contract_identity(
    roundtrip,
    forged_sha256,
):
    summary = pd.read_csv(
...
def forge_roundtrip_route_enable_route_enable_route_enable_contract_identity(
    roundtrip,
    forged_sha256,
):
    summary = pd.read_csv(
...
def test_broker_readiness_revalidates_roundtrip_route_contract_identity(
    tmp_path,
):
    schema_dir, export_dir, upload_dir, roundtrip_dir = (
        write_broker_readiness_input_dirs(
    ...
    expected_sha256 = roundtrip_summary[
        BROKER_DISPATCH_ACK_ROUTE_CONTRACT_IDENTITY_SHA256_FIELD
    ]
    out_dir = tmp_path / "readiness"
    ...
    assert dispatch_item[current_field] == expected_sha256
    assert bool(dispatch_item[verdict_field])
    ...
    assert (
        lineage_config["current_ack_route_contract_identity_sha256"]
        == expected_sha256
    )
    assert lineage_config["ack_route_contract_identity_matches_current"]
    manifest = json.loads(
    ...
    assert (
        f"Terminal round-trip carried route contract identity: "
        f"`{expected_sha256}`"
        in runbook
    )
    assert (
        f"Current acknowledgement route contract identity: "
        f"`{expected_sha256}`"
        in runbook
    )
    assert (
        "Terminal round-trip route contract identity matches current: yes"
        in runbook
    )
    lineage = load_broker_readiness_lineage(
        out_dir / "broker_readiness_config.json"
...
def test_broker_readiness_blocks_remanifested_roundtrip_route_identity_forgery(
    tmp_path,
):
    schema_dir, export_dir, upload_dir, roundtrip_dir = (
        write_broker_readiness_input_dirs(
    ...
    current_sha256 = roundtrip_summary[
    ...
    assert not bool(summary["broker_dispatch_roundtrip_ack_matches_current"])
    assert not bool(
        summary["broker_dispatch_roundtrip_lineage_gate_passed"]
    )
    failed = set(
        report.checks.loc[
    ...
    assert "dispatch_roundtrip_lineage_manifest_current" not in failed
    assert "dispatch_roundtrip_lineage_contract_consistent" not in failed
    assert {
        "dispatch_roundtrip_lineage_ack_matches_current",
        (
            "dispatch_roundtrip_route_contract_identity_"
            "sha256_matches_current"
        ),
    ...
    assert lineage["manifest_current"]
    assert lineage["contract_consistent"], lineage["contract_error"]
    assert not lineage["roundtrip_lineage_gate_passed"]
    assert not lineage["roundtrip_matches_current"]
    assert not lineage[verdict_field]
    assert not lineage["gate_passed"]


def test_broker_readiness_revalidates_roundtrip_broker_route_contract_identity(
    route_enable_route_identity_readiness_inputs,
    tmp_path,
):
    _, schema_dir, export_dir, upload_dir, roundtrip_dir = (
        route_enable_route_identity_readiness_inputs
    )
    roundtrip_summary = pd.read_csv(
        roundtrip_dir / "broker_dispatch_roundtrip_summary.csv",
    ...
    identity_checks = report.checks.set_index("check")
    for check in (
        "dispatch_roundtrip_broker_route_contract_identity_sha256_present",
        (
            "dispatch_roundtrip_broker_route_contract_identity_"
            "sha256_matches_current"
        ),
        "dispatch_roundtrip_broker_route_contract_identity_matches_current",
    ):
        assert bool(identity_checks.loc[check, "passed"])
    dispatch_item = report.items.set_index("component").loc[
        "dispatch_roundtrip"
    ]
    assert bool(dispatch_item[active_field])
    assert dispatch_item[current_field] == expected_sha256
    assert bool(dispatch_item[verdict_field])
    lineage_config = report.config["dispatch_roundtrip"]["lineage"]
    assert lineage_config[
    ...
    assert (
        f"Terminal round-trip carried broker route contract identity: "
        f"`{expected_sha256}`"
        in runbook
    )
    assert (
        f"Current acknowledgement broker route contract identity: "
        f"`{expected_sha256}`"
        in runbook
    )
    assert (
        "Terminal round-trip broker route contract identity matches current: "
        "yes"
        in runbook
    )
    lineage = load_broker_readiness_lineage(
    ...
    assert lineage[verdict_field]
    assert lineage["gate_passed"]
    output_fields = broker_readiness_lineage_fields(lineage)
    ...
    assert (
        output_fields[
            "broker_readiness_roundtrip_current_ack_route_enable_"
            "route_contract_identity_sha256"
        ]
        == expected_sha256
    )
...
def test_broker_readiness_blocks_remanifested_roundtrip_broker_route_forgery(
    route_enable_route_identity_readiness_inputs,
    tmp_path,
):
    source_root, _, _, _, _ = route_enable_route_identity_readiness_inputs
    ...
    forged_sha256 = "a" * 64
    current_sha256 = (
        forge_roundtrip_route_enable_route_contract_identity(
            roundtrip_dir,
            forged_sha256,
        )
    )
...
def test_broker_readiness_revalidates_roundtrip_broker_route_enable_identity(
    route_enable_route_enable_identity_readiness_inputs,
    tmp_path,
):
    _, schema_dir, export_dir, upload_dir, roundtrip_dir = (
        route_enable_route_enable_identity_readiness_inputs
    ...
    for check in (
        (
            "dispatch_roundtrip_broker_route_enable_route_"
    ...
    assert bool(dispatch_item[active_field])
    assert dispatch_item[carried_field] == expected_sha256
    assert dispatch_item[current_field] == expected_sha256
    assert bool(dispatch_item[verdict_field])
    lineage_config = report.config["dispatch_roundtrip"]["lineage"]
    assert (
        lineage_config[
            carried_field.removeprefix(
    ...
    assert (
        "Terminal round-trip carried broker route-enable route contract "
        f"identity: `{expected_sha256}`"
        in runbook
    )
    assert (
        "Current acknowledgement broker route-enable route contract "
        f"identity: `{expected_sha256}`"
        in runbook
    )
    assert (
        "Terminal round-trip broker route-enable route contract identity "
        "matches current: yes"
        in runbook
    )
    lineage = load_broker_readiness_lineage(
...
def test_broker_readiness_blocks_remanifested_roundtrip_broker_route_enable_forgery(
    route_enable_route_enable_identity_readiness_inputs,
    tmp_path,
):
    _, schema_dir, export_dir, upload_dir, source_roundtrip_dir = (
        route_enable_route_enable_identity_readiness_inputs
    )
    roundtrip_dir = tmp_path / "r"
    shutil.copytree(source_roundtrip_dir, roundtrip_dir)
    carried_field = (
        BROKER_DISPATCH_ROUNDTRIP_ROUTE_ENABLE_ROUTE_ENABLE_ROUTE_CONTRACT_IDENTITY_SHA256_FIELD
    )
    forged_sha256 = "b" * 64
    current_sha256 = (
        forge_roundtrip_route_enable_route_enable_contract_identity(
            roundtrip_dir,
            forged_sha256,
        )
    )
...
def test_broker_readiness_revalidates_roundtrip_broker_route_enable_route_enable_identity(
    route_enable_route_enable_route_enable_identity_readiness_inputs,
    tmp_path,
):
    _, schema_dir, export_dir, upload_dir, roundtrip_dir = (
        route_enable_route_enable_route_enable_identity_readiness_inputs
    ...
    for check in (
        (
            "dispatch_roundtrip_broker_route_enable_route_enable_"
    ...
    assert (
        "Terminal round-trip carried broker route-enable route-enable "
        f"route contract identity: `{expected_sha256}`"
        in runbook
    )
    assert (
        "Current acknowledgement broker route-enable route-enable route "
        f"contract identity: `{expected_sha256}`"
        in runbook
    )
    assert (
        "Terminal round-trip broker route-enable route-enable route "
        "contract identity matches current: yes"
        in runbook
    )
    lineage = load_broker_readiness_lineage(
        out_dir / "broker_readiness_config.json"
...
def test_broker_readiness_blocks_remanifested_roundtrip_broker_route_enable_route_enable_forgery(
    route_enable_route_enable_route_enable_identity_readiness_inputs,
    tmp_path,
):
    _, schema_dir, export_dir, upload_dir, source_roundtrip_dir = (
    ...
    forged_sha256 = "c" * 64
    current_sha256 = (
        forge_roundtrip_route_enable_route_enable_route_enable_contract_identity(
            roundtrip_dir,
            forged_sha256,
        )
    )
