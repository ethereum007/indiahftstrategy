...
def write_route_identity_dispatch(
    tmp_path,
    *,
    ...
    from reports.broker_dispatch import write_broker_dispatch_plan
    from tests.test_broker_dispatch import (
        write_contract_identity_dispatch_inputs,
    )

    route, upload, broker_fields = (
        write_contract_identity_dispatch_inputs(
            tmp_path,
    ...
    assert report.ready
    return dispatch, broker_fields


@pytest.fixture(scope="module")
def route_enable_route_identity_dispatch(tmp_path_factory):
    root = tmp_path_factory.mktemp(
        "broker_dispatch_send_route_enable_route_identity"
    )
...
@pytest.fixture(scope="module")
def route_enable_route_enable_identity_dispatch(tmp_path_factory):
    if os.name == "nt":
        short_root = Path(Path.cwd().anchor) / ".hft_test_tmp"
        short_root.mkdir(exist_ok=True)
        root = Path(tempfile.mkdtemp(prefix="bsc_", dir=short_root))
    else:
        root = tmp_path_factory.mktemp("seri")
    try:
        dispatch, broker_fields = write_route_identity_dispatch(
            root,
            contract_identity=False,
        ...
        yield root, dispatch, broker_fields
    finally:
        if os.name == "nt":
...
@pytest.fixture(scope="module")
def route_enable_route_enable_route_enable_identity_dispatch(
    tmp_path_factory,
):
    if os.name == "nt":
        short_root = Path(Path.cwd().anchor) / ".hft_test_tmp"
        short_root.mkdir(exist_ok=True)
        root = Path(tempfile.mkdtemp(prefix="bst_", dir=short_root))
    else:
        root = tmp_path_factory.mktemp(
            "broker_dispatch_send_terminal_identity"
        )
    try:
        dispatch, broker_fields = write_route_identity_dispatch(
            root,
...
@pytest.fixture
def recursive_send_input_copy(tmp_path):
    copied_roots = []
    short_root = None
...
@pytest.fixture(scope="module")
def route_enable_route_enable_identity_send(
    route_enable_route_enable_identity_dispatch,
):
    root, dispatch, broker_fields = (
        route_enable_route_enable_identity_dispatch
    )
    send = root / "dispatch_send"
    report = write_broker_dispatch_send_packet(
...
@pytest.fixture(scope="module")
def route_enable_route_enable_route_enable_identity_send(
    route_enable_route_enable_route_enable_identity_dispatch,
):
    root, dispatch, broker_fields = (
        route_enable_route_enable_route_enable_identity_dispatch
    )
...
@pytest.fixture(scope="module")
def route_enable_route_identity_ack(
    route_enable_route_identity_send,
):
    root, dispatch, send, broker_fields = route_enable_route_identity_send
    acks_path = _write_ack_log_from_send(
        send,
        root / "broker_dispatch_acks.csv",
    )
    ack = root / "dispatch_ack"
    ...
    return root, dispatch, send, ack, broker_fields


@pytest.fixture(scope="module")
def route_enable_route_enable_identity_ack(
    route_enable_route_enable_identity_send,
):
    root, dispatch, send, broker_fields = (
        route_enable_route_enable_identity_send
    )
    acks_path = _write_ack_log_from_send(
        send,
...
@pytest.fixture(scope="module")
def route_enable_route_enable_route_enable_identity_ack(
    route_enable_route_enable_route_enable_identity_send,
):
    root, dispatch, send, broker_fields = (
        route_enable_route_enable_route_enable_identity_send
    )
...
def _forge_dispatch_route_enable_route_contract_identity(
    ...
    digest_fields = (
        ...
        (
            "route_enable_cutover_runtime_telemetry_current_broker_readiness_"
            "route_contract_identity_sha256"
        ),
        (
            "route_enable_cutover_runtime_lineage_current_broker_readiness_"
            "route_contract_identity_sha256"
        ),
        (
            "route_enable_cutover_current_runtime_"
            "route_contract_identity_sha256"
        ),
    ...
    current_sha256 = str(summary.loc[0, digest_fields[0]])
...
def _forge_dispatch_route_enable_route_enable_contract_identity(
    dispatch,
    forged_sha256,
):
    digest_fields = (
        ...
        (
            "route_enable_cutover_current_runtime_route_enable_"
            "route_contract_identity_sha256"
        ),
        (
...
def _forge_dispatch_route_enable_route_enable_route_enable_contract_identity(
    dispatch,
    forged_sha256,
):
    digest_fields = (
        ...
        (
            "route_enable_cutover_current_runtime_route_enable_route_enable_"
            "route_contract_identity_sha256"
        ),
        (
...
def _forge_send_route_enable_route_enable_contract_identity(
    send,
    forged_sha256,
):
    digest_fields = (
        ...
        (
            "broker_dispatch_route_enable_current_cutover_route_enable_"
            "route_contract_identity_sha256"
        ),
    )
    summary = pd.read_csv(
...
def _forge_send_route_enable_route_enable_route_enable_contract_identity(
    send,
    forged_sha256,
):
    digest_fields = (
...
def _forge_ack_route_enable_route_contract_identity(
    ack,
    forged_sha256,
):
    digest_fields = (
        ...
        (
            "broker_dispatch_send_broker_dispatch_current_route_enable_"
            "route_contract_identity_sha256"
        ),
        (
    ...
    current_sha256 = str(summary.loc[0, digest_fields[2]])
    assert current_sha256 and current_sha256 != forged_sha256
    for field in digest_fields:
        assert field in summary.columns
...
def _forge_ack_route_enable_route_enable_contract_identity(
    ack,
    forged_sha256,
):
    digest_fields = (
        ...
        (
            "broker_dispatch_send_broker_dispatch_route_enable_cutover_"
            "current_runtime_route_enable_route_contract_identity_sha256"
        ),
        (
            "broker_dispatch_send_broker_dispatch_route_enable_"
            "current_cutover_route_enable_route_contract_identity_sha256"
        ),
    )
    summary = pd.read_csv(
        ack / "broker_dispatch_ack_summary.csv",
...
def _forge_ack_route_enable_route_enable_route_enable_contract_identity(
    ack,
    forged_sha256,
):
    digest_fields = (
    ...
    for field in digest_fields:
        assert field in summary.columns
        summary[field] = forged_sha256
        acknowledgements_frame[field] = forged_sha256
        config["broker_dispatch_send_lineage"][field] = forged_sha256
    summary.to_csv(
        ack / "broker_dispatch_ack_summary.csv",
        index=False,
    )
    ...
    _refresh_manifest(
        ack / "manifest.json",
        extra_updates={
            field: forged_sha256 for field in digest_fields
        },
    )
    return current_sha256
...
def _write_ack_log_from_send(send, acks_path):
    broker_acks = pd.read_csv(
        send / "broker_dispatch_expected_acks.csv",
        dtype=str,
        keep_default_na=False,
    )
    broker_acks["broker_order_id"] = [
        f"ACK-{index + 1}" for index in range(len(broker_acks))
    ]
    broker_acks["ack_status"] = "dry_run_accepted"
    broker_acks["ack_ts_ns"] = [
        str(1_000_000 + index) for index in range(len(broker_acks))
    ]
...
def _write_route_identity_ack_chain(
    ...
    *,
    ...
    acks_path = _write_ack_log_from_send(
        send,
        tmp_path / "route_identity_broker_acks.csv",
    )
    ack = tmp_path / "route_identity_ack"
    ...
    return dispatch, send, ack, broker_fields
...
def test_broker_dispatch_send_revalidates_route_contract_identity(tmp_path):
    ...
    summary = report.summary.iloc[0]
    assert bool(
        summary["broker_dispatch_route_contract_identity_active"]
    )
    assert (
        summary[
            (
    ...
    assert identity_checks.isdisjoint(
        set(
            report.checks.loc[
    ...
    lineage_config = config["broker_dispatch_lineage"]
    assert lineage_config[
        "broker_dispatch_route_contract_identity_matches_current"
    ]
    assert (
        lineage_config[
    ...
    assert send_lineage["gate_passed"]
    assert send_lineage[
        "broker_dispatch_route_contract_identity_matches_current"
    ]
    runbook = (out_dir / "broker_dispatch_send_runbook.md").read_text(
        encoding="utf-8"
    )
    assert (
        f"Current route contract identity: `{current_sha256}`"
        in runbook
    )


def test_broker_dispatch_send_blocks_remanifested_dispatch_route_identity_forgery(
    tmp_path,
):
    dispatch, broker_fields = write_route_identity_dispatch(tmp_path)
    identity_field = (
    ...
    assert not report.ready
    assert "broker_dispatch_manifest_current" not in failed
    assert "broker_dispatch_lineage_contract_consistent" not in failed
    assert {
    ...
    assert action["component"] == "broker_readiness"
    assert action["next_gate"] == "review-broker-readiness"
    assert not report.requests["submission_enabled"].astype(bool).any()
    assert not report.requests["authorizes_submission"].astype(bool).any()


def test_broker_dispatch_send_verifies_dispatch_broker_route_contract_identity(
    tmp_path,
    route_enable_route_identity_dispatch,
):
    _, dispatch, broker_fields = route_enable_route_identity_dispatch
    out_dir = tmp_path / "dispatch_send"
    ...
    assert requests[carried_field].eq(identity_sha256).all()
    assert requests[current_field].eq(identity_sha256).all()
    assert requests[match_field].astype(bool).all()
    request_payload = json.loads(
        requests.loc[0, "request_payload_json"]
    )
    assert request_payload[carried_field] == identity_sha256
    assert request_payload[current_field] == identity_sha256
    assert request_payload[match_field]
    ...
    assert (
        config["broker_dispatch_lineage"][carried_field]
        == identity_sha256
    )
    assert (
        config["broker_dispatch_lineage"][current_field]
        == identity_sha256
    )
    assert config["broker_dispatch_lineage"][match_field]
    manifest = json.loads(
        (out_dir / "manifest.json").read_text(encoding="utf-8")
    )
    ...
    assert not lineage["dispatch_route_contract_identity_active"]
    assert lineage["dispatch_route_contract_identity_matches_current"]
    assert lineage[
    ...
    assert (
        f"Current route-enable broker route contract identity: "
        f"`{identity_sha256}`"
    ) in runbook
    assert (
        "Dispatch broker route contract identity matches current: yes"
        in runbook
    )


def test_broker_dispatch_send_blocks_remanifested_dispatch_broker_route_forgery(
    tmp_path,
    route_enable_route_identity_dispatch,
):
    source_root, _, broker_fields = route_enable_route_identity_dispatch
    root = tmp_path / "forged_inputs"
    shutil.copytree(source_root, root)
    dispatch = root / "dispatch"
    forged_sha256 = "c" * 64
    current_sha256 = (
        _forge_dispatch_route_enable_route_contract_identity(
            dispatch,
            forged_sha256,
        )
    )
...
def test_broker_dispatch_send_verifies_dispatch_route_enable_identity(
    tmp_path,
    route_enable_route_enable_identity_dispatch,
):
    _, dispatch, broker_fields = route_enable_route_enable_identity_dispatch
    out_dir = tmp_path / "dispatch_send"
    ...
    assert (
        ...
        == "ready_for_non_submitting_broker_sender_review"
    )
    assert len(identity_checks) == 3
    assert identity_checks["passed"].astype(bool).all()
    assert not bool(
    ...
    fields = broker_dispatch_send_lineage_fields(lineage)
    assert fields[
        (
    ...
    assert (
        "Dispatch broker route-enable route contract identity "
        "matches current: yes"
    ) in runbook


def test_broker_dispatch_send_blocks_remanifested_dispatch_dedicated_identity(
    tmp_path,
    route_enable_route_enable_identity_dispatch,
    recursive_send_input_copy,
):
    source_root, _, broker_fields = (
        route_enable_route_enable_identity_dispatch
    )
    root = recursive_send_input_copy(source_root, "bscf_")
    dispatch = root / "dispatch"
    forged_sha256 = "b" * 64
    current_sha256 = (
        _forge_dispatch_route_enable_route_enable_contract_identity(
            dispatch,
            forged_sha256,
        )
    )
    ...
    assert not report.ready
    assert (
        report.summary.iloc[0]["recommendation"]
        == "keep_broker_sender_disabled"
    )
    assert "broker_dispatch_manifest_current" not in failed
    assert "broker_dispatch_lineage_contract_consistent" not in failed
    assert f"{carried_field}_present" not in failed
...
def test_broker_dispatch_send_verifies_dispatch_route_enable_route_enable_identity(
    tmp_path,
    route_enable_route_enable_route_enable_identity_dispatch,
):
    _, dispatch, broker_fields = (
        route_enable_route_enable_route_enable_identity_dispatch
    )
    out_dir = tmp_path / "dispatch_send"
    ...
    assert requests[current_field].eq(identity_sha256).all()
    assert requests[match_field].astype(bool).all()
    assert requests.apply(
        lambda row: hashlib.sha256(
            row["request_payload_json"].encode("utf-8")
        ).hexdigest()
    ...
    assert (
        "Current route-enable broker route-enable route-enable route contract "
        f"identity: `{identity_sha256}`"
    ) in runbook
    assert (
        "Dispatch broker route-enable route-enable route contract identity "
        "matches current: yes"
    ) in runbook


def test_broker_dispatch_send_blocks_remanifested_dispatch_route_enable_route_enable_identity(
    tmp_path,
    route_enable_route_enable_route_enable_identity_dispatch,
    recursive_send_input_copy,
):
    ...
    root = recursive_send_input_copy(source_root, "bstf_")
    dispatch = root / "dispatch"
    forged_sha256 = "b" * 64
    current_sha256 = (
        _forge_dispatch_route_enable_route_enable_route_enable_contract_identity(
            dispatch,
            forged_sha256,
        )
    )
...
def test_broker_dispatch_ack_verifies_send_route_enable_identity(
    tmp_path,
    route_enable_route_enable_identity_send,
):
    _, dispatch, send, broker_fields = (
        route_enable_route_enable_identity_send
    )
    acks_path = _write_ack_log_from_send(
        send,
    ...
    assert (
        "Current dispatch broker route-enable route contract identity: "
        f"`{identity_sha256}`"
    ) in runbook
    assert (
        "Send broker route-enable route contract identity "
        "matches current: yes"
    ) in runbook


def test_broker_dispatch_ack_blocks_remanifested_send_dedicated_identity(
    tmp_path,
    route_enable_route_enable_identity_send,
    recursive_send_input_copy,
):
    ...
    root = recursive_send_input_copy(source_root, "bacf_")
    dispatch = root / "dispatch"
    send = root / "dispatch_send"
    forged_sha256 = "a" * 64
    current_sha256 = (
        _forge_send_route_enable_route_enable_contract_identity(
            send,
            forged_sha256,
        )
    )
...
def test_broker_dispatch_ack_verifies_send_route_enable_route_enable_identity(
    tmp_path,
    route_enable_route_enable_route_enable_identity_send,
):
    _, dispatch, send, broker_fields = (
    ...
    assert (
        "Current dispatch broker route-enable route-enable route contract "
        f"identity: `{identity_sha256}`"
    ) in runbook
    assert (
        "Send broker route-enable route-enable route contract identity "
        "matches current: yes"
    ) in runbook


def test_broker_dispatch_ack_blocks_remanifested_send_route_enable_route_enable_identity(
    tmp_path,
    route_enable_route_enable_route_enable_identity_send,
    recursive_send_input_copy,
):
    ...
    root = recursive_send_input_copy(source_root, "backf_")
    dispatch = root / "dispatch"
    send = root / "dispatch_send"
    forged_sha256 = "a" * 64
    current_sha256 = (
        _forge_send_route_enable_route_enable_route_enable_contract_identity(
            send,
            forged_sha256,
        )
    )
...
def test_broker_dispatch_ack_revalidates_send_route_contract_identity(
    tmp_path,
):
    dispatch, broker_fields = write_route_identity_dispatch(tmp_path)
    ...
    summary = report.summary.iloc[0]
    assert bool(summary[active_field])
    assert summary[carried_field] == current_sha256
    assert summary[current_field] == current_sha256
    ...
    assert report.acknowledgements[verdict_field].astype(bool).all()
    assert (
        report.config["broker_dispatch_send_lineage"][current_field]
        == current_sha256
    )
    assert report.config["broker_dispatch_send_lineage"][verdict_field]
    manifest = json.loads(
        (ack / "manifest.json").read_text(encoding="utf-8")
    )
    ...
    assert lineage["gate_passed"]
    assert lineage[verdict_field]
    runbook = (ack / "broker_dispatch_ack_runbook.md").read_text(
        encoding="utf-8"
    )
    assert (
        f"Current dispatch route contract identity: `{current_sha256}`"
        in runbook
    )
    assert "Send route contract identity matches current: yes" in runbook


def test_broker_dispatch_ack_blocks_remanifested_send_route_identity_forgery(
    tmp_path,
):
    dispatch, broker_fields = write_route_identity_dispatch(tmp_path)
    send = tmp_path / "dispatch_send"
    ...
    assert "broker_dispatch_send_manifest_current" not in failed
    assert (
        "broker_dispatch_send_lineage_contract_consistent" not in failed
    )
    assert {
        "broker_dispatch_send_broker_dispatch_matches_current",
        digest_check,
    ...
    assert summary[carried_field] == forged_sha256
    assert summary[current_field] == current_sha256
    assert not bool(summary[verdict_check])
    action = report.action_queue.loc[
        report.action_queue["check"] == verdict_check
    ].iloc[0]
    assert action["component"] == "broker_readiness"
    assert action["next_gate"] == "review-broker-readiness"
    assert (
    ...
    assert not lineage["send_lineage_gate_passed"]
    assert not lineage["send_matches_current"]
    assert not lineage[verdict_check]
    assert not lineage["gate_passed"]


def test_broker_dispatch_roundtrip_revalidates_ack_route_contract_identity(
    tmp_path,
):
    dispatch, send, ack, broker_fields = (
        _write_route_identity_ack_chain(tmp_path)
    )
    roundtrip = tmp_path / "route_identity_roundtrip"
    ...
    identity_checks = {
        ...
        (
            "broker_dispatch_ack_broker_dispatch_send_broker_dispatch_"
            "route_enable_cutover_runtime_telemetry_broker_readiness_"
            "roundtrip_contract_identity_sha256_matches_current"
        ),
        verdict_field,
    }
    assert identity_checks.isdisjoint(
    ...
    assert report.orders[verdict_field].astype(bool).all()
    assert (
        report.config["broker_dispatch_ack_lineage"][current_field]
        == current_sha256
    )
    assert report.config["broker_dispatch_ack_lineage"][verdict_field]
    manifest = json.loads(
        (roundtrip / "manifest.json").read_text(encoding="utf-8")
    )
    assert manifest["extra"][current_field] == current_sha256
    assert manifest["extra"][verdict_field]
    ...
    assert lineage[verdict_field]
    runbook = (
        roundtrip / "broker_dispatch_roundtrip_runbook.md"
    ).read_text(encoding="utf-8")
    assert (
        f"Current send route contract identity: `{current_sha256}`"
        in runbook
    )
    assert (
        "Acknowledgement route contract identity matches current: yes"
        in runbook
    )


def test_broker_dispatch_roundtrip_blocks_remanifested_ack_route_identity_forgery(
    tmp_path,
):
    dispatch, send, ack, broker_fields = (
        _write_route_identity_ack_chain(tmp_path)
    ...
    assert not report.passed
    assert "broker_dispatch_ack_manifest_current" not in failed
    assert "broker_dispatch_ack_lineage_contract_consistent" not in failed
    assert {
    ...
    assert not report.orders["authorizes_submission"].astype(bool).any()
    lineage = load_broker_dispatch_roundtrip_lineage(
        roundtrip / "broker_dispatch_roundtrip_config.json",
        expected_broker_dispatch_ack_config_path=(
            ack / "broker_dispatch_ack_config.json"
    ...
    assert lineage["manifest_current"]
    assert lineage["contract_consistent"], lineage["contract_error"]
    assert not lineage["ack_lineage_gate_passed"]
    assert not lineage["ack_matches_current"]
    assert not lineage[verdict_check]
    assert not lineage["gate_passed"]


def test_broker_dispatch_roundtrip_verifies_ack_broker_route_contract_identity(
    tmp_path,
    route_enable_route_identity_ack,
):
    _, dispatch, send, ack, broker_fields = route_enable_route_identity_ack
    roundtrip = tmp_path / "dispatch_roundtrip"
    ...
    assert summary[current_field] == identity_sha256
    assert bool(summary[match_field])
    assert report.orders[carried_field].eq(identity_sha256).all()
    assert report.orders[current_field].eq(identity_sha256).all()
    assert report.orders[match_field].astype(bool).all()
    assert (
        report.config["broker_dispatch_ack_lineage"][carried_field]
        == identity_sha256
    )
    assert (
        report.config["broker_dispatch_ack_lineage"][current_field]
    ...
    assert report.config["broker_dispatch_ack_lineage"][match_field]
    manifest = json.loads(
        (roundtrip / "manifest.json").read_text(encoding="utf-8")
    )
    assert manifest["extra"][carried_field] == identity_sha256
    ...
    assert lineage["gate_passed"]
    assert not lineage["ack_route_contract_identity_active"]
    assert lineage["ack_route_contract_identity_matches_current"]
    assert lineage[
    ...
    output_fields = broker_dispatch_roundtrip_lineage_fields(lineage)
    assert output_fields[
    ...
    assert (
        f"Current send broker route contract identity: "
        f"`{identity_sha256}`"
    ) in runbook
    assert (
        "Acknowledgement broker route contract identity matches current: yes"
        in runbook
    )


def test_broker_dispatch_roundtrip_blocks_remanifested_ack_broker_route_forgery(
    tmp_path,
    route_enable_route_identity_ack,
):
    source_root, _, _, _, broker_fields = route_enable_route_identity_ack
    root = tmp_path / "forged_inputs"
    shutil.copytree(source_root, root)
    dispatch = root / "dispatch"
    send = root / "dispatch_send"
    ack = root / "dispatch_ack"
    forged_sha256 = "f" * 64
    current_sha256 = _forge_ack_route_enable_route_contract_identity(
        ack,
        forged_sha256,
    )
    assert current_sha256 == broker_fields[
...
def test_broker_dispatch_roundtrip_verifies_ack_route_enable_identity(
    tmp_path,
    route_enable_route_enable_identity_ack,
):
    _, dispatch, send, ack, broker_fields = (
        route_enable_route_enable_identity_ack
    )
    roundtrip = tmp_path / "dispatch_roundtrip"
    ...
    assert (
        "Acknowledgement broker route-enable route contract identity "
        "matches current: yes"
    ) in runbook


def test_broker_dispatch_roundtrip_blocks_remanifested_ack_dedicated_identity(
    tmp_path,
    route_enable_route_enable_identity_ack,
    recursive_send_input_copy,
):
    source_root, _, _, _, broker_fields = (
        route_enable_route_enable_identity_ack
    )
    root = recursive_send_input_copy(source_root, "brdf_")
    dispatch = root / "dispatch"
    send = root / "dispatch_send"
    ack = root / "dispatch_ack"
    forged_sha256 = "c" * 64
    current_sha256 = (
        _forge_ack_route_enable_route_enable_contract_identity(
            ack,
            forged_sha256,
        )
    )
...
def test_broker_dispatch_roundtrip_verifies_ack_route_enable_route_enable_identity(
    tmp_path,
    route_enable_route_enable_route_enable_identity_ack,
):
    _, dispatch, send, ack, broker_fields = (
        route_enable_route_enable_route_enable_identity_ack
    )
    ...
    assert (
        "Current send broker route-enable route-enable route contract "
        f"identity: `{identity_sha256}`"
    ) in runbook
    assert (
        "Acknowledgement broker route-enable route-enable route contract "
        "identity matches current: yes"
    ) in runbook


def test_broker_dispatch_roundtrip_blocks_remanifested_ack_terminal_identity(
    tmp_path,
    route_enable_route_enable_route_enable_identity_ack,
    recursive_send_input_copy,
):
    ...
    root = recursive_send_input_copy(source_root, "brtf_")
    dispatch = root / "dispatch"
    send = root / "dispatch_send"
    ack = root / "dispatch_ack"
    forged_sha256 = "b" * 64
    current_sha256 = (
        _forge_ack_route_enable_route_enable_route_enable_contract_identity(
            ack,
            forged_sha256,
        )
    )
    ...
    roundtrip = root / "dispatch_roundtrip"
...
def test_broker_dispatch_ack_lineage_closes_final_roundtrip(tmp_path):
    ...
    summary = report.summary.iloc[0]
    manifest = json.loads(
        (tmp_path / "roundtrip" / "manifest.json").read_text(
            encoding="utf-8"
        )
    )
    assert bool(summary["strategy_portfolio_leadlag_ack_contract_consistent"])
    assert summary[
        "strategy_portfolio_leadlag_edge_lineage_contract_sha256"
    ] == "c" * 64
    ...
    assert "Lead-lag acknowledgement contract consistent: yes" in (
        tmp_path / "roundtrip" / "broker_dispatch_roundtrip_runbook.md"
    ).read_text(encoding="utf-8")
...
def test_broker_dispatch_roundtrip_blocks_verified_ack_leadlag_disagreement(
    tmp_path,
):
    dispatch, send, ack = _write_verified_ack_chain(
        tmp_path,
    ...
    dispatch_config = json.loads(
        (dispatch / "broker_dispatch_config.json").read_text(encoding="utf-8")
    )
    send_config = json.loads(
        (send / "broker_dispatch_send_config.json").read_text(encoding="utf-8")
    )
    ack_config = json.loads(
        (ack / "broker_dispatch_ack_config.json").read_text(encoding="utf-8")
    )
    ack_config["strategy_portfolio"][
        "leadlag_edge_lineage_contract_sha256"
    ] = "d" * 64

    report = evaluate_broker_dispatch_roundtrip(
        dispatch_summary=pd.read_csv(dispatch / "broker_dispatch_summary.csv"),
        dispatch_orders=pd.read_csv(dispatch / "broker_dispatch_orders.csv"),
        send_summary=pd.read_csv(send / "broker_dispatch_send_summary.csv"),
        send_requests=pd.read_csv(send / "broker_dispatch_send_requests.csv"),
        ack_summary=pd.read_csv(ack / "broker_dispatch_ack_summary.csv"),
        acknowledgements=pd.read_csv(
            ack / "broker_dispatch_acknowledgements.csv"
        ),
        dispatch_config=dispatch_config,
    ...
    assert "strategy_portfolio_leadlag_ack_contract_consistent" in failed
    assert report.action_queue is not None
    action = report.action_queue.loc[
...
def test_broker_dispatch_ack_lineage_reads_legacy_noncanonical_packet(
    ...
    report = write_broker_dispatch_roundtrip(
        ...
        send_dir=send,
        ack_dir=ack,
        output_dir=tmp_path / "legacy_roundtrip",
        thresholds=BrokerDispatchRoundTripThresholds(
            require_ack_lineage=True
        ),
    )
