...
def test_provider_market_data_research_handoff_carries_capture_bundle_provenance(tmp_path):
    ...
    assert bool(summary["adapter_receipt_proofs_match_evidence_manifest"])
    ...
    assert report.datasets["capture_fingerprint_matches_evidence"].astype(bool).all()
    assert report.datasets["adapter_receipt_fingerprint_matches_evidence"].astype(bool).all()
...
def test_provider_market_data_research_handoff_blocks_capture_and_receipt_changed_after_evidence(tmp_path):
    evidence, _ = _write_bundle_linked_real_evidence(tmp_path)
    proof = evidence.config["adapter_receipt_proof"]
    receipt_path = Path(proof["receipts"][0]["adapter_receipt_path"])
    receipt_path.write_text(
        receipt_path.read_text(encoding="utf-8") + "\n",
        encoding="utf-8",
    )
    capture_path = Path(str(evidence.captures.iloc[0]["capture_path"]))
    ...
    assert "adapter_receipt_fingerprints_match_evidence" in failed
    assert "tick_fold_capture_fingerprints_match_evidence" in failed
    assert report.summary.iloc[0]["adapter_receipt_fingerprint_match_count"] == 1
    assert report.summary.iloc[0]["capture_fingerprint_match_count"] == 1
    assert report.action_queue.loc[0, "action"] == (
        "regenerate_live_evidence_with_adapter_receipt_proof"
    )
...
def test_provider_market_data_research_handoff_blocks_missing_adapter_execution_contract(tmp_path):
    evidence, _ = _write_bundle_linked_real_evidence(tmp_path)
    evidence_config_path = evidence.output_dir / "provider_market_data_live_evidence_config.json"
    manifest_path = evidence.output_dir / "manifest.json"
    _mutate_json(
    ...
    assert report.action_queue.loc[0, "action"] == "regenerate_live_evidence_with_adapter_execution_contract"
    assert report.action_queue.loc[0, "next_gate"] == "review-provider-market-data-live-evidence"


def test_provider_market_data_research_handoff_blocks_missing_provider_profile(tmp_path):
    evidence, _ = _write_bundle_linked_real_evidence(tmp_path)
    evidence_config_path = evidence.output_dir / "provider_market_data_live_evidence_config.json"
    manifest_path = evidence.output_dir / "manifest.json"
    _mutate_json(
    ...
    summary = report.summary.iloc[0]
    assert not report.ready
    assert "live_evidence_provider_profile_carried" in failed
    assert "live_evidence_provider_profile_matches_session" in failed
    assert "adapter_execution_contract_provider_profile_matches_evidence" in failed
    assert summary["provider_profile_sha256"] == ""
    assert not bool(summary["provider_profile_matches_session"])
    assert report.action_queue.loc[0, "action"] == "regenerate_live_evidence_with_provider_profile"
    assert report.action_queue.loc[0, "next_gate"] == "review-provider-market-data-live-evidence"
...
def test_provider_market_data_research_handoff_blocks_synthetic_smoke_evidence(tmp_path):
    ...
    assert "synthetic_sidecar_proof_ready" not in failed
    assert report.action_queue.loc[0, "queue_status"] == "blocked"
    assert report.action_queue.loc[0, "next_gate"] == "review-provider-market-data-live-evidence"


def test_provider_market_data_research_handoff_blocks_missing_synthetic_sidecar_proof(tmp_path):
    evidence = _write_synthetic_smoke_evidence(tmp_path)
    _mutate_json(
    ...
    assert "synthetic_sidecar_proof_carried" in failed
    assert "synthetic_sidecar_proof_ready" in failed
    assert "live_evidence_research_ready" not in failed
    assert "synthetic_rehearsal_absent" not in failed
    assert report.action_queue.loc[0, "action"] == "regenerate_live_evidence_with_synthetic_sidecar_proof"
    assert report.action_queue.loc[0, "next_gate"] == "review-provider-market-data-live-evidence"
