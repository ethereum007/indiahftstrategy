        ...
        checks.extend(
            [
                _check(
                    ...
                    (
                        "terminal round-trip broker route-enable route "
                ...
                _check(
                    ...
                    (
                        "terminal round-trip broker route-enable route "
                        "contract identity digest differs from the current "
    ...
    if bool(
        ...
        checks.extend(
            [
                _check(
                    (
                        "dispatch_roundtrip_broker_route_enable_"
                    ...
                    (
                        "terminal round-trip broker route-enable "
                ...
                _check(
                    ...
                    (
                        "terminal round-trip broker route-enable "
                        "route-enable route contract identity digest differs "
                        "from the current acknowledgement source"
                    ),
                ),
                _check(
                    (
                    ...
                    (
                        ...
                        "matches the current acknowledgement source"
                    ),
                ),
            ]
        )
...
def _runbook_markdown(summary: pd.DataFrame, items: pd.DataFrame, action_queue: pd.DataFrame) -> str:
    ...
    lines = [
        ...
        f"- Terminal round-trip broker route-enable route contract identity active: {_yes_no(_item_bool(row, 'broker_dispatch_roundtrip_ack_route_enable_route_enable_route_contract_identity_active'))}",
        f"- Terminal round-trip carried broker route-enable route contract identity: `{_item_text(row, BROKER_DISPATCH_ROUNDTRIP_ROUTE_ENABLE_ROUTE_ENABLE_ROUTE_CONTRACT_IDENTITY_SHA256_FIELD)}`",
        f"- Current acknowledgement broker route-enable route contract identity: `{_item_text(row, 'broker_dispatch_roundtrip_current_ack_route_enable_route_enable_route_contract_identity_sha256')}`",
        f"- Terminal round-trip broker route-enable route contract identity matches current: {_yes_no(_item_bool(row, 'broker_dispatch_roundtrip_ack_route_enable_route_enable_route_contract_identity_matches_current'))}",
        f"- Terminal round-trip broker route-enable route-enable route contract identity active: {_yes_no(_item_bool(row, 'broker_dispatch_roundtrip_ack_route_enable_route_enable_route_enable_route_contract_identity_active'))}",
        f"- Terminal round-trip carried broker route-enable route-enable route contract identity: `{_item_text(row, BROKER_DISPATCH_ROUNDTRIP_ROUTE_ENABLE_ROUTE_ENABLE_ROUTE_ENABLE_ROUTE_CONTRACT_IDENTITY_SHA256_FIELD)}`",
        f"- Current acknowledgement broker route-enable route-enable route contract identity: `{_item_text(row, 'broker_dispatch_roundtrip_current_ack_route_enable_route_enable_route_enable_route_contract_identity_sha256')}`",
        f"- Terminal round-trip broker route-enable route-enable route contract identity matches current: {_yes_no(_item_bool(row, 'broker_dispatch_roundtrip_ack_route_enable_route_enable_route_enable_route_contract_identity_matches_current'))}",
