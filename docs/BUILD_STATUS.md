...
Latest ack-to-round-trip route contract-identity proof: broker-dispatch
acknowledgement lineage now reopens the manifest-bound current send packet and
independently compares every active route identity claim carried by the
acknowledgement bundle. It publishes the acknowledgement-carried canonical
digest, the digest recovered from the current recursive send source, and a
dedicated acknowledgement-to-current identity verdict. Final round-trip review
requires the carried digest to be present, equal the recovered current digest,
and retain that passing verdict before terminal broker evidence can pass.
These facts survive in every final order plus summary, config, manifest, and
runbook evidence, and identity failures route to broker-readiness review. An
acknowledgement bundle with a forged route digest is blocked even after its
rows, summary, config, and manifest are made internally consistent and
re-manifested; authorization remains disabled. Identity-inactive legacy
packets continue through neutral defaults. The full `108`-test broker-dispatch
round-trip module, `2` new recursive ack-to-round-trip identity tests, and `10`
focused lineage, legacy, source-binding, and artifact compatibility cases
pass. Source compilation succeeds. Repository collection is healthy at
`2802 tests` across `169` files.

Latest round-trip-to-broker-readiness route contract-identity proof: terminal
round-trip lineage now reopens its manifest-bound current acknowledgement
bundle and independently compares every active route identity claim. It
publishes the round-trip-carried canonical digest, the digest recovered from
the current recursive acknowledgement source, and a dedicated
round-trip-to-current identity verdict. Broker readiness retains those facts
in its component item, summary, nested config, manifest metadata, and runbook,
and requires the carried digest to be present, equal the recovered current
digest, and retain the passing verdict. A terminal round-trip bundle with a
forged route digest remains blocked after its orders, summary, config, and
manifest are made internally consistent and re-manifested; remediation routes
back to final broker dispatch round-trip review. Identity-inactive legacy
evidence continues through neutral defaults. The `2` new recursive readiness
identity tests, all `5` broker-readiness lineage tests, and `4` established
terminal identity, remanifesting, and loose-artifact compatibility tests pass.
Source compilation succeeds. Repository collection is healthy at `2804 tests`
across `169` files. The full broker-readiness module was not rerun because the
...
Latest route-enable-to-broker-dispatch route contract-identity proof: dispatch
planning now requires the route-enable-carried cutover route digest to be
present, equal the digest independently recovered from the current recursive
cutover source, and retain a passing source-match verdict. Every dry-run order
plus summary, nested route-enable-lineage config, manifest metadata, and runbook
preserves the active flag, carried digest, current digest, and verdict.
Reloadable broker-dispatch lineage publishes a separate active flag, current
route-enable route digest, and route-enable-route-to-current verdict without
conflating this dedicated proof with the older terminal broker contract
identity. A route-enable bundle with every copied route digest forged is
blocked after its packet, summary, config, and manifest are made internally
consistent and re-manifested; remediation routes to broker-readiness review
and dispatch remains non-authorizing. Route-identity-inactive legacy packets
continue through neutral defaults. The two new clean/forged dispatch tests,
two established dispatch terminal-identity regressions, and the three
send/ack/round-trip terminal-identity continuity tests pass. Source
compilation succeeds. Repository collection is healthy at `2814 tests` across
...
Latest route-enable-to-broker-dispatch dedicated route-enable route-enable
route-contract identity proof: broker dispatch now treats the newest
route-enable terminal identity as a separate fourth family and reopens the
manifest-bound route-enable packet plus its current cutover, runtime-session,
scale-up, and recursive broker-readiness sources before accepting the carried
digest. Three dedicated checks require that digest to be present, equal the
independently recovered current digest, and retain a passing route-enable
current-source verdict before dry-run dispatch can arm. Every dispatch order
plus the summary, nested route-enable config lineage, manifest metadata, and
runbook preserves the active flag, carried digest, current digest, and verdict.
Reloadable broker-dispatch lineage compares all `13` detailed route-enable
identity fields, includes the resulting verdict in its gate, and exports a
separate `16`-field broker-dispatch proof for send preparation. A copied
route-enable packet with all four visible terminal digests rewritten remains
manifest-valid and internally contract-consistent after its packet, summary,
config, and manifest are resealed, but dispatch still blocks against the
unchanged cutover source and remediation returns to broker-readiness review.
Dispatch artifacts remain non-authorizing, earlier identity families retain
their established behavior, and inactive new fields remain neutral. The new
clean/forged end-to-end pair passes in `8995.62s`; the established two-test
route-enable route-contract compatibility pair passes in `4167.53s`, eight
lightweight broker-dispatch baselines pass in about `7.4s`, and one downstream
broker-send compatibility scenario passes in about `9s`. The Windows recursive
fixture/copy harness now uses a short temporary root so signed source trees and
re-manifested copies do not exceed `MAX_PATH`. Source compilation and diff
checks succeed. Repository collection is healthy at `2850 tests` across `169`
files. The full suite was not rerun because the two real-chain pairs alone take
about 220 minutes when sharing the same drive.

Latest broker-dispatch-to-send dedicated route-enable route-enable
route-contract identity proof: broker-send preparation now reopens the
manifest-bound dispatch plus its current route-enable, cutover,
runtime-session, scale-up, and recursive broker-readiness sources before
accepting the dispatch-carried terminal digest. Three dedicated checks require
that digest to be present, equal the independently recovered current digest,
and retain a passing broker-dispatch current-source verdict before a dry-run
send packet can pass. Every request and hashed request envelope plus the
summary, nested dispatch config lineage, manifest metadata, and runbook
preserves the active flag, carried digest, current digest, and verdict.
Reloadable broker-send lineage compares all `16` detailed broker-dispatch
identity fields, includes the resulting verdict in its gate, and exports a
separate `19`-field proof for acknowledgement reconciliation. A copied
dispatch packet with all five visible terminal digests rewritten remains
manifest-valid and internally contract-consistent after its orders, summary,
config, and manifest are resealed, but send still blocks against the unchanged
route-enable source and remediation returns to broker-readiness review. Send
artifacts remain non-authorizing, earlier identity families retain their
established behavior, and inactive new fields remain neutral. The new
clean/forged end-to-end pair passes in `6451.87s`; the established two-test
route-enable identity compatibility pair passes in `1519.96s`, `14`
lightweight broker-send baselines pass in `26.5s`, and one downstream
acknowledgement baseline passes in `22.62s`. The Windows recursive fixture and
copy harness uses short temporary roots so signed source trees and
re-manifested copies remain below `MAX_PATH`. Source compilation and diff
checks succeed. Repository collection is healthy at `2852 tests` across `169`
files. The full suite was not rerun because the two real-chain pairs alone take
...
Latest runtime-session-to-cutover terminal route-enable route-enable
route-enable route-contract identity proof: cutover now treats the newest
broker-readiness identity as a fifth independent family. It emits `13`
dedicated checks requiring the scale-up carried/current pair,
runtime-telemetry carried/current pair, and runtime-session lineage active
state, recovered current digest, and current-source verdict to converge on the
digest reopened from the manifest-bound broker-readiness source.
Authorization, summary, nested config, manifest metadata, and the runbook
preserve that proof. Reloadable cutover lineage compares all seven session
identity fields with the current runtime session, includes the resulting
verdict in the cutover gate, and exports a dedicated active flag, current
digest, and current-source verdict for route enablement. A copied session
whose visible fifth-family digests and runbook are rewritten remains
manifest-valid and internally contract-consistent after resealing, but both
the cutover report and reloadable lineage still reject it against the
unchanged broker-readiness source and route remediation to broker-readiness
review. Earlier identity families retain their established behavior, and
inactive fifth-family fields remain neutral. The clean/forged production-chain
pair passes during a shared approximately four-hour run; six lightweight
cutover and operational-lineage compatibility cases also pass. Source
compilation, diff checks, and collection succeed. Repository collection is
healthy at `2866 tests` across `169` files. The full suite was not rerun
because the production-chain pair alone takes roughly four hours on the shared
drive.

Latest cutover-to-route-enable terminal route-enable route-enable route-enable
route-contract identity proof: route enable now treats the newest
broker-readiness identity as a fifth independent family. Three dedicated
checks require the cutover-carried digest to be present, equal the current
digest recovered through the manifest-bound runtime-session, scale-up, and
recursive broker-readiness sources, and retain a passing cutover
current-source verdict. The packet, summary, nested cutover lineage, manifest
metadata, and runbook preserve the active flag, carried digest, current
digest, and verdict. Reloadable route-enable lineage reopens the current
cutover, compares all `10` detailed fifth-family fields, includes the result in
its gate, and exports a separate `13`-field proof for broker-dispatch planning.
A copied cutover with all three visible fifth-family digests rewritten remains
manifest-valid and internally contract-consistent after its authorization,
summary, config, and manifest are resealed, but route enable still blocks
against the unchanged broker-readiness source and remediation returns to
broker-readiness review. Earlier identity families retain their established
behavior, and inactive fifth-family fields remain neutral. The clean/forged
production-chain pair passes in `9529.8s`; six lightweight route-enable and
cutover-lineage compatibility cases pass in `34.9s`. Source compilation, diff
checks, and collection succeed. Repository collection is healthy at
