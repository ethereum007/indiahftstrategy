...
- Controlled scale-up plan report that combines strategy evidence, shadow
  ...
  dispatch round-trip failed checks, dispatch route proof, or shadow proof-refresh
...
- Route-enable packet that consumes ready cutover and broker upload evidence,
  ...
  live-dry-run dispatch round-trip proof plus nested route proof, and emits the
...
- Broker dispatch acknowledgement reconciliation that matches dry-run dispatch
  ...
  and fails closed on missing, rejected, duplicate, dirty-proof, stale-proof, or
...
Current passing suite: 500 tests.
