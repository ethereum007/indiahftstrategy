...
seventy. It now also consumes additive view seventy-eight from nested dispatch
config or flattened
`route_confirmed_verified_reconciled_current_latest_extended_complete_final_*`
summary fields, revalidates the exact confirmed verified-reconciled chain
against the established view-seventy broker and independently recomputed
sender anchors, and emits fresh view seventy-nine under
...
Acknowledgement continues to derive established view seventy-two from view
seventy-one and intentionally ignores additive view seventy-nine.
...
For an active dedicated broker-readiness round-trip route-enable
route-contract identity, telemetry separately carries
`scaleup_broker_readiness_route_enable_route_contract_identity_active`,
the carried and current-source digests, and
`scaleup_broker_readiness_route_enable_route_contract_identity_matches_current`.
It requires the carried digest to be present, equal the current digest
recovered through the manifest-bound broker-readiness source, and retain the
passing scale-up verdict. Rewriting both visible dedicated digests in
...
For an active dedicated route-enable route-contract identity, the guard also
compares every detailed scale-up-carried identity field with the current
scale-up provenance, requires both carried/current digest pairs to agree, and
requires both current-source verdicts. The reconciled digest and verdict are
retained under
`runtime_telemetry_broker_readiness_route_enable_route_contract_identity_*`
fields in guard metrics, summary, config, checks, and manifest metadata. A
...
config or flattened
`route_confirmed_verified_reconciled_current_latest_extended_complete_final_*`
summary fields. It revalidates the exact 77-field confirmed verified-reconciled
source against the established broker and a fresh canonical sender
recomputation, and emits a 78-field view seventy-nine under
`send_confirmed_verified_reconciled_current_latest_extended_complete_final_broker_dispatch_roundtrip_vendor_market_data_batch_lineage_comparison`.
Acknowledgement continues to derive established view seventy-two from view
seventy-one and intentionally ignores additive view seventy-nine.
The established view-thirty-seven `route_extended_complete_final_*` output
...
`dispatch_confirmed_verified_reconciled_current_latest_extended_complete_final_broker_dispatch_roundtrip_vendor_market_data_batch_lineage_comparison`.
Sender preparation continues to derive established view seventy-one from view
seventy, now consumes additive view seventy-eight, and extends the confirmed
verified-reconciled epoch with fresh view seventy-nine under
`send_confirmed_verified_reconciled_current_latest_extended_complete_final_broker_dispatch_roundtrip_vendor_market_data_batch_lineage_comparison`.
Acknowledgement continues to derive established view seventy-two from view
seventy-one and intentionally ignores additive view seventy-nine; distinct-
digest regressions cover all thirty-one compatibility boundaries.
...
For an active broker route contract identity, dispatch planning separately
reopens the manifest-bound current route-enable packet and compares its
cutover-carried route digest and every dedicated route identity field with the
current recursive cutover source. The carried digest, independently recovered
current digest, and verdict surface as
`route_enable_cutover_runtime_telemetry_broker_readiness_route_contract_identity_sha256`,
`route_enable_current_cutover_route_contract_identity_sha256`, and
`route_enable_cutover_route_contract_identity_matches_current` in every
dispatch order plus summary, nested config, manifest metadata, and runbook.
The reloadable broker-dispatch lineage publishes the compact
`broker_dispatch_route_enable_route_contract_identity_active`,
`broker_dispatch_current_route_enable_route_contract_identity_sha256`, and
`broker_dispatch_route_enable_route_contract_identity_matches_current`
handoff. Dedicated checks require the carried digest to be present, equal the
current digest, and retain a passing verdict before dispatch can arm. A
route-enable bundle with every copied route digest forged remains blocked
after its packet, summary, config, and manifest are made internally consistent
and re-manifested; remediation returns to `review-broker-readiness`, and all
dispatch output remains non-authorizing. This route proof is kept distinct
from the terminal broker contract identity, while route-identity-inactive
legacy packets retain neutral defaults.

For an active broker route-enable route contract identity, dispatch planning
also follows the manifest-bound route-enable packet through its current
cutover, runtime-session, scale-up, and recursive broker-readiness sources.
The carried digest, independently recovered current digest, and verdict
surface as
`route_enable_cutover_runtime_telemetry_broker_readiness_route_enable_route_contract_identity_sha256`,
`route_enable_current_cutover_route_enable_route_contract_identity_sha256`,
and
`route_enable_cutover_route_enable_route_contract_identity_matches_current`
in every dispatch order plus summary, nested config, manifest metadata, and
runbook. Reloadable broker-dispatch lineage publishes the compact
`broker_dispatch_route_enable_route_enable_route_contract_identity_active`,
...
handoff. Dedicated checks require the carried digest to be present, equal the
current digest, and retain a passing verdict before dry-run dispatch can arm.
A route-enable bundle whose dedicated digest is consistently forged remains
disabled after its packet, summary, config, and manifest are re-manifested;
remediation returns to `review-broker-readiness`, and dispatch output remains
non-authorizing. This proof stays separate from both the older route and
...
For an active broker-readiness route contract identity, send preparation also
reopens the dispatch-manifest-bound route-enable source and compares the
dedicated route proof with the current recursive cutover source. The
dispatch-carried digest, independently recovered current digest, and verdict
surface as
`broker_dispatch_route_enable_cutover_runtime_telemetry_broker_readiness_route_contract_identity_sha256`,
`broker_dispatch_current_route_enable_route_contract_identity_sha256`, and
`broker_dispatch_route_enable_route_contract_identity_matches_current` in
every request, hashed request envelope, summary, config, runbook, and manifest.
Dedicated sender checks require the carried digest to be present, equal the
current digest, and retain a passing current-source verdict. Reloadable send
lineage preserves the proof as
`broker_dispatch_send_dispatch_route_enable_route_contract_identity_active`,
...
A dispatch bundle with every copied broker-readiness route digest forged
remains blocked after its orders, summary, config, and manifest are made
internally consistent and re-manifested. Remediation routes to broker-readiness
...
For an active route contract identity, final round-trip review now reopens the
ack-manifest-bound current send packet and independently compares every
acknowledgement-carried route identity claim. The carried digest, current send
digest, and acknowledgement-to-current verdict surface as
`broker_dispatch_ack_broker_dispatch_send_broker_dispatch_route_enable_cutover_runtime_telemetry_broker_readiness_roundtrip_contract_identity_sha256`,
`broker_dispatch_ack_current_send_route_contract_identity_sha256`, and
`broker_dispatch_ack_send_route_contract_identity_matches_current` in every
final order plus summary, config, manifest, and runbook evidence. Dedicated
round-trip checks require the carried digest to be present, equal the
independently recovered current digest, and retain a passing current-source
verdict. An acknowledgement bundle with a consistently forged route digest
remains blocked after its rows, summary, config, and manifest are rewritten
and re-manifested; remediation routes to `review-broker-readiness`, and final
artifacts remain non-authorizing. Identity-inactive legacy acknowledgement
packets retain neutral defaults.

For an active broker-readiness route contract identity, final round-trip review
also reopens the ack-manifest-bound send source and follows the dedicated proof
through the current dispatch, route-enable, and recursive cutover source. The
acknowledgement-carried digest, independently recovered current digest, and
verdict surface as
`broker_dispatch_ack_broker_dispatch_send_broker_dispatch_route_enable_cutover_runtime_telemetry_broker_readiness_route_contract_identity_sha256`,
`broker_dispatch_ack_current_send_route_enable_route_contract_identity_sha256`,
and
`broker_dispatch_ack_send_route_enable_route_contract_identity_matches_current`
in every final order plus summary, config, manifest, and runbook. Dedicated
round-trip checks require the carried digest to be present, equal the current
digest, and retain a passing current-source verdict. Reloadable round-trip
lineage preserves the proof as
`broker_dispatch_roundtrip_ack_route_enable_route_contract_identity_active`,
...
An acknowledgement bundle with all seven copied broker-readiness route digests
forged remains blocked after its rows, summary, config, and manifest are made
internally consistent and re-manifested. Remediation routes to
...
For an active broker-readiness route-enable route contract identity, final
round-trip review now reopens the ack-manifest-bound send source and
independently recovers the current dedicated digest through dispatch,
route-enable, cutover, runtime-session, scale-up, and recursive
broker-readiness lineage. The acknowledgement-carried digest, independently
recovered current digest, and verdict surface as
`broker_dispatch_ack_broker_dispatch_send_broker_dispatch_route_enable_cutover_runtime_telemetry_broker_readiness_route_enable_route_contract_identity_sha256`,
`broker_dispatch_ack_current_send_route_enable_route_enable_route_contract_identity_sha256`,
and
`broker_dispatch_ack_send_route_enable_route_enable_route_contract_identity_matches_current`
in every final order plus summary, config, manifest, and runbook. Dedicated
round-trip checks require the carried digest to be present, equal the current
digest, and retain a passing current-source verdict. Reloadable round-trip
lineage preserves the proof separately as
`broker_dispatch_roundtrip_ack_route_enable_route_enable_route_contract_identity_active`,
...
An acknowledgement bundle with every copied dedicated digest forged remains
manifest-valid and internally contract-consistent after its rows, summary,
config, and manifest are resealed, but final review stays blocked against the
unchanged send source. Remediation routes to `review-broker-readiness`, final
...
requires current source-fingerprinted replay manifests, exact proof artifacts,
current inputs, and explicit non-routing/non-submission authority. A freshly
re-sealed but altered proof, or an outer proof manifest re-sealed over stale
replay lineage, fails `baseline_proof_verified` or `latest_proof_verified`.

The gate records reported versus effective proof pass state, detailed
verification status/error, and strategy/market identity from baseline proof,
latest proof, and calibrated replay summaries. Mixed available strategy or
market identities fail closed, and `--strategy`/`--market` enforce the expected
target when those identities are present. Its manifest binds each proof
directory, proof manifest, and recursively flattened replay dependency set, so
later replay-source drift invalidates retained refresh evidence.
`proof_refresh_summary.csv` also carries `failed_check_count`,
`failed_check_names`, `first_failed_reason`, `primary_blocker_*`,
`action_queue_count`, `blocked_action_count`, `next_gate`,
`next_gate_help_command`, and `primary_action_status`. The
`proof_refresh_action_queue.csv`, `proof_refresh_config.json`, and
`proof_refresh_runbook.md` sidecars mirror invalid or missing proof, failed
proof, calibrated replay, and identity blockers back to
`review-proof-refresh`. Use `--fail-on-blocked-actions` to fail only when
blocked proof-refresh actions exist, or `--fail-on-actions` when any
proof-refresh action should stop automation.
