...
exchange/session metadata inherited from preflight, then fingerprints that
source env-template in the bundle manifest. The command queue is adapter-neutral
by default (`provider-adapter capture ... --exchange NFO ...`) and can be
...
exchange, endpoint, source/market session metadata, output schema columns,
per-window capture commands, credential env-var names, source and capture blank
...
`live_fetch_contract`, and credential-safe `adapter_execution_contract` carried
by the nested research handoff. If the adapter contract is missing, stores
...
`live_fetch_contract` plus credential-safe `adapter_execution_contract` in its
provider wrapper summary/config/runbook and manifest. Missing, unsafe, or
mismatched adapter-contract proof blocks runtime-session monitoring and sends
the packet back through provider runtime telemetry. It converts guard halts into a ready
...
`adapter_execution_contract`, and source-provenance consistency flags so broker
dispatch reviewers can trace the exact live data source before generating
non-submitting dry-run orders. If the route-enable-retained round-trip adapter
contract is missing, unsafe, stale, or mismatched against the runtime-session
contract, broker-dispatch blocks send preparation and routes back to
...
explicit summary `False` values authoritative. The
