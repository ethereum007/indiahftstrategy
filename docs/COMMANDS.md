...
capture bundle, blank credential env-template, and adapter handoff paths in its
provider wrapper summary/config/runbook and manifest. Clean sessions route to
...
trace the live data source from the cutover artifact set. If broker-readiness
...
env-template, and adapter handoff paths into summary/config/runbook artifacts
plus manifest inputs/extra metadata, and writes provider checks/action
artifacts.
