...
env-template proof and upstream `live_fetch_contract`, without storing
...
`live_fetch_contract` before strategy research starts. Synthetic smoke evidence,
...
and `live_fetch_contract` carried by the nested research handoff. If the live
...
env-template proof, exchange/session metadata, capture-bundle session match
proof, and `live_fetch_contract` before scale-up planning begins.
...
`live_fetch_contract` in its provider wrapper summary/config/runbook and
manifest. It converts guard halts into a ready
...
`dispatch_roundtrip_*` provenance, adds manifest inputs/metadata for every
proof root including the round-trip source credential env-template, and fails
closed back to
