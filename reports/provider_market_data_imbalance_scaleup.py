...
def write_provider_market_data_imbalance_scaleup_plan(
    ...
    *,
    ...
    scorecard_manifest, scorecard_manifest_error = _read_json(
        scorecard_dir / "manifest.json"
    )
...
def _prechecks(
    ...
    config_receipt_proof = _mapping(scorecard_config.get("adapter_receipt_proof"))
    manifest_receipt_proof = _mapping(
        _mapping(scorecard_manifest.get("extra")).get("adapter_receipt_proof")
    )
    ...
    return pd.DataFrame(
        [
            ...
            _check(
                "provider_scorecard_manifest_readable",
                scorecard_manifest_error or "ok",
                "is",
                "ok",
                not scorecard_manifest_error,
                scorecard_manifest_error or "provider imbalance scorecard manifest could not be read",
            ),
            _check(
                "provider_scorecard_manifest_type",
                _text(scorecard_manifest.get("run_type")),
                "is",
                "provider_market_data_imbalance_scorecard",
                _text(scorecard_manifest.get("run_type"))
                == "provider_market_data_imbalance_scorecard",
                "provider imbalance scorecard manifest run_type is not expected",
            ),
            _check(
            ...
            _check(
                "scorecard_adapter_receipt_proof_carried",
                ...
                "provider imbalance scorecard is missing ready adapter receipt proof",
            ),
            _check(
                "scorecard_adapter_receipt_proof_matches_manifest",
                receipt_proofs_match,
                "is",
                True,
                receipt_proofs_match if bundle_provided else True,
                "adapter receipt proof differs between scorecard config and manifest",
            ),
            _check(
                "scorecard_adapter_receipts_valid",
                ...
                "provider imbalance scorecard did not preserve valid required adapter receipts",
            ),
            _check(
                "scorecard_adapter_receipt_fingerprints_current",
                ...
                "adapter receipt files changed after provider scorecard review",
            ),
            _check(
                "scorecard_capture_fingerprints_current",
                ...
                "provider capture files changed after provider scorecard review",
            ),
            _check(
...
def _checks(
    ...
    rows.append(
        _check(
            "scorecard_adapter_execution_contract_carried",
            _adapter_contract_metadata_text(scorecard_summary),
            "is_not",
            "",
            adapter_contract_carried if bundle_provided else True,
            "provider imbalance scorecard is missing credential-safe adapter execution contract proof",
        )
    )
    rows.append(
        _check(
            "scorecard_adapter_execution_contract_matches_evidence",
            _adapter_contract_metadata_text(scorecard_summary),
            "matches",
            "live evidence",
            _first_bool(scorecard_summary, "adapter_contract_metadata_matches_evidence") if bundle_provided else True,
            "provider imbalance scorecard adapter execution contract no longer matches live evidence",
        )
    )
    rows.append(
        _check(
            "scorecard_provider_profile_carried",
            _first_text(scorecard_summary, "provider_profile_sha256"),
            "has",
            "provider profile",
            provider_profile_carried,
            "provider imbalance scorecard is missing provider-profile proof",
        )
    )
    rows.append(
        _check(
            "scorecard_provider_profile_matches_session",
            _first_text(scorecard_summary, "provider_profile_sha256"),
            "matches",
            "live session",
            _first_bool(scorecard_summary, "provider_profile_matches_session"),
            "provider imbalance scorecard provider-profile proof no longer matches the live session packet",
        )
    )
    rows.append(
        _check(
            "scorecard_provider_profile_matches_bundle",
            _first_text(scorecard_summary, "capture_bundle_provider_profile_sha256"),
            "matches",
            _first_text(scorecard_summary, "provider_profile_sha256"),
            _first_bool(scorecard_summary, "provider_profile_matches_bundle") if bundle_provided else True,
            "provider imbalance scorecard provider-profile proof no longer matches the capture bundle",
        )
    )
    rows.append(
        _check(
            "scorecard_adapter_provider_profile_matches_evidence",
            _first_text(scorecard_summary, "adapter_contract_provider_profile_sha256"),
            "==",
            _first_text(scorecard_summary, "provider_profile_sha256"),
            _first_bool(scorecard_summary, "adapter_contract_provider_profile_matches_evidence")
            if bundle_provided
            else True,
        ...
        [
            {
                ...
                "provider_scorecard_manifest_run_type": _text(
                    scorecard_manifest.get("run_type")
                ),
                ...
                "adapter_contract_provider": _first_text(scorecard_summary, "adapter_contract_provider"),
                "adapter_contract_transport": _first_text(scorecard_summary, "adapter_contract_transport"),
                "adapter_contract_market": _first_text(scorecard_summary, "adapter_contract_market"),
                "adapter_contract_exchange": _first_text(scorecard_summary, "adapter_contract_exchange"),
                "adapter_contract_values_stored": _first_bool(scorecard_summary, "adapter_contract_values_stored"),
                "adapter_contract_metadata_matches_evidence": _first_bool(
                    scorecard_summary, "adapter_contract_metadata_matches_evidence"
                ),
                "provider_profile_sha256": _first_text(scorecard_summary, "provider_profile_sha256"),
                "provider_profile_adapter": _first_text(scorecard_summary, "provider_profile_adapter"),
                "provider_profile_auth_required": _first_bool(scorecard_summary, "provider_profile_auth_required"),
                "provider_profile_transports": _first_text(scorecard_summary, "provider_profile_transports"),
                "provider_profile_capabilities": _first_text(scorecard_summary, "provider_profile_capabilities"),
                "capture_bundle_provider_profile_sha256": _first_text(
                    scorecard_summary, "capture_bundle_provider_profile_sha256"
                ),
                "provider_profile_matches_session": _first_bool(scorecard_summary, "provider_profile_matches_session"),
                "provider_profile_matches_bundle": _first_bool(scorecard_summary, "provider_profile_matches_bundle")
                if _first_bool(scorecard_summary, "capture_bundle_provided")
                else True,
                "adapter_contract_provider_profile_sha256": _first_text(
                    scorecard_summary, "adapter_contract_provider_profile_sha256"
                ),
                "adapter_contract_provider_profile_matches_evidence": _first_bool(
                    scorecard_summary, "adapter_contract_provider_profile_matches_evidence"
                ),
                "provider_capture_command_count": int(
...
def _config(
    ...
    return {
        ...
        "provider_profile": _mapping(scorecard_config.get("provider_profile")),
        "live_session_provider_profile": _mapping(scorecard_config.get("live_session_provider_profile")),
...
def _next_gate_for_check(check: str) -> str:
    if (
        ...
        or check.startswith("scorecard_adapter_receipt")
        or check.startswith("scorecard_capture_fingerprint")
...
def _adapter_contract_carried(scorecard_summary: pd.DataFrame) -> bool:
    return (
        bool(_first_text(scorecard_summary, "adapter_contract_provider"))
        and bool(_first_text(scorecard_summary, "adapter_contract_transport"))
        and bool(_first_text(scorecard_summary, "adapter_contract_market"))
        and bool(_first_text(scorecard_summary, "adapter_contract_exchange"))
        and not _first_bool(scorecard_summary, "adapter_contract_values_stored")
    )


def _provider_profile_carried(scorecard_summary: pd.DataFrame) -> bool:
    return (
        bool(_first_text(scorecard_summary, "provider_profile_sha256"))
        and bool(_first_text(scorecard_summary, "provider_profile_adapter"))
        and bool(_first_text(scorecard_summary, "provider_profile_transports"))
    )


def _adapter_contract_metadata_text(scorecard_summary: pd.DataFrame) -> str:
    return (
        f"{_first_text(scorecard_summary, 'adapter_contract_provider')}|"
        f"{_first_text(scorecard_summary, 'adapter_contract_transport')}|"
        f"{_first_text(scorecard_summary, 'adapter_contract_market')}|"
        f"{_first_text(scorecard_summary, 'adapter_contract_exchange')}"
    )
...
def _provider_capture_bundle(summary: pd.Series, scorecard_config: dict[str, Any]) -> dict[str, Any]:
    ...
    if payload:
        ...
        carried.setdefault("adapter_execution_contract", _mapping(scorecard_config.get("adapter_execution_contract")))
        carried.setdefault(
            "adapter_receipt_proof",
            _mapping(scorecard_config.get("adapter_receipt_proof")),
        )
        carried.setdefault("provider_profile", _mapping(scorecard_config.get("provider_profile")))
        carried.setdefault("live_session_provider_profile", _mapping(scorecard_config.get("live_session_provider_profile")))
    ...
    return {
        ...
        "capture_bundle_provider_profile": _mapping(
            _mapping(scorecard_config.get("capture_bundle")).get("capture_bundle_provider_profile")
        ),
