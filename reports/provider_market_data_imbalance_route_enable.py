...
def _prechecks(
    ...
    return pd.DataFrame(
        [
            ...
            _check(
                "provider_cutover_manifest_readable",
                provider_manifest_error or "ok",
                "is",
                "ok",
                not provider_manifest_error,
                provider_manifest_error or "provider imbalance cutover manifest could not be read",
            ),
            _check(
                "provider_cutover_manifest_type",
                _clean(provider_manifest.get("run_type")),
                "is",
                "provider_market_data_imbalance_cutover",
                _clean(provider_manifest.get("run_type"))
                == "provider_market_data_imbalance_cutover",
                "provider imbalance cutover manifest run_type is not expected",
            ),
            _check(
            ...
            _check(
                "provider_cutover_adapter_receipt_proof_carried",
                ...
                "provider imbalance cutover is missing ready adapter receipt proof",
            ),
            _check(
                "provider_cutover_adapter_receipt_proof_matches_manifest",
                receipt_proofs_match,
                "is",
                True,
                receipt_proofs_match if bundle_provided else True,
                "adapter receipt proof differs between cutover config and manifest",
            ),
            _check(
                "provider_cutover_adapter_receipts_valid",
                ...
                "provider imbalance cutover did not preserve valid required adapter receipts",
            ),
            _check(
                "provider_cutover_adapter_receipt_fingerprints_current",
                ...
                "adapter receipt files changed after provider cutover review",
            ),
            _check(
                "provider_cutover_capture_fingerprints_current",
                ...
                "provider capture files changed after provider cutover review",
            ),
            _check(
                ...
                "provider cutover is missing final round-trip adapter receipt proof",
            ),
            _check(
                ...
                "final round-trip adapter receipt proof differs between cutover config and manifest",
            ),
            _check(
                ...
                "final round-trip adapter receipt proof differs from cutover runtime proof",
            ),
            _check(
                ...
                "provider cutover did not preserve valid final round-trip adapter receipts",
            ),
            _check(
                ...
                "final round-trip adapter receipt files changed after provider cutover",
            ),
            _check(
                ...
                "final round-trip provider capture files changed after provider cutover",
            ),
            _check(
...
def _checks(
    ...
    rows.append(
        _check(
            "provider_cutover_provider_profile_carried",
            _first_text(provider_summary, "provider_profile_sha256"),
            "has",
            "provider profile",
            provider_profile_carried,
            "provider imbalance cutover is missing provider-profile proof",
        )
    )
    rows.append(
        _check(
            "provider_cutover_provider_profile_matches_session",
            _first_text(provider_summary, "provider_profile_sha256"),
            "matches",
            "live session",
            _first_bool(provider_summary, "provider_profile_matches_session"),
            "provider imbalance cutover provider-profile proof no longer matches the live session packet",
        )
    )
    rows.append(
        _check(
            "provider_cutover_provider_profile_matches_bundle",
            ...
            "provider imbalance cutover provider-profile proof no longer matches the capture bundle",
        )
    )
    rows.append(
        _check(
            "provider_cutover_adapter_provider_profile_matches_evidence",
            ...
            "provider imbalance cutover adapter contract provider-profile SHA no longer matches live evidence",
        )
    )
    rows.append(
        _check(
            "provider_cutover_synthetic_sidecar_proof_carried",
            ...
            "provider imbalance cutover is missing synthetic rehearsal sidecar proof",
        )
    )
    rows.append(
        _check(
            "provider_cutover_synthetic_sidecar_proof_ready",
    ...
    rows.append(
        _check(
            "provider_cutover_route_readiness_provider_lineage_selection_contract",
            ...
            "provider cutover does not carry a complete active-lineage selection contract",
        )
    )
    rows.append(
        _check(
            "provider_cutover_route_readiness_provider_lineage_selection_contract_matches_artifacts",
            ...
            "provider cutover lineage selection contract differs across summary, config, and manifest",
        )
    )
    ...
    rows.append(
        _check(
            "provider_cutover_dispatch_roundtrip_provider_profile_carried",
            dispatch_provider_profile_metadata,
            "is_not",
            "",
            dispatch_provider_profile_carried if dispatch_bundle_provided else True,
            "provider imbalance route-enable is missing cutover round-trip provider-profile proof",
        )
    )
    rows.append(
        _check(
            "provider_cutover_dispatch_roundtrip_provider_profile_matches_session",
            ...
            "provider imbalance route-enable round-trip provider-profile proof no longer matches live session",
        )
    )
    rows.append(
        _check(
            "provider_cutover_dispatch_roundtrip_provider_profile_matches_bundle",
            ...
            "provider imbalance route-enable round-trip provider-profile proof no longer matches capture bundle",
        )
    )
    rows.append(
        _check(
            "provider_cutover_dispatch_roundtrip_adapter_provider_profile_matches_evidence",
            ...
            "provider imbalance route-enable round-trip adapter contract provider-profile SHA no longer matches evidence",
        )
    )
    rows.append(
        _check(
            ...
            "provider imbalance route-enable round-trip provider profile no longer matches runtime-session proof",
        )
    )
    rows.append(
        _check(
            "provider_cutover_dispatch_roundtrip_synthetic_sidecar_proof_carried",
            ...
            "provider imbalance route-enable is missing cutover round-trip synthetic rehearsal sidecar proof",
        )
    )
    rows.append(
        _check(
            "provider_cutover_dispatch_roundtrip_synthetic_sidecar_proof_ready",
            ...
            "provider imbalance route-enable cutover round-trip synthetic rehearsal sidecar proof is not ready",
        )
    )
    rows.append(
        _check(
            "provider_cutover_dispatch_roundtrip_route_readiness_provider_sidecar_breach_pairs",
            ...
            "provider imbalance route-enable carries cutover final round-trip route-readiness sidecar breaches",
        )
    )
        ...
        [
            {
                ...
                "provider_cutover_manifest_run_type": _clean(
                ...
                "synthetic_sidecar_proof_ready": _first_bool_from_frames(
                    "synthetic_sidecar_proof_ready",
                    provider_summary,
                    provider_broker_summary,
                ),
                ...
                "dispatch_roundtrip_route_readiness_provided": _first_bool_from_frames(
                    "dispatch_roundtrip_route_readiness_provided",
                    provider_summary,
                    provider_broker_summary,
                ),
                "dispatch_roundtrip_route_readiness_ops_launch_controls_present": _first_bool_from_frames(
                ...
                "dispatch_roundtrip_synthetic_sidecar_proof_ready": _first_bool_from_frames(
                    "dispatch_roundtrip_synthetic_sidecar_proof_ready",
                    provider_summary,
                    provider_broker_summary,
                ),
                ...
                "dispatch_roundtrip_provider_profile_sha256": _first_text_from_frames(
                    "dispatch_roundtrip_provider_profile_sha256",
                    provider_summary,
                    provider_broker_summary,
                ),
                "dispatch_roundtrip_provider_profile_adapter": _first_text_from_frames(
                    "dispatch_roundtrip_provider_profile_adapter",
                    provider_summary,
                    provider_broker_summary,
                ),
                "dispatch_roundtrip_provider_profile_auth_required": _first_bool_from_frames(
                    "dispatch_roundtrip_provider_profile_auth_required",
                    provider_summary,
                    provider_broker_summary,
                ),
                "dispatch_roundtrip_provider_profile_transports": _first_text_from_frames(
                    "dispatch_roundtrip_provider_profile_transports",
                    provider_summary,
                    provider_broker_summary,
                ),
                "dispatch_roundtrip_provider_profile_capabilities": _first_text_from_frames(
                    "dispatch_roundtrip_provider_profile_capabilities",
                    provider_summary,
                    provider_broker_summary,
                ),
                "dispatch_roundtrip_capture_bundle_provider_profile_sha256": _first_text_from_frames(
                    "dispatch_roundtrip_capture_bundle_provider_profile_sha256",
                    provider_summary,
                    provider_broker_summary,
                ),
                "dispatch_roundtrip_provider_profile_matches_session": _first_bool_from_frames(
                ...
                "dispatch_roundtrip_provider_profile_matches_bundle": _first_bool_from_frames(
                ...
                "dispatch_roundtrip_adapter_contract_provider_profile_sha256": _first_text_from_frames(
                    "dispatch_roundtrip_adapter_contract_provider_profile_sha256",
                    provider_summary,
                    provider_broker_summary,
                ),
                "dispatch_roundtrip_adapter_contract_provider_profile_matches_evidence": _first_bool_from_frames(
...
def _dispatch_roundtrip_provider_profile(*configs: dict[str, Any]) -> dict[str, Any]:
    for config in configs:
        dispatch_roundtrip = _dispatch_roundtrip_provenance(config)
        profile = _mapping(dispatch_roundtrip.get("provider_profile")) or _mapping(
            config.get("provider_profile")
        )
        if profile:
            return profile
    return {}
...
def _dispatch_roundtrip_live_session_provider_profile(*configs: dict[str, Any]) -> dict[str, Any]:
    for config in configs:
        dispatch_roundtrip = _dispatch_roundtrip_provenance(config)
        profile = _mapping(dispatch_roundtrip.get("live_session_provider_profile")) or _mapping(
            config.get("live_session_provider_profile")
        )
        if profile:
            return profile
    return {}
...
def _dispatch_roundtrip_capture_bundle_provider_profile(*configs: dict[str, Any]) -> dict[str, Any]:
    for config in configs:
        dispatch_roundtrip = _dispatch_roundtrip_provenance(config)
...
def _dispatch_roundtrip_synthetic_sidecar_proof(*configs: dict[str, Any]) -> dict[str, Any]:
    for config in configs:
        dispatch_roundtrip = _dispatch_roundtrip_provenance(config)
        proof = _mapping(dispatch_roundtrip.get("synthetic_sidecar_proof"))
        if proof:
            return proof
    return {}
...
def _action_priority(check: str) -> int:
    if check.startswith("provider_cutover_dispatch_roundtrip_route_readiness_provider_sidecar"):
        return 0
...
def _config(
    ...
    return {
        ...
        "dispatch_roundtrip_provenance": {
            ...
            "synthetic_sidecar_proof": _dispatch_roundtrip_synthetic_sidecar_proof(
                provider_config,
                provider_broker_config,
            ),
            ...
            "provider_profile": _dispatch_roundtrip_provider_profile(
                provider_config,
                provider_broker_config,
            ),
            "live_session_provider_profile": _dispatch_roundtrip_live_session_provider_profile(
                provider_config,
                provider_broker_config,
            ),
            "capture_bundle_provider_profile": _dispatch_roundtrip_capture_bundle_provider_profile(
                provider_config,
                provider_broker_config,
            ),
...
def _runbook_markdown(summary: pd.Series, checks: pd.DataFrame, action_queue: pd.DataFrame) -> str:
    ...
    lines = [
        ...
        f"- Adapter receipt proof: {'ready' if bool(summary['adapter_receipt_proof_ready']) else 'blocked'} ({summary['adapter_receipt_fingerprint_match_count']}/{summary['adapter_receipt_required_count']} sealed; cutover manifest match: {'yes' if bool(summary['adapter_receipt_proof_matches_manifest']) else 'no'})",
        dispatch_roundtrip_receipt_line,
...
def _next_gate_for_check(check: str, route_enable: RouteEnableReport | None) -> str:
    if check.startswith("provider_cutover_route_readiness_provider_lineage"):
...
def _recommendation_for_check(check: str) -> str:
    if check.startswith("provider_cutover_route_readiness_provider_lineage"):
        return "review_provider_lineage_selection_contract_before_route_enable"
    if check.startswith("provider_cutover_dispatch_roundtrip_route_readiness_provider_sidecar"):
        return "review_provider_roundtrip_route_readiness_sidecar_proof_before_route_enable"
    if check.startswith("provider_cutover_route_readiness_provider_sidecar"):
