...
def write_provider_market_data_imbalance_runtime_guard(
    ...
    *,
    ...
    provider_config, provider_config_error = _read_json(
        telemetry_root / "provider_market_data_imbalance_runtime_telemetry_config.json"
    )
    provider_manifest, provider_manifest_error = _read_json(
        telemetry_root / "manifest.json"
    )
    ...
    return pd.DataFrame(
        [
            ...
            _check(
                ...
                provider_config_error or "provider imbalance runtime telemetry config could not be read",
            ),
            _check(
                "provider_runtime_telemetry_manifest_readable",
                provider_manifest_error or "ok",
                "is",
                "ok",
                not provider_manifest_error,
                provider_manifest_error or "provider imbalance runtime telemetry manifest could not be read",
            ),
            _check(
                "provider_runtime_telemetry_manifest_type",
                _clean(provider_manifest.get("run_type")),
                "is",
                "provider_market_data_imbalance_runtime_telemetry_snapshot",
                _clean(provider_manifest.get("run_type"))
                == "provider_market_data_imbalance_runtime_telemetry_snapshot",
                "provider imbalance runtime telemetry manifest run_type is not expected",
            ),
            _check(
            ...
            _check(
                "provider_runtime_telemetry_adapter_receipt_proof_carried",
                ...
                "provider imbalance runtime telemetry is missing ready adapter receipt proof",
            ),
            _check(
                "provider_runtime_telemetry_adapter_receipt_proof_matches_manifest",
                receipt_proofs_match,
                "is",
                True,
                receipt_proofs_match if bundle_provided else True,
                "adapter receipt proof differs between runtime telemetry config and manifest",
            ),
            _check(
                "provider_runtime_telemetry_adapter_receipts_valid",
                ...
                "provider imbalance runtime telemetry did not preserve valid required adapter receipts",
            ),
            _check(
                "provider_runtime_telemetry_adapter_receipt_fingerprints_current",
                ...
                "adapter receipt files changed after provider runtime telemetry generation",
            ),
            _check(
                "provider_runtime_telemetry_capture_fingerprints_current",
                ...
                "provider capture files changed after provider runtime telemetry generation",
            ),
            _check(
...
def _checks(
    ...
    rows.append(
        _check(
            "provider_runtime_telemetry_provider_capture_commands_carried",
            ...
            "provider imbalance runtime telemetry is missing capture-bundle provider command proof",
        )
    )
    rows.append(
        _check(
            "provider_runtime_telemetry_provider_capture_commands_match_session",
            bundle_provider_capture_command_count,
            "matches",
            provider_capture_command_count,
    ...
    rows.append(
        _check(
            "provider_runtime_telemetry_provider_profile_carried",
            _first_text(provider_summary, "provider_profile_sha256"),
            "has",
            "provider profile",
            provider_profile_carried,
            "provider imbalance runtime telemetry is missing provider-profile proof",
        )
    )
    rows.append(
        _check(
            "provider_runtime_telemetry_provider_profile_matches_session",
            _first_text(provider_summary, "provider_profile_sha256"),
            "matches",
            "live session",
            _first_bool(provider_summary, "provider_profile_matches_session"),
            "provider imbalance runtime telemetry provider-profile proof no longer matches the live session packet",
        )
    )
    rows.append(
        _check(
            "provider_runtime_telemetry_provider_profile_matches_bundle",
            ...
            "provider imbalance runtime telemetry provider-profile proof no longer matches the capture bundle",
        )
    )
    rows.append(
        _check(
            "provider_runtime_telemetry_adapter_provider_profile_matches_evidence",
            ...
            "provider imbalance runtime telemetry adapter contract provider-profile SHA no longer matches live evidence",
        )
    )
    rows.append(
        _check(
            "provider_runtime_telemetry_synthetic_sidecar_proof_carried",
            ...
            "provider imbalance runtime telemetry synthetic folds are missing rehearsal sidecar proof",
        )
    )
    rows.append(
        _check(
            "provider_runtime_telemetry_synthetic_sidecar_proof_ready",
    ...
    rows.append(
        _check(
            "provider_runtime_telemetry_route_readiness_provider_lineage_selection_contract",
            ...
            "provider runtime telemetry does not carry a complete active-lineage selection contract",
        )
    )
    rows.append(
        _check(
            "provider_runtime_telemetry_route_readiness_provider_lineage_selection_contract_matches_artifacts",
            ...
            "provider runtime telemetry lineage selection contract differs across summary, config, and manifest",
        )
    )
        ...
        [
            {
                ...
                "provider_runtime_telemetry_manifest_run_type": _clean(
...
def _runbook_markdown(summary: pd.Series, checks: pd.DataFrame, action_queue: pd.DataFrame) -> str:
    lines = [
        ...
        f"- Adapter receipt proof: {'ready' if bool(summary['adapter_receipt_proof_ready']) else 'blocked'} ({summary['adapter_receipt_fingerprint_match_count']}/{summary['adapter_receipt_required_count']} sealed; telemetry manifest match: {'yes' if bool(summary['adapter_receipt_proof_matches_manifest']) else 'no'})",
...
def _next_gate_for_check(check: str, guard: RuntimeGuardReport | None) -> str:
    ...
    if check == "provider_runtime_telemetry_ready":
        return "build-provider-market-data-imbalance-runtime-telemetry"
    if check.startswith("provider_runtime_telemetry_manifest"):
        return "build-provider-market-data-imbalance-runtime-telemetry"
    if check.startswith("provider_runtime_telemetry_adapter_receipt"):
        return "build-provider-market-data-imbalance-runtime-telemetry"
    if check.startswith("provider_runtime_telemetry_capture_fingerprint"):
        return "build-provider-market-data-imbalance-runtime-telemetry"
    ...
    if check.startswith("provider_runtime_telemetry_adapter_execution_contract"):
        return "build-provider-market-data-imbalance-runtime-telemetry"
    if check.startswith("provider_runtime_telemetry_provider_profile"):
        return "build-provider-market-data-imbalance-runtime-telemetry"
    if check.startswith("provider_runtime_telemetry_adapter_provider_profile"):
        return "build-provider-market-data-imbalance-runtime-telemetry"
    if check.startswith("provider_runtime_telemetry_synthetic_sidecar"):
        return "build-provider-market-data-imbalance-runtime-telemetry"
    if check.startswith("provider_runtime_telemetry_route_readiness_provider_"):
...
def _recommendation_for_check(check: str, guard: RuntimeGuardReport | None) -> str:
    ...
    if check.startswith("provider_runtime_telemetry_route_readiness_provider_lineage"):
        return "review_provider_lineage_selection_contract_before_guard"
...
def _provider_capture_bundle(summary: pd.Series, provider_config: dict[str, Any]) -> dict[str, Any]:
    ...
    if payload:
        ...
        carried.setdefault("provider_profile", _mapping(provider_config.get("provider_profile")))
        carried.setdefault(
            "live_session_provider_profile",
            _mapping(provider_config.get("live_session_provider_profile")),
        )
