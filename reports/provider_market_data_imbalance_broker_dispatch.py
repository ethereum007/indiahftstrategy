...
def _prechecks(
    ...
    return pd.DataFrame(
        [
            ...
            _check(
                "provider_route_enable_manifest_readable",
                provider_manifest_error or "ok",
                "is",
                "ok",
                not provider_manifest_error,
                provider_manifest_error or "provider imbalance route-enable manifest could not be read",
            ),
            _check(
                "provider_route_enable_manifest_type",
                _clean(provider_manifest.get("run_type")),
                "is",
                "provider_market_data_imbalance_route_enable",
                _clean(provider_manifest.get("run_type"))
                == "provider_market_data_imbalance_route_enable",
                "provider imbalance route-enable manifest run_type is not expected",
            ),
            _check(
            ...
            _check(
                "provider_route_enable_adapter_receipt_proof_carried",
                ...
                "provider imbalance route-enable is missing ready adapter receipt proof",
            ),
            _check(
                ...
                "adapter receipt proof differs between route-enable config and manifest",
            ),
            _check(
                "provider_route_enable_adapter_receipts_valid",
                ...
                "provider imbalance route-enable did not preserve valid required adapter receipts",
            ),
            _check(
                ...
                "adapter receipt files changed after provider route-enable review",
            ),
            _check(
                "provider_route_enable_capture_fingerprints_current",
...
def _checks(
    ...
    rows.append(
        _check(
            "provider_route_enable_route_readiness_provider_sidecar_breach_pairs",
            route_sidecar_breach_pairs,
            "<=",
            0,
            route_sidecar_breach_pairs <= 0 if route_sidecar_gate_active else True,
            (
                "provider imbalance route-enable carries breached route-readiness "
    ...
    rows.append(
        _check(
            "provider_route_enable_route_readiness_provider_lineage_selection_contract",
            ...
            "provider route-enable does not carry a complete active-lineage selection contract",
        )
    )
    rows.append(
        _check(
            "provider_route_enable_route_readiness_provider_lineage_selection_contract_matches_artifacts",
            ...
            "provider route-enable lineage selection contract differs across summary, config, and manifest",
        )
    )
    ...
    rows.append(
        _check(
            ...
            (
                "provider imbalance broker dispatch carries breached route-enable "
                "round-trip route-readiness sidecar proof"
            ),
        )
    )
        ...
        [
            {
                ...
                "provider_route_enable_manifest_run_type": _clean(
...
def _next_gate_for_check(check: str, broker_dispatch: BrokerDispatchReport | None) -> str:
    if check.startswith("provider_route_enable_route_readiness_provider_lineage"):
        return "review-provider-market-data-imbalance-route-readiness"
    if check.startswith("provider_route_enable_dispatch_roundtrip_route_readiness_provider_sidecar"):
        return "review-provider-market-data-imbalance-route-readiness"
    if check.startswith("provider_route_enable_route_readiness_provider_sidecar"):
        return "review-provider-market-data-imbalance-route-readiness"
...
def _recommendation_for_check(check: str) -> str:
    if check.startswith("provider_route_enable_route_readiness_provider_lineage"):
        return "review_provider_lineage_selection_contract_before_broker_dispatch"
    if check.startswith("provider_route_enable_dispatch_roundtrip_route_readiness_provider_sidecar"):
        return "review_provider_roundtrip_route_readiness_sidecar_proof_before_broker_dispatch"
    if check.startswith("provider_route_enable_route_readiness_provider_sidecar"):
        return "review_provider_route_readiness_sidecar_proof_before_broker_dispatch"
    if check.startswith("provider_route_enable"):
