...
def _runbook_markdown(summary: pd.Series, action_queue: pd.DataFrame) -> str:
    ...
    lines = [
        ...
        f"- Broker route-enable route-enable identity active: {'yes' if _to_bool(summary.get('runtime_telemetry_broker_readiness_route_enable_route_enable_route_contract_identity_active')) else 'no'}",
        f"- Broker route-enable route-enable identity digest: {_code(summary.get('runtime_telemetry_broker_readiness_route_enable_route_enable_route_contract_identity_sha256'))}",
        f"- Current broker route-enable route-enable identity digest: {_code(summary.get('runtime_telemetry_current_broker_readiness_route_enable_route_enable_route_contract_identity_sha256'))}",
        f"- Broker route-enable route-enable identity matches current: {'yes' if _to_bool(summary.get('runtime_telemetry_broker_readiness_route_enable_route_enable_route_contract_identity_matches_current')) else 'no'}",
        f"- Broker route-enable route-enable route-enable identity active: {'yes' if _to_bool(summary.get('runtime_telemetry_broker_readiness_route_enable_route_enable_route_enable_route_contract_identity_active')) else 'no'}",
        f"- Broker route-enable route-enable route-enable identity digest: {_code(summary.get('runtime_telemetry_broker_readiness_route_enable_route_enable_route_enable_route_contract_identity_sha256'))}",
        f"- Current broker route-enable route-enable route-enable identity digest: {_code(summary.get('runtime_telemetry_current_broker_readiness_route_enable_route_enable_route_enable_route_contract_identity_sha256'))}",
        f"- Broker route-enable route-enable route-enable identity matches current: {'yes' if _to_bool(summary.get('runtime_telemetry_broker_readiness_route_enable_route_enable_route_enable_route_contract_identity_matches_current')) else 'no'}",
...
def _broker_route_readiness_fields(primary: pd.Series, fallback: pd.Series | None = None) -> dict[str, object]:
    fallback = primary if fallback is None else fallback
    return {
        "broker_route_readiness_required": _identity_bool(
            primary,
            fallback,
            "broker_route_readiness_required",
        ),
        "broker_route_readiness_provided": _identity_bool(
            primary,
            fallback,
            "broker_route_readiness_provided",
        ),
        "broker_route_readiness_ready": _identity_bool(primary, fallback, "broker_route_readiness_ready"),
        "broker_route_readiness_strategy": _identity_text(primary, fallback, "broker_route_readiness_strategy"),
        "broker_route_readiness_market": _identity_text(primary, fallback, "broker_route_readiness_market"),
        "broker_route_readiness_route_ready_pairs": int(
            _identity_float(primary, fallback, "broker_route_readiness_route_ready_pairs")
        ),
        "broker_route_readiness_gap_pairs": int(
            _identity_float(primary, fallback, "broker_route_readiness_gap_pairs")
        ),
        "broker_route_readiness_recommendation": _identity_text(
            primary,
            fallback,
            "broker_route_readiness_recommendation",
        ),
        "broker_route_readiness_ops_launch_controls_ready": _identity_bool(
            primary,
            fallback,
            "broker_route_readiness_ops_launch_controls_ready",
        ),
        "broker_route_readiness_ops_launch_control_failures": _identity_text(
            primary,
            fallback,
