...
def _checks(
    ...
    checks.extend(
        _broker_readiness_route_enable_route_enable_route_enable_route_contract_identity_checks(
            scaleup_provenance,
            runtime,
        )
    )
...
def _broker_readiness_route_enable_route_enable_route_enable_route_contract_identity_checks(
    provenance: dict[str, Any],
    runtime: dict[str, Any],
) -> list[dict[str, object]]:
    scaleup = scaleup_runtime_fields(provenance)
    ...
    return [
        ...
        _check(
            ...
            bool(scaleup_sha256),
            (
                "scale-up broker route-enable route-enable route-enable "
        ...
        _check(
            ...
            (
                "scale-up broker route-enable route-enable route-enable "
                "route contract identity digest differs from the current "
                "recursive broker-readiness source"
            ),
        ),
        _check(
            ...
            (
                "scale-up claimed-current broker route-enable route-enable "
                "route-enable route contract identity differs from the "
                "current recursive broker-readiness source"
            ),
        ),
        _check(
            ...
            (
                "scale-up broker route-enable route-enable route-enable "
                "route contract identity current-source verdict failed"
            ),
        ),
        _check(
            f"{runtime_prefix}active",
            ...
            (
                "runtime session did not retain the active broker "
                "route-enable route-enable route-enable route identity"
            ),
        ),
        _check(
            f"{runtime_prefix}sha256_present",
            ...
            bool(runtime_sha256),
            (
                "runtime broker route-enable route-enable route-enable "
                "route contract identity digest is missing"
            ),
        ),
        _check(
        ...
        _check(
            ...
            (
                "runtime telemetry claimed-current broker route-enable "
                "route-enable route-enable route identity differs from the "
                "current recursive broker-readiness source"
            ),
        ),
        _check(
            ...
            (
                "runtime broker route-enable route-enable route-enable "
                "route identity no longer matches current scale-up"
            ),
        ),
        _check(
            f"{lineage_prefix}active",
        ...
        _check(
            ...
            (
                "runtime lineage did not recover the current broker "
                "route-enable route-enable route-enable route contract "
                "identity digest"
            ),
        ),
        _check(
            ...
            (
                "runtime route-enable route-enable route-enable route "
                "contract identity no longer matches the current recursive "
                "broker-readiness source"
            ),
        ),
    ]
...
def _runbook_markdown(summary_row: pd.Series, action_queue: pd.DataFrame) -> str:
    ...
    lines = [
        ...
        f"- Broker route-enable route-enable route-enable route contract identity active: {'yes' if _to_bool(summary_row.get('runtime_lineage_broker_readiness_route_enable_route_enable_route_enable_route_contract_identity_active')) else 'no'}",
        f"- Broker route-enable route-enable route-enable route contract identity digest: {_code(summary_row.get('runtime_telemetry_broker_readiness_route_enable_route_enable_route_enable_route_contract_identity_sha256'))}",
        f"- Current broker route-enable route-enable route-enable route contract identity digest: {_code(summary_row.get('runtime_lineage_current_broker_readiness_route_enable_route_enable_route_enable_route_contract_identity_sha256'))}",
        f"- Broker route-enable route-enable route-enable route contract identity matches current: {'yes' if _to_bool(summary_row.get('runtime_lineage_broker_readiness_route_enable_route_enable_route_enable_route_contract_identity_matches_current')) else 'no'}",
