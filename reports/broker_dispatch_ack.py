        ...
        send_lineage_checks = [
            ...
            *(
                [
                    _check(
                        (
                            "broker_dispatch_send_broker_dispatch_route_"
                            "enable_cutover_runtime_telemetry_broker_"
                            "readiness_roundtrip_contract_identity_"
                            "sha256_present"
                        ),
                        ...
                        (
                            "send packet route contract identity digest "
                            "is missing"
                        ),
                    ),
                    _check(
                        ...
                        (
                            "send packet route contract identity digest "
                            "differs from the current dispatch source"
                        ),
                    ),
                    _check(
                        (
                        ...
                        (
                            "send packet route contract identity no longer "
                            "matches the current dispatch source"
                        ),
                    ),
                ]
            ...
            *(
                [
                    _check(
                        ...
                        (
                            "send packet broker route contract identity "
                            "digest is missing"
                        ),
                    ),
                    _check(
                        ...
                        (
                            "send packet broker route contract identity "
                            "digest differs from the current dispatch source"
                        ),
                    ),
                    _check(
                        (
                        ...
                        _to_bool(
                            dispatch_summary.get(
                                (
                                    "broker_dispatch_send_dispatch_"
                        ...
                        (
                            "send packet broker route contract identity no "
                            "longer matches the current dispatch source"
                        ),
                    ),
                ]
            ...
            *(
                [
                    _check(
                        ...
                        (
                            "send packet broker route-enable route contract "
                    ...
                    _check(
                        ...
                        (
                            "send packet broker route-enable route contract "
                            "identity digest differs from the current dispatch "
                            "source"
                        ),
                    ),
                    _check(
                        ...
                        _to_bool(
                            dispatch_summary.get(
                                (
                                    "broker_dispatch_send_dispatch_"
                                    "route_enable_route_enable_route_contract_"
                        ...
                        (
                            "send packet broker route-enable route contract "
                            "identity no longer matches the current dispatch "
                            "source"
                        ),
                    ),
                ]
            ...
            *(
                [
                    _check(
                        ...
                        (
                            "send packet broker route-enable route-enable "
                    ...
                    _check(
                        ...
                        (
                            "send packet broker route-enable route-enable "
                            "route contract identity digest differs from the "
...
def _component(check: str) -> str:
    if (
        ...
        or (
            check.startswith("broker_dispatch_send_")
            and "contract_identity" in check
            and (
                "broker_dispatch_route_enable_cutover" in check
                or check.startswith(
                    "broker_dispatch_send_dispatch_route_"
                    "contract_identity_"
                )
...
def _action_recommendation(check: str) -> str:
    ...
    if component == "broker_readiness" and (
        ...
        or check.startswith(
            "broker_dispatch_send_dispatch_route_contract_identity_"
        )
        or check.startswith(
            "broker_dispatch_send_dispatch_route_enable_"
            "route_contract_identity_"
...
def _runbook_markdown(summary_row: pd.Series, action_queue: pd.DataFrame) -> str:
    ...
    lines = [
        ...
        (
            "- Send route contract identity active: "
            f"{'yes' if _to_bool(summary_row.get('broker_dispatch_send_dispatch_route_contract_identity_active')) else 'no'}"
        ),
        (
            "- Send carried route contract identity: "
            f"{_code(summary_row.get('broker_dispatch_send_broker_dispatch_route_enable_cutover_runtime_telemetry_broker_readiness_roundtrip_contract_identity_sha256'))}"
        ),
        (
            "- Current dispatch route contract identity: "
            f"{_code(summary_row.get('broker_dispatch_send_current_dispatch_route_contract_identity_sha256'))}"
        ),
        (
            "- Send route contract identity matches current: "
            f"{'yes' if _to_bool(summary_row.get('broker_dispatch_send_dispatch_route_contract_identity_matches_current')) else 'no'}"
        ),
        (
            "- Send broker route contract identity active: "
            f"{'yes' if _to_bool(summary_row.get('broker_dispatch_send_dispatch_route_enable_route_contract_identity_active')) else 'no'}"
        ),
        (
            "- Send carried broker route contract identity: "
            f"{_code(summary_row.get('broker_dispatch_send_broker_dispatch_route_enable_cutover_runtime_telemetry_broker_readiness_route_contract_identity_sha256'))}"
        ),
        (
            "- Current dispatch broker route contract identity: "
            f"{_code(summary_row.get('broker_dispatch_send_current_dispatch_route_enable_route_contract_identity_sha256'))}"
        ),
        (
            "- Send broker route contract identity matches current: "
            f"{'yes' if _to_bool(summary_row.get('broker_dispatch_send_dispatch_route_enable_route_contract_identity_matches_current')) else 'no'}"
        ),
        (
            "- Send broker route-enable route contract identity active: "
            f"{'yes' if _to_bool(summary_row.get('broker_dispatch_send_dispatch_route_enable_route_enable_route_contract_identity_active')) else 'no'}"
        ),
        (
            "- Send carried broker route-enable route contract identity: "
            f"{_code(summary_row.get('broker_dispatch_send_broker_dispatch_route_enable_cutover_runtime_telemetry_broker_readiness_route_enable_route_contract_identity_sha256'))}"
        ),
        (
            "- Current dispatch broker route-enable route contract identity: "
            f"{_code(summary_row.get('broker_dispatch_send_current_dispatch_route_enable_route_enable_route_contract_identity_sha256'))}"
        ),
        (
            "- Send broker route-enable route contract identity matches current: "
            f"{'yes' if _to_bool(summary_row.get('broker_dispatch_send_dispatch_route_enable_route_enable_route_contract_identity_matches_current')) else 'no'}"
        ),
        (
            "- Send broker route-enable route-enable route contract identity active: "
            f"{'yes' if _to_bool(summary_row.get('broker_dispatch_send_dispatch_route_enable_route_enable_route_enable_route_contract_identity_active')) else 'no'}"
        ),
        (
            "- Send carried broker route-enable route-enable route contract identity: "
            f"{_code(summary_row.get('broker_dispatch_send_broker_dispatch_route_enable_cutover_runtime_telemetry_broker_readiness_route_enable_route_enable_route_contract_identity_sha256'))}"
        ),
        (
            "- Current dispatch broker route-enable route-enable route contract identity: "
            f"{_code(summary_row.get('broker_dispatch_send_current_dispatch_route_enable_route_enable_route_enable_route_contract_identity_sha256'))}"
        ),
        (
            "- Send broker route-enable route-enable route contract identity matches current: "
            f"{'yes' if _to_bool(summary_row.get('broker_dispatch_send_dispatch_route_enable_route_enable_route_enable_route_contract_identity_matches_current')) else 'no'}"
        ),
