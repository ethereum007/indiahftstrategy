...
STRATEGY_PORTFOLIO_LEADLAG_SEND_FIELDS = (
    *STRATEGY_PORTFOLIO_LEADLAG_DISPATCH_FIELDS,
    "leadlag_dispatch_contract_consistent",
)
STRATEGY_PORTFOLIO_LEADLAG_ACK_FIELDS = (
    *STRATEGY_PORTFOLIO_LEADLAG_SEND_FIELDS,
...
ACK_COMPLETE_FINAL_LINEAGE_FIELD_PREFIX = (
    "ack_complete_final_broker_dispatch_roundtrip_vendor_market_data_batch"
)
ACK_COMPLETE_FINAL_LINEAGE_SUMMARY_FIELD_PREFIX = (
    "send_final_broker_dispatch_roundtrip_vendor_market_data_batch"
)
ACK_COMPLETE_FINAL_LINEAGE_DIGEST_FIELDS: tuple[str, ...] = (
    "current_application_lineage_sha256",
    "broker_application_lineage_sha256",
    "scaleup_carried_application_lineage_sha256",
    "cutover_carried_application_lineage_sha256",
...
ACK_EXTENDED_COMPLETE_FINAL_LINEAGE_FIELD_PREFIX = (
    "ack_extended_complete_final_broker_dispatch_roundtrip_vendor_market_data_batch"
)
ACK_EXTENDED_COMPLETE_FINAL_LINEAGE_SUMMARY_FIELD_PREFIX = (
    "send_complete_final_broker_dispatch_roundtrip_vendor_market_data_batch"
)
ACK_EXTENDED_COMPLETE_FINAL_LINEAGE_DIGEST_FIELDS: tuple[str, ...] = (
    *ACK_COMPLETE_FINAL_LINEAGE_DIGEST_FIELDS,
...
ACK_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_40_FIELD_PREFIX = (
    "ack_latest_extended_complete_final_broker_dispatch_roundtrip_vendor_market_data_batch"
)
ACK_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_40_SUMMARY_FIELD_PREFIX = (
    "send_extended_complete_final_broker_dispatch_roundtrip_vendor_market_data_batch"
)
ACK_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_40_DIGEST_FIELDS: tuple[str, ...] = (
    *ACK_EXTENDED_COMPLETE_FINAL_LINEAGE_DIGEST_FIELDS,
...
ACK_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_48_FIELD_PREFIX = (
    "ack_current_latest_extended_complete_final_broker_dispatch_roundtrip_vendor_market_data_batch"
)
ACK_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_48_SUMMARY_FIELD_PREFIX = (
    "send_latest_extended_complete_final_broker_dispatch_roundtrip_vendor_market_data_batch"
)
ACK_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_48_DIGEST_FIELDS: tuple[str, ...] = (
    *ACK_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_40_DIGEST_FIELDS,
...
ACK_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_64_FIELD_PREFIX = (
    "ack_verified_reconciled_current_latest_extended_complete_final_broker_dispatch_roundtrip_vendor_market_data_batch"
)
ACK_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_64_SUMMARY_FIELD_PREFIX = (
    "send_reconciled_current_latest_extended_complete_final_broker_dispatch_roundtrip_vendor_market_data_batch"
)
ACK_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_64_DIGEST_FIELDS: tuple[
    str, ...
] = ACK_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_56_DIGEST_FIELDS
ACK_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_64_STAGE_FIELDS: tuple[
    str, ...
] = ACK_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_56_STAGE_FIELDS
ACK_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_64_CURRENT_STAGE_FIELDS: tuple[
    str, ...
] = (
...
ACK_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_64_REVIEW_FIELDS: tuple[
    str, ...
] = (
...
ACK_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_64_ACK_REVIEW_FIELD = (
...
ACK_CONFIRMED_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_72_FIELD_PREFIX = (
...
ACK_CONFIRMED_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_72_SUMMARY_FIELD_PREFIX = (
    "send_verified_reconciled_current_latest_extended_complete_final_broker_dispatch_roundtrip_vendor_market_data_batch"
)
ACK_CONFIRMED_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_72_VERIFIED_REVIEW_FIELDS: tuple[
    str, ...
] = (
...
ACK_CONFIRMED_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_72_ACK_REVIEW_FIELD = (
...
def evaluate_broker_dispatch_roundtrip(
    *,
    ...
    ack_config = ack_config or {}
    dispatch_summary_row = dispatch_summary.iloc[0]
    send_summary_row = send_summary.iloc[0]
    ack_summary_row = ack_summary.iloc[0]
    dispatch_row = _component_summary_state(dispatch_summary_row, dispatch_config)
    send_row = _component_summary_state(send_summary_row, send_config)
    ack_row = _component_summary_state(ack_summary_row, ack_config)
    ack_lineage = broker_dispatch_ack_lineage or empty_broker_dispatch_ack_lineage(
    ...
    for column, value in _strategy_portfolio_leadlag_roundtrip_state(
    ...
    orders = pd.concat(
        [
            orders,
            pd.DataFrame(roundtrip_metadata, index=orders.index),
        ],
        axis=1,
    )
    checks = _checks(
...
def _component_summary_state(row: pd.Series, config: dict[str, Any]) -> pd.Series:
    ...
    broker_vendor_latest_extended_complete_final_lineage_40 = (
        _broker_vendor_latest_extended_complete_final_lineage_40_state(
            state,
            config,
        )
    )
    broker_vendor_current_latest_extended_complete_final_lineage_48 = (
        _broker_vendor_current_latest_extended_complete_final_lineage_48_state(
            state,
            config,
        )
    )
    ...
    broker_vendor_verified_reconciled_current_latest_extended_complete_final_lineage_64 = (
        _broker_vendor_verified_reconciled_current_latest_extended_complete_final_lineage_64_state(
            state,
            config,
        )
    )
    broker_vendor_confirmed_verified_reconciled_current_latest_extended_complete_final_lineage_72 = (
        _broker_vendor_confirmed_verified_reconciled_current_latest_extended_complete_final_lineage_72_state(
            state,
            config,
        )
    )
    ...
    for field, value in broker_vendor_extended_complete_final_lineage.items():
        state[field] = value
    for field, value in broker_vendor_latest_extended_complete_final_lineage_40.items():
        state[field] = value
    ...
    for field, value in broker_vendor_reconciled_current_latest_extended_complete_final_lineage_56.items():
        state[field] = value
    for field, value in broker_vendor_verified_reconciled_current_latest_extended_complete_final_lineage_64.items():
        state[field] = value
    for field, value in broker_vendor_confirmed_verified_reconciled_current_latest_extended_complete_final_lineage_72.items():
        state[field] = value
    return state
...
def _broker_vendor_final_lineage_comparison_source(
    ...
    comparison = config.get(ACK_FINAL_LINEAGE_COMPARISON_KEY)
...
def _broker_vendor_complete_final_lineage_comparison_source(
    ...
    comparison = config.get(ACK_COMPLETE_FINAL_LINEAGE_COMPARISON_KEY)
    return comparison if isinstance(comparison, dict) else {}
...
def _broker_vendor_extended_complete_final_lineage_comparison_source(
    ...
    comparison = config.get(ACK_EXTENDED_COMPLETE_FINAL_LINEAGE_COMPARISON_KEY)
    return comparison if isinstance(comparison, dict) else {}


def _broker_vendor_latest_extended_complete_final_lineage_40_comparison_source(
    config: dict[str, Any],
) -> dict[str, Any]:
...
def _broker_vendor_current_latest_extended_complete_final_lineage_48_comparison_source(
    config: dict[str, Any],
) -> dict[str, Any]:
    comparison = config.get(
        ACK_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_48_COMPARISON_KEY
...
def _broker_vendor_verified_reconciled_current_latest_extended_complete_final_lineage_64_comparison_source(
    config: dict[str, Any],
) -> dict[str, Any]:
    comparison = config.get(
        ACK_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_64_COMPARISON_KEY
...
def _broker_vendor_confirmed_verified_reconciled_current_latest_extended_complete_final_lineage_72_comparison_source(
    config: dict[str, Any],
) -> dict[str, Any]:
    comparison = config.get(
        ACK_CONFIRMED_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_72_COMPARISON_KEY
...
def _broker_vendor_final_lineage_state(
    ...
    prefix = ACK_FINAL_LINEAGE_FIELD_PREFIX
    summary_prefix = ACK_FINAL_LINEAGE_SUMMARY_FIELD_PREFIX
    ...
    for field in ACK_FINAL_LINEAGE_DIGEST_FIELDS:
...
def _broker_vendor_complete_final_lineage_state(
    ...
    prefix = ACK_COMPLETE_FINAL_LINEAGE_FIELD_PREFIX
    summary_prefix = ACK_COMPLETE_FINAL_LINEAGE_SUMMARY_FIELD_PREFIX
    fields: dict[str, object] = {
        f"{prefix}_lineage_match_required": _to_bool(
            comparison.get(
                "required",
    ...
    for field in ACK_COMPLETE_FINAL_LINEAGE_DIGEST_FIELDS:
        fields[f"{prefix}_{field}"] = _sha256_text(
            _first_text(
                comparison.get(field, ""),
                row.get(f"{summary_prefix}_{field}", ""),
...
def _broker_vendor_extended_complete_final_lineage_state(
    ...
    prefix = ACK_EXTENDED_COMPLETE_FINAL_LINEAGE_FIELD_PREFIX
    summary_prefix = ACK_EXTENDED_COMPLETE_FINAL_LINEAGE_SUMMARY_FIELD_PREFIX
    fields: dict[str, object] = {
        f"{prefix}_lineage_match_required": _to_bool(
            comparison.get(
                "required",
        ...
        f"{prefix}_carried_application_lineage_sha256": _sha256_text(
            _first_text(
                comparison.get("carried_application_lineage_sha256", ""),
                row.get(
                    f"{summary_prefix}_ack_extended_complete_final_review_carried_application_lineage_sha256",
                    "",
                ),
            )
        ),
    }
    for field in ACK_EXTENDED_COMPLETE_FINAL_LINEAGE_DIGEST_FIELDS:
        fields[f"{prefix}_{field}"] = _sha256_text(
            _first_text(
                comparison.get(field, ""),
                row.get(f"{summary_prefix}_{field}", ""),
...
def _broker_vendor_latest_extended_complete_final_lineage_40_state(
    row: pd.Series,
    config: dict[str, Any],
) -> dict[str, object]:
    comparison = (
        _broker_vendor_latest_extended_complete_final_lineage_40_comparison_source(
            config
        )
    )
    prefix = ACK_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_40_FIELD_PREFIX
    summary_prefix = ACK_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_40_SUMMARY_FIELD_PREFIX
    fields: dict[str, object] = {
        f"{prefix}_lineage_match_required": _to_bool(
            comparison.get(
                "required",
        ...
        f"{prefix}_carried_application_lineage_sha256": _sha256_text(
            _first_text(
                comparison.get("carried_application_lineage_sha256", ""),
                row.get(
                    f"{summary_prefix}_ack_latest_extended_complete_final_review_carried_application_lineage_sha256",
                    "",
                ),
            )
        ),
    }
    for field in ACK_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_40_DIGEST_FIELDS:
        fields[f"{prefix}_{field}"] = _sha256_text(
            _first_text(
                comparison.get(field, ""),
                row.get(f"{summary_prefix}_{field}", ""),
...
def _broker_vendor_current_latest_extended_complete_final_lineage_48_state(
    row: pd.Series,
    config: dict[str, Any],
) -> dict[str, object]:
    comparison = (
        _broker_vendor_current_latest_extended_complete_final_lineage_48_comparison_source(
            config
        )
    )
    prefix = ACK_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_48_FIELD_PREFIX
    summary_prefix = (
        ACK_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_48_SUMMARY_FIELD_PREFIX
    )
    fields: dict[str, object] = {
    ...
    fields[f"{prefix}_carried_application_lineage_sha256"] = _sha256_text(
        _first_text(
            comparison.get("carried_application_lineage_sha256", ""),
            row.get(
                f"{summary_prefix}_ack_current_latest_extended_complete_final_review_carried_application_lineage_sha256",
                "",
            ),
        )
    )
...
def _broker_vendor_verified_reconciled_current_latest_extended_complete_final_lineage_64_state(
    row: pd.Series,
    config: dict[str, Any],
) -> dict[str, object]:
    comparison = (
        _broker_vendor_verified_reconciled_current_latest_extended_complete_final_lineage_64_comparison_source(
            config
        )
    )
    prefix = ACK_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_64_FIELD_PREFIX
    summary_prefix = (
        ACK_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_64_SUMMARY_FIELD_PREFIX
    )
    ack_review_field = (
    ...
    fields: dict[str, object] = {
        ...
        f"{prefix}_carried_application_lineage_sha256": _sha256_text(
            _first_text(
                comparison.get("carried_application_lineage_sha256", ""),
                row.get(f"{summary_prefix}_{ack_review_field}", ""),
            )
        ),
    }
    for field in (
        *ACK_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_64_DIGEST_FIELDS,
        *ACK_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_64_STAGE_FIELDS,
        *ACK_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_64_CURRENT_STAGE_FIELDS,
        *ACK_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_64_REVIEW_FIELDS,
        ack_review_field,
    ):
        fields[f"{prefix}_{field}"] = _sha256_text(
            _first_text(
...
def _broker_vendor_confirmed_verified_reconciled_current_latest_extended_complete_final_lineage_72_state(
    row: pd.Series,
    config: dict[str, Any],
) -> dict[str, object]:
    comparison = (
        _broker_vendor_confirmed_verified_reconciled_current_latest_extended_complete_final_lineage_72_comparison_source(
            config
        )
    )
    prefix = ACK_CONFIRMED_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_72_FIELD_PREFIX
    summary_prefix = ACK_CONFIRMED_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_72_SUMMARY_FIELD_PREFIX
    ack_review_field = ACK_CONFIRMED_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_72_ACK_REVIEW_FIELD
    fields: dict[str, object] = {
        f"{prefix}_lineage_match_required": _to_bool(
            comparison.get(
                "required",
    ...
    for field in (
        ...
        *ACK_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_64_REVIEW_FIELDS,
        ACK_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_64_ACK_REVIEW_FIELD,
        *ACK_CONFIRMED_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_72_VERIFIED_REVIEW_FIELDS,
        ack_review_field,
    ):
        fields[f"{prefix}_{field}"] = _sha256_text(
            _first_text(
...
def _checks(
    ...
    if _to_bool(
        ...
        route_identity_active = _to_bool(
            ack_summary.get(
                (
                    "broker_dispatch_ack_send_route_"
        ...
        current_route_identity_sha256 = _object_text(
            ack_summary.get(
                (
                    "broker_dispatch_ack_current_send_route_"
                    "contract_identity_sha256"
                ),
                "",
            )
        ...
        ack_lineage_checks = [
            ...
            *(
                [
                    _check(
                        ...
                        (
                            "acknowledgement route contract identity digest "
                            "is missing"
                        ),
                    ),
                    _check(
                    ...
                    _check(
                        ...
                        (
                            "acknowledgement route contract identity no "
                            "longer matches the current send source"
                        ),
                    ),
                ]
                ...
                [
                    _check(
                        (
                            "broker_dispatch_ack_broker_dispatch_send_"
                            "broker_dispatch_route_enable_cutover_runtime_"
                            "telemetry_broker_readiness_route_contract_"
                        ...
                        (
                            "acknowledgement broker route contract identity "
                            "digest is missing"
                        ),
                    ),
                    _check(
                        ...
                        (
                            "acknowledgement broker route contract identity "
                            "digest differs from the current send source"
                        ),
                    ),
                    _check(
                        (
                        ...
                        (
                            "acknowledgement broker route contract identity "
                            "no longer matches the current send source"
                        ),
                    ),
                ]
            ...
            *(
                [
                    _check(
                        ...
                        "present",
                        True,
                        bool(route_enable_route_enable_identity_sha256),
                        (
                            "acknowledgement broker route-enable route "
                            "contract identity digest is missing"
                        ),
                    ),
                    _check(
                        (
                        ...
                        (
                            "acknowledgement broker route-enable route "
                            "contract identity digest differs from the "
                            "current send source"
                        ),
                    ),
                    _check(
                        ...
                        (
                            "acknowledgement broker route-enable route "
                            "contract identity no longer matches the current "
                            "send source"
                        ),
                    ),
                ]
            ...
            *(
                [
                    _check(
                        "broker_dispatch_ack_broker_dispatch_send_broker_dispatch_route_enable_cutover_runtime_lineage_source_bound",
                        _to_bool(
                            ack_summary.get(
                                "broker_dispatch_ack_broker_dispatch_send_broker_dispatch_route_enable_cutover_runtime_lineage_source_bound",
                                False,
                    ...
                    _check(
                        "broker_dispatch_ack_broker_dispatch_send_broker_dispatch_route_enable_cutover_runtime_lineage_matches_current",
                        _to_bool(
                            ack_summary.get(
                                "broker_dispatch_ack_broker_dispatch_send_broker_dispatch_route_enable_cutover_runtime_lineage_matches_current",
                                False,
                    ...
                    _check(
                        "broker_dispatch_ack_broker_dispatch_send_broker_dispatch_route_enable_cutover_broker_readiness_source_matches_scaleup",
                        _to_bool(
                            ack_summary.get(
                                "broker_dispatch_ack_broker_dispatch_send_broker_dispatch_route_enable_cutover_broker_readiness_source_matches_scaleup",
                                False,
                    ...
                    _check(
                        "broker_dispatch_ack_broker_dispatch_send_broker_dispatch_route_enable_cutover_broker_readiness_matches_current",
                        _to_bool(
                            ack_summary.get(
                                "broker_dispatch_ack_broker_dispatch_send_broker_dispatch_route_enable_cutover_broker_readiness_matches_current",
                                False,
                ...
                if _to_bool(
                    ack_summary.get(
                        "broker_dispatch_ack_broker_dispatch_send_broker_dispatch_route_enable_cutover_broker_readiness_required",
                        False,
                    )
                )
                else []
...
def _strategy_portfolio_checks(*rows: pd.Series) -> list[dict[str, object]]:
    ...
    if _strategy_portfolio_leadlag_active(*rows):
        proof = rows[-1]
        ...
        checks.extend(
            [
                _check(
                    ...
                    "dispatch, send, acknowledgement, and verified acknowledgement lineage disagree on the lead-lag contract",
                ),
                _check(
                    ...
                    "acknowledgement proof did not retain a consistent send contract",
                ),
                _check(
                    ...
                    "send proof did not retain a consistent dispatch contract",
                ),
                _check(
                    ...
                    "dispatch proof did not retain a consistent route-enable contract",
                ),
                _check(
                    ...
                    "dispatch proof did not retain a consistent cutover contract",
                ),
                _check(
                    ...
                    "round-trip proof did not carry the required lead-lag lineage marker",
                ),
                _check(
                    "strategy_portfolio_leadlag_profile",
                    _text(proof, "strategy_portfolio_selected_profile"),
                    "==",
                    "leadlag",
                    _identity_key(
                        proof.get("strategy_portfolio_selected_profile", "")
                    )
                    == "leadlag",
                    "round-trip lead-lag lineage is attached to a different portfolio profile",
                ),
                _check(
                    ...
                    True,
                    lineage_ready,
                    "round-trip proof lost or malformed the measured-edge lead-lag lineage",
                ),
                _check(
                    ...
                    "round-trip proof did not retain the guard-validated lead-lag scale-up match",
                ),
            ]
        )
    return checks
...
def _broker_vendor_final_lineage_checks(
    ...
    *,
    ...
    source_prefix = ACK_FINAL_LINEAGE_FIELD_PREFIX
    prefix = "broker_dispatch_roundtrip_vendor_market_data_batch"
    ...
    ack_lineage_sha256 = _sha256_text(
        ack_row.get(
            "ack_carried_broker_dispatch_roundtrip_vendor_market_data_batch_application_lineage_sha256",
            "",
        )
    ...
    checks = [
        _check(
            ...
            True,
            lineage_match_required,
            "reconciled target round-trip review requires acknowledgement's final lineage comparison",
        ),
        _check(
            ...
            "acknowledgement reconciliation did not match every final target-lineage view",
        ),
        _check(
            ...
            "acknowledgement's final source lineage does not match final broker proof",
        ),
        _check(
            ...
            "round-trip acknowledgement broker digest does not match the final comparison",
        ),
        _check(
            ...
            "acknowledgement's independently recomputed batch digest does not match final comparison",
        ),
    ]
    carried_fields = (
    ...
    for stage, field in carried_fields:
        carried_sha256 = _sha256_text(ack_row.get(f"{source_prefix}_{field}", ""))
        checks.append(
            _check(
                ...
                (
                    f"acknowledgement's {stage.replace('_', '-')} target lineage does not "
    ...
    checks.extend(
        [
            _check(
                ...
                "acknowledgement's sender-review lineage does not match final broker proof",
            ),
            _check(
                ...
                "acknowledgement's carried review lineage does not match final broker proof",
            ),
            _check(
                f"{prefix}_roundtrip_final_review_carried_lineage_sha256_matches",
                roundtrip_lineage_sha256,
                "==",
                final_broker_lineage_sha256,
                bool(
                    lineage_match_required
                    and roundtrip_lineage_sha256
                    and final_broker_lineage_sha256
                    and roundtrip_lineage_sha256 == final_broker_lineage_sha256
                ),
                "round-trip review's independently recomputed target lineage does not match final broker proof",
            ),
        ]
    )
    return checks
...
def _broker_vendor_complete_final_lineage_checks(
    ...
    *,
    ...
    source_prefix = ACK_COMPLETE_FINAL_LINEAGE_FIELD_PREFIX
    compatibility_prefix = ACK_FINAL_LINEAGE_FIELD_PREFIX
    check_prefix = (
    ...
    compatibility_ack_review_lineage_sha256 = _sha256_text(
        ack_row.get(
    ...
    checks = [
        _check(
            ...
            True,
            lineage_match_required,
            "reconciled target round-trip review requires acknowledgement's complete final lineage comparison",
        ),
        _check(
            ...
            True,
            bool(lineage_match_required and lineage_matches),
            "acknowledgement did not match every complete final target-lineage view",
        ),
        _check(
            ...
            "acknowledgement complete-final source lineage does not match final broker proof",
        ),
        _check(
            ...
            "round-trip compatibility broker digest does not match acknowledgement's complete-final proof",
        ),
        _check(
            f"{check_prefix}_compatibility_ack_review_lineage_sha256_matches",
            compatibility_ack_review_lineage_sha256,
            "==",
            broker_lineage_sha256,
            bool(
                lineage_match_required
                and compatibility_ack_review_lineage_sha256
                and broker_lineage_sha256
                and compatibility_ack_review_lineage_sha256
                == broker_lineage_sha256
            ),
            "round-trip compatibility acknowledgement review does not match acknowledgement's complete-final proof",
        ),
    ]
    carried_fields = (
        ("prior_scaleup", "scaleup_carried_application_lineage_sha256"),
    ...
    checks.extend(
        [
            _check(
                ...
                "acknowledgement's carried complete-final review does not match final broker proof",
            ),
            _check(
                ...
                "round-trip review's independently recomputed target lineage does not match acknowledgement's complete-final proof",
            ),
        ]
    )
    return checks
...
def _broker_vendor_extended_complete_final_lineage_checks(
    ...
    *,
    ...
    source_prefix = ACK_EXTENDED_COMPLETE_FINAL_LINEAGE_FIELD_PREFIX
    compatibility_prefix = ACK_COMPLETE_FINAL_LINEAGE_FIELD_PREFIX
    check_prefix = (
    ...
    compatibility_ack_complete_final_review_lineage_sha256 = _sha256_text(
        ack_row.get(
            f"{compatibility_prefix}_carried_application_lineage_sha256",
            "",
        )
    ...
    checks = [
        _check(
            ...
            True,
            lineage_match_required,
            "reconciled target round-trip review requires acknowledgement's extended complete-final lineage comparison",
        ),
        _check(
            f"{check_prefix}_lineage_matches",
            lineage_matches,
            ...
            True,
            bool(lineage_match_required and lineage_matches),
            "acknowledgement did not match every extended complete-final target-lineage view",
        ),
        _check(
            f"{check_prefix}_source_lineage_sha256_matches",
            current_lineage_sha256,
            ...
            "acknowledgement extended complete-final source lineage does not match final broker proof",
        ),
        _check(
            f"{check_prefix}_compatibility_broker_lineage_sha256_matches",
            compatibility_broker_lineage_sha256,
            ...
            "round-trip compatibility broker digest does not match acknowledgement's extended proof",
        ),
        _check(
            f"{check_prefix}_compatibility_ack_complete_final_review_carried_lineage_sha256_matches",
            compatibility_ack_complete_final_review_lineage_sha256,
            "==",
            broker_lineage_sha256,
            bool(
                lineage_match_required
                and compatibility_ack_complete_final_review_lineage_sha256
                and broker_lineage_sha256
                and compatibility_ack_complete_final_review_lineage_sha256
                == broker_lineage_sha256
            ),
            "round-trip compatibility acknowledgement complete-final review does not match acknowledgement's extended proof",
        ),
    ]
    carried_fields = (
        ("prior_scaleup", "scaleup_carried_application_lineage_sha256"),
    ...
    send_complete_final_review_lineage_sha256 = _sha256_text(
        ack_row.get(
            f"{source_prefix}_send_complete_final_review_carried_application_lineage_sha256",
            "",
        )
    )
    ...
    roundtrip_extended_complete_final_review_lineage_sha256 = _sha256_text(
        roundtrip_lineage_sha256
    )
    checks.extend(
        [
            _check(
                ...
                "acknowledgement's carried scale-up complete-final review lineage does not match final broker proof",
            ),
            _check(
                ...
                "acknowledgement's carried cutover complete-final review lineage does not match final broker proof",
            ),
            _check(
                ...
                "acknowledgement's carried route complete-final review lineage does not match final broker proof",
            ),
            _check(
                ...
                "acknowledgement's carried dispatch complete-final review lineage does not match final broker proof",
            ),
            _check(
                ...
                "acknowledgement's carried sender complete-final review lineage does not match final broker proof",
            ),
            _check(
                ...
                "acknowledgement's carried extended complete-final review lineage does not match final broker proof",
            ),
            _check(
                f"{check_prefix}_roundtrip_extended_complete_final_review_carried_lineage_sha256_matches",
                roundtrip_extended_complete_final_review_lineage_sha256,
                "==",
                broker_lineage_sha256,
                bool(
                    lineage_match_required
                    and roundtrip_extended_complete_final_review_lineage_sha256
                    and broker_lineage_sha256
                    and roundtrip_extended_complete_final_review_lineage_sha256
                    == broker_lineage_sha256
                ),
                "round-trip review's independently recomputed target lineage does not match acknowledgement's extended proof",
            ),
        ]
    )
    return checks
...
def _broker_vendor_latest_extended_complete_final_lineage_40_checks(
    ack_row: pd.Series,
    *,
    roundtrip_lineage_sha256: str,
) -> list[dict[str, object]]:
    source_prefix = ACK_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_40_FIELD_PREFIX
    compatibility_prefix = ACK_EXTENDED_COMPLETE_FINAL_LINEAGE_FIELD_PREFIX
    check_prefix = (
    ...
    compatibility_ack_extended_complete_final_review_lineage_sha256 = _sha256_text(
        ack_row.get(
            f"{compatibility_prefix}_carried_application_lineage_sha256",
            "",
        )
    ...
    checks = [
        _check(
            ...
            True,
            lineage_match_required,
            "reconciled target round-trip review requires acknowledgement's latest extended complete-final lineage comparison",
        ),
        _check(
            f"{check_prefix}_lineage_matches",
            lineage_matches,
            ...
            True,
            bool(lineage_match_required and lineage_matches),
            "acknowledgement did not match every latest extended complete-final target-lineage view",
        ),
        _check(
            f"{check_prefix}_source_lineage_sha256_matches",
            current_lineage_sha256,
            ...
            "acknowledgement latest extended complete-final source lineage does not match final broker proof",
        ),
        _check(
            f"{check_prefix}_compatibility_broker_lineage_sha256_matches",
            compatibility_broker_lineage_sha256,
            ...
            "round-trip compatibility broker digest does not match acknowledgement's latest extended proof",
        ),
        _check(
            f"{check_prefix}_compatibility_ack_extended_complete_final_review_carried_lineage_sha256_matches",
            compatibility_ack_extended_complete_final_review_lineage_sha256,
            "==",
            broker_lineage_sha256,
            bool(
                lineage_match_required
                and compatibility_ack_extended_complete_final_review_lineage_sha256
                and broker_lineage_sha256
                and compatibility_ack_extended_complete_final_review_lineage_sha256
                == broker_lineage_sha256
            ),
            "round-trip compatibility acknowledgement extended review does not match acknowledgement's latest extended proof",
        ),
    ]
    carried_fields = (
        ("prior_scaleup", "scaleup_carried_application_lineage_sha256"),
    ...
    send_extended_complete_final_review_lineage_sha256 = _sha256_text(
        ack_row.get(
            f"{source_prefix}_send_extended_complete_final_review_carried_application_lineage_sha256",
            "",
        )
    )
    ...
    roundtrip_latest_extended_complete_final_review_lineage_sha256 = _sha256_text(
        roundtrip_lineage_sha256
    )
    checks.extend(
        [
            _check(
                ...
                "acknowledgement's broker-readiness extended review lineage does not match final broker proof",
            ),
            _check(
                ...
                "acknowledgement's scale-up extended review lineage does not match final broker proof",
            ),
            _check(
                ...
                "acknowledgement's cutover extended review lineage does not match final broker proof",
            ),
            _check(
                ...
                "acknowledgement's route extended review lineage does not match final broker proof",
            ),
            _check(
                ...
                "acknowledgement's dispatch extended review lineage does not match final broker proof",
            ),
            _check(
                ...
                "acknowledgement's send extended review lineage does not match final broker proof",
            ),
            _check(
                ...
                "acknowledgement's carried latest extended complete-final review lineage does not match final broker proof",
            ),
            _check(
                f"{check_prefix}_roundtrip_latest_extended_complete_final_review_carried_lineage_sha256_matches",
                roundtrip_latest_extended_complete_final_review_lineage_sha256,
                "==",
                broker_lineage_sha256,
                bool(
                    lineage_match_required
                    and roundtrip_latest_extended_complete_final_review_lineage_sha256
                    and broker_lineage_sha256
                    and roundtrip_latest_extended_complete_final_review_lineage_sha256
                    == broker_lineage_sha256
                ),
                "round-trip review's independently recomputed target lineage does not match acknowledgement's latest extended proof",
            ),
        ]
    )
    return checks
...
def _broker_vendor_current_latest_extended_complete_final_lineage_48_checks(
    ack_row: pd.Series,
    *,
    roundtrip_lineage_sha256: str,
) -> list[dict[str, object]]:
    source_prefix = ACK_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_48_FIELD_PREFIX
    compatibility_prefix = ACK_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_40_FIELD_PREFIX
    check_prefix = (
        "broker_dispatch_roundtrip_vendor_market_data_batch_"
        "ack_current_latest_extended_complete_final"
    )
    lineage_match_required = _to_bool(
        ack_row.get(f"{source_prefix}_lineage_match_required", False)
    )
    ...
    compatibility_ack_latest_extended_complete_final_review_lineage_sha256 = (
        _sha256_text(
            ack_row.get(
                f"{compatibility_prefix}_carried_application_lineage_sha256",
                "",
    ...
    checks = [
        _check(
            ...
            True,
            lineage_match_required,
            "reconciled target round-trip review requires acknowledgement's current latest extended complete-final lineage comparison",
        ),
        _check(
            f"{check_prefix}_lineage_matches",
            lineage_matches,
            ...
            True,
            bool(lineage_match_required and lineage_matches),
            "acknowledgement did not match every current latest extended complete-final target-lineage view",
        ),
        _check(
            f"{check_prefix}_source_lineage_sha256_matches",
            current_lineage_sha256,
            ...
            "acknowledgement current latest extended complete-final source lineage does not match final broker proof",
        ),
        _check(
            f"{check_prefix}_compatibility_broker_lineage_sha256_matches",
            compatibility_broker_lineage_sha256,
            ...
            "round-trip compatibility broker digest does not match acknowledgement's current latest proof",
        ),
        _check(
            f"{check_prefix}_compatibility_ack_latest_extended_complete_final_review_carried_lineage_sha256_matches",
            compatibility_ack_latest_extended_complete_final_review_lineage_sha256,
            "==",
            broker_lineage_sha256,
            bool(
                lineage_match_required
                and compatibility_ack_latest_extended_complete_final_review_lineage_sha256
                and broker_lineage_sha256
                and compatibility_ack_latest_extended_complete_final_review_lineage_sha256
                == broker_lineage_sha256
            ),
            "round-trip compatibility acknowledgement latest review does not match acknowledgement's current proof",
        ),
    ]
    for field in ACK_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_48_DIGEST_FIELDS:
        if field in {
    ...
    roundtrip_current_latest_extended_complete_final_review_lineage_sha256 = (
        _sha256_text(roundtrip_lineage_sha256)
    )
    checks.append(
        _check(
            f"{check_prefix}_roundtrip_current_latest_extended_complete_final_review_carried_lineage_sha256_matches",
            roundtrip_current_latest_extended_complete_final_review_lineage_sha256,
            "==",
            broker_lineage_sha256,
            bool(
                lineage_match_required
                and roundtrip_current_latest_extended_complete_final_review_lineage_sha256
                and broker_lineage_sha256
                and roundtrip_current_latest_extended_complete_final_review_lineage_sha256
                == broker_lineage_sha256
            ),
            "round-trip review's independently recomputed target lineage does not match acknowledgement's current latest proof",
        )
    )
    return checks
...
def _broker_vendor_verified_reconciled_current_latest_extended_complete_final_lineage_64_checks(
    ack_row: pd.Series,
    *,
    roundtrip_lineage_sha256: str,
) -> list[dict[str, object]]:
    source_prefix = ACK_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_64_FIELD_PREFIX
    compatibility_prefix = ACK_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_56_FIELD_PREFIX
    check_prefix = (
        "broker_dispatch_roundtrip_vendor_market_data_batch_"
        "ack_verified_reconciled_current_latest_extended_complete_final"
    )
    lineage_match_required = _to_bool(
        ack_row.get(f"{source_prefix}_lineage_match_required", False)
    )
    ...
    roundtrip_verified_lineage_sha256 = _sha256_text(roundtrip_lineage_sha256)
    checks = [
        _check(
            f"{check_prefix}_lineage_match_required",
            lineage_match_required,
            ...
            True,
            lineage_match_required,
            "verified reconciled target round-trip review requires acknowledgement's verified reconciled lineage comparison",
        ),
        _check(
            f"{check_prefix}_lineage_matches",
            lineage_matches,
            ...
            True,
            bool(lineage_match_required and lineage_matches),
            "acknowledgement did not match every verified reconciled target-lineage view",
        ),
        _check(
            f"{check_prefix}_source_lineage_sha256_matches",
            current_lineage_sha256,
            ...
            "acknowledgement verified reconciled source lineage does not match final broker proof",
        ),
        _check(
            f"{check_prefix}_compatibility_broker_lineage_sha256_matches",
            compatibility_broker_lineage_sha256,
            ...
            "established round-trip broker digest does not match acknowledgement's verified reconciled proof",
        ),
        _check(
            f"{check_prefix}_compatibility_roundtrip_verified_reconciled_current_latest_extended_complete_final_review_carried_lineage_sha256_matches",
            roundtrip_verified_lineage_sha256,
            "==",
            broker_lineage_sha256,
            bool(
                lineage_match_required
                and roundtrip_verified_lineage_sha256
                and broker_lineage_sha256
                and roundtrip_verified_lineage_sha256 == broker_lineage_sha256
            ),
            "independently recomputed round-trip lineage does not match acknowledgement's verified reconciled proof",
        ),
    ]
    for field in ACK_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_64_DIGEST_FIELDS:
        if field in {
            "current_application_lineage_sha256",
            "broker_application_lineage_sha256",
        }:
        ...
        checks.append(
            _check(
                ...
                f"acknowledgement's {stage.replace('_', '-')} target lineage does not match verified reconciled broker proof",
            )
        )
    for field in (
        *ACK_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_64_STAGE_FIELDS,
    ...
    ack_verified_generic_lineage_sha256 = _sha256_text(
        ack_row.get(f"{source_prefix}_carried_application_lineage_sha256", "")
    )
    for stage, carried_sha256 in (
        (
            "ack_verified_reconciled_current_latest_extended_complete_final_review_generic",
            ack_verified_generic_lineage_sha256,
        ),
        (
            "roundtrip_verified_reconciled_current_latest_extended_complete_final_review",
            roundtrip_verified_lineage_sha256,
        ),
    ):
        checks.append(
            _check(
                ...
                f"{stage.replace('_', '-')} target lineage does not match acknowledgement's verified reconciled proof",
            )
        )
    return checks


def _broker_vendor_confirmed_verified_reconciled_current_latest_extended_complete_final_lineage_72_checks(
    ack_row: pd.Series,
    *,
    roundtrip_lineage_sha256: str,
) -> list[dict[str, object]]:
    source_prefix = ACK_CONFIRMED_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_72_FIELD_PREFIX
    compatibility_prefix = ACK_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_64_FIELD_PREFIX
    check_prefix = (
        "broker_dispatch_roundtrip_vendor_market_data_batch_"
        "ack_confirmed_verified_reconciled_current_latest_extended_complete_final"
    )
    lineage_match_required = _to_bool(
        ack_row.get(f"{source_prefix}_lineage_match_required", False)
    )
    ...
    checks = [
        _check(
            ...
            True,
            lineage_match_required,
            "confirmed verified-reconciled target round-trip review requires acknowledgement's confirmed lineage comparison",
        ),
        _check(
            f"{check_prefix}_lineage_matches",
            lineage_matches,
            ...
            True,
            bool(lineage_match_required and lineage_matches),
            "acknowledgement did not match every confirmed verified-reconciled target-lineage view",
        ),
        _check(
            f"{check_prefix}_source_lineage_sha256_matches",
            current_lineage_sha256,
            ...
            "acknowledgement confirmed verified-reconciled source lineage does not match final broker proof",
        ),
        _check(
            f"{check_prefix}_compatibility_broker_lineage_sha256_matches",
            compatibility_broker_lineage_sha256,
            ...
            "established round-trip broker digest does not match acknowledgement's confirmed verified-reconciled proof",
        ),
        _check(
            f"{check_prefix}_compatibility_roundtrip_verified_reconciled_current_latest_extended_complete_final_review_carried_lineage_sha256_matches",
            roundtrip_verified_lineage_sha256,
            ...
            "independently recomputed round-trip lineage does not match acknowledgement's confirmed verified-reconciled proof",
        ),
    ]
    for field in ACK_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_64_DIGEST_FIELDS:
        if field in {
        ...
        checks.append(
            _check(
                ...
                f"acknowledgement's {stage.replace('_', '-')} target lineage does not match confirmed verified-reconciled broker proof",
            )
        )
    for field in (
        *ACK_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_64_STAGE_FIELDS,
    ...
    ack_confirmed_generic_lineage_sha256 = _sha256_text(
        ack_row.get(f"{source_prefix}_carried_application_lineage_sha256", "")
    )
    checks.extend(
        [
            _check(
                f"{check_prefix}_ack_confirmed_verified_reconciled_current_latest_extended_complete_final_review_generic_carried_lineage_sha256_matches",
                ack_confirmed_generic_lineage_sha256,
                "==",
                broker_lineage_sha256,
                bool(
                    lineage_match_required
                    and ack_confirmed_generic_lineage_sha256
                    and broker_lineage_sha256
                    and ack_confirmed_generic_lineage_sha256
                    == broker_lineage_sha256
                ),
                "acknowledgement's generic confirmed verified-reconciled review lineage does not match final broker proof",
            ),
            _check(
                f"{check_prefix}_roundtrip_confirmed_verified_reconciled_current_latest_extended_complete_final_review_carried_lineage_sha256_matches",
                roundtrip_verified_lineage_sha256,
                "==",
                broker_lineage_sha256,
                bool(
                ...
                "round-trip's independently recomputed target lineage does not match acknowledgement's confirmed verified-reconciled proof",
            ),
        ]
    )
    return checks
...
def _broker_vendor_market_data_batch_checks(*rows: pd.Series) -> list[dict[str, object]]:
    ...
    if lineage_consistency_required:
        roundtrip_lineage_sha256 = _sha256_text(
            ack_row.get(
                "roundtrip_carried_broker_dispatch_roundtrip_vendor_market_data_batch_application_lineage_sha256",
                "",
            )
        ...
        checks.extend(
            _broker_vendor_latest_extended_complete_final_lineage_40_checks(
                ack_row,
                roundtrip_lineage_sha256=roundtrip_lineage_sha256,
            )
        )
        ...
        checks.extend(
            _broker_vendor_verified_reconciled_current_latest_extended_complete_final_lineage_64_checks(
                ack_row,
                roundtrip_lineage_sha256=roundtrip_lineage_sha256,
            )
        )
        checks.extend(
            _broker_vendor_confirmed_verified_reconciled_current_latest_extended_complete_final_lineage_72_checks(
                ack_row,
                roundtrip_lineage_sha256=roundtrip_lineage_sha256,
            )
        )
        ...
        [
            {
                ...
                **_broker_vendor_final_lineage_summary_fields(ack_summary),
                **_broker_vendor_complete_final_lineage_summary_fields(ack_summary),
                **_broker_vendor_extended_complete_final_lineage_summary_fields(
                    ack_summary
                ),
                **_broker_vendor_latest_extended_complete_final_lineage_40_summary_fields(
                    ack_summary
                ),
                ...
                **_broker_vendor_verified_reconciled_current_latest_extended_complete_final_lineage_64_summary_fields(
                    ack_summary
                ),
                **_broker_vendor_confirmed_verified_reconciled_current_latest_extended_complete_final_lineage_72_summary_fields(
                    ack_summary
                ),
...
def _component(check: str) -> str:
    if (
        check.startswith(
            ...
            "route_enable_cutover_broker_readiness_"
        )
        or (
            check.startswith("broker_dispatch_ack_")
            and "contract_identity" in check
            and (
                "broker_dispatch_send_broker_dispatch_route_enable_cutover"
                in check
                or check.startswith(
                    "broker_dispatch_ack_send_route_contract_identity_"
                )
                or check.startswith(
                    "broker_dispatch_ack_send_route_enable_"
...
def _action_recommendation(check: str) -> str:
    component = _component(check)
    if component == "broker_readiness" and (
        check.startswith(
            "broker_dispatch_ack_broker_dispatch_send_broker_dispatch_"
...
def _broker_vendor_final_lineage_summary_fields(
    ...
    source_prefix = ACK_FINAL_LINEAGE_FIELD_PREFIX
    field_prefix = ACK_FINAL_LINEAGE_FIELD_PREFIX
    fields: dict[str, object] = {
        f"{field_prefix}_lineage_match_required": _to_bool(
            ack_summary.get(f"{source_prefix}_lineage_match_required", False)
        ),
        f"{field_prefix}_lineage_matches": _to_bool(
            ack_summary.get(f"{source_prefix}_lineage_matches", False)
        ),
        f"{field_prefix}_ack_reconciliation_review_carried_application_lineage_sha256": (
            _sha256_text(
                ack_summary.get(
    ...
    for field in ACK_FINAL_LINEAGE_DIGEST_FIELDS:
        fields[f"{field_prefix}_{field}"] = _sha256_text(
            ack_summary.get(f"{source_prefix}_{field}", "")
        )
    return fields
...
def _broker_vendor_extended_complete_final_lineage_summary_fields(
    ...
    fields: dict[str, object] = {
        ...
        f"{prefix}_ack_extended_complete_final_review_carried_application_lineage_sha256": _sha256_text(
            ack_summary.get(
                f"{prefix}_carried_application_lineage_sha256",
                "",
            )
        ),
        f"{prefix}_roundtrip_extended_complete_final_review_carried_application_lineage_sha256": roundtrip_lineage_sha256,
    }
    for field in ACK_EXTENDED_COMPLETE_FINAL_LINEAGE_DIGEST_FIELDS:
        fields[f"{prefix}_{field}"] = _sha256_text(
            ack_summary.get(f"{prefix}_{field}", "")
...
def _broker_vendor_latest_extended_complete_final_lineage_40_summary_fields(
    ack_summary: pd.Series,
) -> dict[str, object]:
    prefix = ACK_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_40_FIELD_PREFIX
    roundtrip_lineage_sha256 = _target_application_lineage_sha256(
    ...
    fields: dict[str, object] = {
        ...
        f"{prefix}_ack_latest_extended_complete_final_review_carried_application_lineage_sha256": _sha256_text(
            ack_summary.get(
                f"{prefix}_carried_application_lineage_sha256",
                "",
            )
        ),
        f"{prefix}_roundtrip_latest_extended_complete_final_review_carried_application_lineage_sha256": roundtrip_lineage_sha256,
    }
    for field in ACK_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_40_DIGEST_FIELDS:
        fields[f"{prefix}_{field}"] = _sha256_text(
            ack_summary.get(f"{prefix}_{field}", "")
...
def _broker_vendor_current_latest_extended_complete_final_lineage_48_summary_fields(
    ack_summary: pd.Series,
) -> dict[str, object]:
    prefix = ACK_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_48_FIELD_PREFIX
    roundtrip_lineage_sha256 = _target_application_lineage_sha256(
    ...
    fields[
        ...
        ack_summary.get(f"{prefix}_carried_application_lineage_sha256", "")
    )
    fields[
...
def _broker_vendor_verified_reconciled_current_latest_extended_complete_final_lineage_64_summary_fields(
    ack_summary: pd.Series,
) -> dict[str, object]:
    prefix = ACK_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_64_FIELD_PREFIX
    roundtrip_lineage_sha256 = _target_application_lineage_sha256(
    ...
    fields: dict[str, object] = {
        ...
        f"{prefix}_roundtrip_verified_reconciled_current_latest_extended_complete_final_review_carried_application_lineage_sha256": roundtrip_lineage_sha256,
    }
    for field in (
        *ACK_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_64_DIGEST_FIELDS,
        *ACK_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_64_STAGE_FIELDS,
...
def _broker_vendor_confirmed_verified_reconciled_current_latest_extended_complete_final_lineage_72_summary_fields(
    ack_summary: pd.Series,
) -> dict[str, object]:
    prefix = ACK_CONFIRMED_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_72_FIELD_PREFIX
    roundtrip_lineage_sha256 = _target_application_lineage_sha256(
    ...
    fields: dict[str, object] = {
        ...
        f"{prefix}_roundtrip_confirmed_verified_reconciled_current_latest_extended_complete_final_review_carried_application_lineage_sha256": roundtrip_lineage_sha256,
    }
    for field in (
        *ACK_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_64_DIGEST_FIELDS,
        *ACK_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_64_STAGE_FIELDS,
...
def _broker_vendor_roundtrip_final_lineage_config(
    summary: pd.Series,
) -> dict[str, object]:
    field_prefix = ACK_FINAL_LINEAGE_FIELD_PREFIX
    config: dict[str, object] = {
        ...
        "ack_reconciliation_review_carried_application_lineage_sha256": _text(
            summary,
            f"{field_prefix}_ack_reconciliation_review_carried_application_lineage_sha256",
        ),
...
def _broker_vendor_roundtrip_complete_final_lineage_config(
    summary: pd.Series,
) -> dict[str, object]:
    prefix = ACK_COMPLETE_FINAL_LINEAGE_FIELD_PREFIX
    config: dict[str, object] = {
        ...
        "ack_complete_final_review_carried_application_lineage_sha256": _text(
            summary,
            f"{prefix}_ack_complete_final_review_carried_application_lineage_sha256",
        ),
        "carried_application_lineage_sha256": _text(
...
def _broker_vendor_roundtrip_extended_complete_final_lineage_config(
    summary: pd.Series,
) -> dict[str, object]:
    prefix = ACK_EXTENDED_COMPLETE_FINAL_LINEAGE_FIELD_PREFIX
    config: dict[str, object] = {
        ...
        "ack_extended_complete_final_review_carried_application_lineage_sha256": _text(
            summary,
            f"{prefix}_ack_extended_complete_final_review_carried_application_lineage_sha256",
        ),
        "carried_application_lineage_sha256": _text(
            summary,
            f"{prefix}_roundtrip_extended_complete_final_review_carried_application_lineage_sha256",
        ),
    }
    for field in ACK_EXTENDED_COMPLETE_FINAL_LINEAGE_DIGEST_FIELDS:
        config[field] = _text(summary, f"{prefix}_{field}")
...
def _broker_vendor_roundtrip_latest_extended_complete_final_lineage_41_config(
    summary: pd.Series,
) -> dict[str, object]:
    prefix = ACK_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_40_FIELD_PREFIX
    config: dict[str, object] = {
        ...
        "ack_latest_extended_complete_final_review_carried_application_lineage_sha256": _text(
            summary,
            f"{prefix}_ack_latest_extended_complete_final_review_carried_application_lineage_sha256",
        ),
        "carried_application_lineage_sha256": _text(
            summary,
            f"{prefix}_roundtrip_latest_extended_complete_final_review_carried_application_lineage_sha256",
        ),
    }
    for field in ACK_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_40_DIGEST_FIELDS:
        config[field] = _text(summary, f"{prefix}_{field}")
...
def _broker_vendor_roundtrip_current_latest_extended_complete_final_lineage_49_config(
    summary: pd.Series,
) -> dict[str, object]:
    prefix = ACK_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_48_FIELD_PREFIX
    config: dict[str, object] = {
...
def _broker_vendor_roundtrip_verified_reconciled_current_latest_extended_complete_final_lineage_65_config(
    summary: pd.Series,
) -> dict[str, object]:
    prefix = ACK_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_64_FIELD_PREFIX
    roundtrip_lineage_sha256 = _text(
        summary,
        f"{prefix}_roundtrip_verified_reconciled_current_latest_extended_complete_final_review_carried_application_lineage_sha256",
    )
    config: dict[str, object] = {
        "required": _to_bool(summary[f"{prefix}_lineage_match_required"]),
        "matches": _to_bool(summary[f"{prefix}_lineage_matches"]),
    ...
    config["carried_application_lineage_sha256"] = roundtrip_lineage_sha256
    return config


def _broker_vendor_roundtrip_confirmed_verified_reconciled_current_latest_extended_complete_final_lineage_73_config(
    summary: pd.Series,
) -> dict[str, object]:
    prefix = ACK_CONFIRMED_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_72_FIELD_PREFIX
    roundtrip_lineage_sha256 = _text(
        summary,
        f"{prefix}_roundtrip_confirmed_verified_reconciled_current_latest_extended_complete_final_review_carried_application_lineage_sha256",
    )
    config: dict[str, object] = {
        "required": _to_bool(summary[f"{prefix}_lineage_match_required"]),
        "matches": _to_bool(summary[f"{prefix}_lineage_matches"]),
...
def _config(
    ...
    return {
        ...
        ROUNDTRIP_FINAL_LINEAGE_COMPARISON_KEY: (
            _broker_vendor_roundtrip_final_lineage_config(summary)
        ),
        ROUNDTRIP_COMPLETE_FINAL_LINEAGE_COMPARISON_KEY: (
            _broker_vendor_roundtrip_complete_final_lineage_config(summary)
        ),
        ROUNDTRIP_EXTENDED_COMPLETE_FINAL_LINEAGE_COMPARISON_KEY: (
            _broker_vendor_roundtrip_extended_complete_final_lineage_config(
                summary
            )
        ),
        ROUNDTRIP_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_41_COMPARISON_KEY: (
            _broker_vendor_roundtrip_latest_extended_complete_final_lineage_41_config(
                summary
            )
        ),
        ROUNDTRIP_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_49_COMPARISON_KEY: (
        ...
        ROUNDTRIP_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_65_COMPARISON_KEY: (
            _broker_vendor_roundtrip_verified_reconciled_current_latest_extended_complete_final_lineage_65_config(
                summary
            )
        ),
        ROUNDTRIP_CONFIRMED_VERIFIED_RECONCILED_CURRENT_LATEST_EXTENDED_COMPLETE_FINAL_LINEAGE_73_COMPARISON_KEY: (
            _broker_vendor_roundtrip_confirmed_verified_reconciled_current_latest_extended_complete_final_lineage_73_config(
                summary
            )
        ),
...
def _runbook_markdown(summary_row: pd.Series, action_queue: pd.DataFrame) -> str:
    ...
    lines = [
        ...
        (
            "- Acknowledgement route contract identity active: "
            f"{'yes' if _to_bool(summary_row.get('broker_dispatch_ack_send_route_contract_identity_active')) else 'no'}"
        ),
        (
            "- Acknowledgement carried route contract identity: "
            f"{_code(summary_row.get('broker_dispatch_ack_broker_dispatch_send_broker_dispatch_route_enable_cutover_runtime_telemetry_broker_readiness_roundtrip_contract_identity_sha256'))}"
        ),
        (
            "- Current send route contract identity: "
            f"{_code(summary_row.get('broker_dispatch_ack_current_send_route_contract_identity_sha256'))}"
        ),
        (
            "- Acknowledgement route contract identity matches current: "
            f"{'yes' if _to_bool(summary_row.get('broker_dispatch_ack_send_route_contract_identity_matches_current')) else 'no'}"
        ),
        (
            "- Acknowledgement broker route contract identity active: "
            f"{'yes' if _to_bool(summary_row.get('broker_dispatch_ack_send_route_enable_route_contract_identity_active')) else 'no'}"
        ),
        (
            "- Acknowledgement carried broker route contract identity: "
            f"{_code(summary_row.get('broker_dispatch_ack_broker_dispatch_send_broker_dispatch_route_enable_cutover_runtime_telemetry_broker_readiness_route_contract_identity_sha256'))}"
        ),
        (
            "- Current send broker route contract identity: "
            f"{_code(summary_row.get('broker_dispatch_ack_current_send_route_enable_route_contract_identity_sha256'))}"
        ),
        (
            "- Acknowledgement broker route contract identity matches current: "
            f"{'yes' if _to_bool(summary_row.get('broker_dispatch_ack_send_route_enable_route_contract_identity_matches_current')) else 'no'}"
        ),
        (
            "- Acknowledgement broker route-enable route contract identity active: "
            f"{'yes' if _to_bool(summary_row.get('broker_dispatch_ack_send_route_enable_route_enable_route_contract_identity_active')) else 'no'}"
        ),
        (
            "- Acknowledgement carried broker route-enable route contract identity: "
            f"{_code(summary_row.get('broker_dispatch_ack_broker_dispatch_send_broker_dispatch_route_enable_cutover_runtime_telemetry_broker_readiness_route_enable_route_contract_identity_sha256'))}"
        ),
        (
            "- Current send broker route-enable route contract identity: "
            f"{_code(summary_row.get('broker_dispatch_ack_current_send_route_enable_route_enable_route_contract_identity_sha256'))}"
        ),
        (
        ...
        (
            ...
            f"{'yes' if _to_bool(summary_row.get('broker_dispatch_ack_broker_dispatch_send_broker_dispatch_route_enable_cutover_broker_readiness_matches_current')) else 'no'}"
        ),
        (
            "- Current broker-readiness manifest: "
            f"{_code(summary_row.get('broker_dispatch_ack_broker_dispatch_send_broker_dispatch_route_enable_cutover_current_broker_readiness_manifest_sha256'))}"
        ),
        "- Lead-lag acknowledgement contract consistent: "
        f"{'yes' if _to_bool(summary_row.get('strategy_portfolio_leadlag_ack_contract_consistent')) else 'no'}",
...
def _strategy_portfolio_active(*rows: pd.Series) -> bool:
    return any(
        ...
        or _strategy_portfolio_leadlag_active(row)
        for row in rows
    )


def _strategy_portfolio_leadlag_active(*rows: pd.Series) -> bool:
    return any(
        _to_bool(
            row.get(
                "strategy_portfolio_leadlag_edge_lineage_required",
...
def _strategy_portfolio_leadlag_roundtrip_state(
    ...
    ack_row: pd.Series,
    ack_config: dict[str, Any],
    ack_lineage: dict[str, Any],
) -> dict[str, Any]:
    dispatch = _strategy_portfolio_leadlag_component_state(
        dispatch_row,
        dispatch_config,
        STRATEGY_PORTFOLIO_LEADLAG_DISPATCH_FIELDS,
    )
    send = _strategy_portfolio_leadlag_component_state(
        send_row,
        send_config,
        STRATEGY_PORTFOLIO_LEADLAG_SEND_FIELDS,
    )
    ack = _strategy_portfolio_leadlag_component_state(
        ack_row,
        ack_config,
        STRATEGY_PORTFOLIO_LEADLAG_ACK_FIELDS,
    )
    lineage_values = _strategy_portfolio_leadlag_values(
        ack_lineage,
        prefix="strategy_portfolio_",
        fields=STRATEGY_PORTFOLIO_LEADLAG_ACK_FIELDS,
    )
    active = bool(
        dispatch["active"]
        or send["active"]
    ...
    source_profiles = (
        dispatch["summary_profile"],
        dispatch["config_profile"],
        send["summary_profile"],
        send["config_profile"],
        ack["summary_profile"],
        ack["config_profile"],
    )
    profiles_consistent = bool(
        not active
        or all(_identity_key(profile) == "leadlag" for profile in source_profiles)
    )
    dispatch_to_send_consistent = bool(
        not active
        or (
            dispatch["internally_consistent"]
            and send["internally_consistent"]
            and _strategy_portfolio_leadlag_values_match(
                dispatch["values"],
                send["values"],
                STRATEGY_PORTFOLIO_LEADLAG_DISPATCH_FIELDS,
            )
    ...
    send_to_ack_consistent = bool(
        not active
        or (
            dispatch_to_send_consistent
            and ack["internally_consistent"]
            and _strategy_portfolio_leadlag_values_match(
                send["values"],
                ack["values"],
                STRATEGY_PORTFOLIO_LEADLAG_SEND_FIELDS,
    ...
    lineage_has_contract = all(
        f"strategy_portfolio_{field}" in ack_lineage
        for field in STRATEGY_PORTFOLIO_LEADLAG_ACK_FIELDS
    )
    ack_contract_consistent = bool(
        not active
        or (
            profiles_consistent
            and send_to_ack_consistent
            and lineage_has_contract
            and _to_bool(ack_lineage.get("provided", False))
            and _to_bool(ack_lineage.get("gate_passed", False))
            and _strategy_portfolio_leadlag_values_match(
                ack["values"],
                lineage_values,
                STRATEGY_PORTFOLIO_LEADLAG_ACK_FIELDS,
    ...
    return {
        **{
            ...
            for field, value in ack["values"].items()
        },
        "strategy_portfolio_leadlag_ack_contract_consistent": (
            ack_contract_consistent
        ),
    }


def _strategy_portfolio_leadlag_component_state(
    row: pd.Series,
    config: dict[str, Any],
    fields: tuple[str, ...],
) -> dict[str, Any]:
    raw_portfolio = config.get("strategy_portfolio", {})
    portfolio = raw_portfolio if isinstance(raw_portfolio, dict) else {}
    summary_values = _strategy_portfolio_leadlag_values(
        row,
        prefix="strategy_portfolio_",
        fields=fields,
    )
    config_values = _strategy_portfolio_leadlag_values(
        portfolio,
        prefix="",
        fields=fields,
    )
    ...
    config_profile = _first_text(portfolio.get("selected_profile", ""))
    active = bool(
        _identity_key(summary_profile) == "leadlag"
        or _identity_key(config_profile) == "leadlag"
        or summary_values["leadlag_edge_lineage_required"]
        or config_values["leadlag_edge_lineage_required"]
    )
    summary_has_contract = all(
        f"strategy_portfolio_{field}" in row.index for field in fields
    )
    config_has_contract = all(field in portfolio for field in fields)
    internally_consistent = bool(
        not active
        or (
            summary_has_contract
            and config_has_contract
            and _identity_key(summary_profile) == _identity_key(config_profile)
            and _strategy_portfolio_leadlag_values_match(
                summary_values,
                config_values,
                fields,
...
def _strategy_portfolio_leadlag_values(
    source: Any,
    *,
    prefix: str,
    fields: tuple[str, ...],
    ...
    values = {
        "leadlag_edge_lineage_required": _to_bool(
            getter(f"{prefix}leadlag_edge_lineage_required", False)
        ),
        **leadlag_lineage_fields(source, source_prefix=prefix),
        "leadlag_edge_lineage_matches_scaleup": _to_bool(
            getter(f"{prefix}leadlag_edge_lineage_matches_scaleup", False)
        ),
        "leadlag_cutover_contract_consistent": _to_bool(
            getter(f"{prefix}leadlag_cutover_contract_consistent", False)
        ),
        "leadlag_route_contract_consistent": _to_bool(
            getter(f"{prefix}leadlag_route_contract_consistent", False)
        ),
        "leadlag_dispatch_contract_consistent": _to_bool(
            getter(f"{prefix}leadlag_dispatch_contract_consistent", False)
        ),
        "leadlag_send_contract_consistent": _to_bool(
            getter(f"{prefix}leadlag_send_contract_consistent", False)
        ),
    }
    return {field: values[field] for field in fields}


def _strategy_portfolio_leadlag_values_match(
    left: dict[str, Any],
    right: dict[str, Any],
    fields: tuple[str, ...],
) -> bool:
