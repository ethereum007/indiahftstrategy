    ...
    if route["route_enable_lineage_required"]:
        ...
        if route["route_enable_cutover_broker_readiness_required"]:
            ...
            if route[
                ...
                cutover_route_identity_sha256 = _object_text(
                    route[
                        (
                            "route_enable_cutover_runtime_telemetry_"
                ...
                current_cutover_route_identity_sha256 = _object_text(
                    route[
                        (
                ...
                checks.extend(
                    [
                        _check(
                            (
                                "route_enable_cutover_runtime_telemetry_"
                                "broker_readiness_route_contract_identity_"
                                "sha256_present"
                            ),
                            cutover_route_identity_sha256,
                            "present",
                            True,
                            bool(cutover_route_identity_sha256),
                            (
                                "route-enable broker route contract identity "
                                "digest is missing"
                            ),
                        ),
                        _check(
                            ...
                            cutover_route_identity_sha256,
                            "==",
                            current_cutover_route_identity_sha256,
                            bool(
                                cutover_route_identity_sha256
                                and current_cutover_route_identity_sha256
                                and cutover_route_identity_sha256
                                == current_cutover_route_identity_sha256
                            ),
                            (
                                "route-enable broker route contract identity "
                        ...
                        _check(
                            (
                                "route_enable_cutover_route_contract_identity_"
                                "matches_current"
                            ),
                            route[
                                (
                                    "route_enable_cutover_route_contract_"
                            ...
                            (
                                "route-enable broker route contract identity "
                                "no longer matches the current cutover source"
                            ),
                        ),
                    ]
                )
            if route[
                ...
                cutover_route_enable_identity_sha256 = _object_text(
                    route[
                        (
                            "route_enable_cutover_runtime_telemetry_"
                ...
                current_cutover_route_enable_identity_sha256 = _object_text(
                    route[
                        (
                ...
                checks.extend(
                    [
                        _check(
                            ...
                            cutover_route_enable_identity_sha256,
                            "present",
                            True,
                            bool(cutover_route_enable_identity_sha256),
                            (
                                "route-enable broker route-enable route "
                        ...
                        _check(
                            ...
                            cutover_route_enable_identity_sha256,
                            "==",
                            current_cutover_route_enable_identity_sha256,
                            bool(
                                cutover_route_enable_identity_sha256
                                and current_cutover_route_enable_identity_sha256
                                and (
                                    cutover_route_enable_identity_sha256
                                    == current_cutover_route_enable_identity_sha256
                                )
...
def _runbook_markdown(summary_row: pd.Series, action_queue: pd.DataFrame) -> str:
    ...
    lines = [
        ...
        (
            "- Route-enable broker route contract identity active: "
            f"{'yes' if _to_bool(summary_row.get('route_enable_cutover_route_contract_identity_active')) else 'no'}"
        ),
        (
            "- Route-enable carried broker route contract identity: "
            f"{_code(summary_row.get('route_enable_cutover_runtime_telemetry_broker_readiness_route_contract_identity_sha256'))}"
        ),
        (
            "- Current cutover broker route contract identity: "
            f"{_code(summary_row.get('route_enable_current_cutover_route_contract_identity_sha256'))}"
        ),
        (
            "- Route-enable broker route contract identity matches current: "
            f"{'yes' if _to_bool(summary_row.get('route_enable_cutover_route_contract_identity_matches_current')) else 'no'}"
        ),
        (
            "- Route-enable broker route-enable route contract identity active: "
            f"{'yes' if _to_bool(summary_row.get('route_enable_cutover_route_enable_route_contract_identity_active')) else 'no'}"
        ),
        (
            "- Route-enable carried broker route-enable route contract identity: "
            f"{_code(summary_row.get('route_enable_cutover_runtime_telemetry_broker_readiness_route_enable_route_contract_identity_sha256'))}"
        ),
        (
            "- Current cutover broker route-enable route contract identity: "
            f"{_code(summary_row.get('route_enable_current_cutover_route_enable_route_contract_identity_sha256'))}"
        ),
        (
