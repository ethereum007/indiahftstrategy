    ...
    *,
    ...
    write_experiment_manifest(
        ...
        extra={
            ...
            "broker_dispatch_send_ready": bool(summary_row["broker_dispatch_send_ready"]),
            "profile": PROFILE,
...
def _prechecks(
    ...
    return pd.DataFrame(
        [
            ...
            _check(
                "provider_broker_dispatch_manifest_readable",
                provider_manifest_error or "ok",
                "is",
                "ok",
                not provider_manifest_error,
                provider_manifest_error or "provider imbalance broker-dispatch manifest could not be read",
            ),
            _check(
                "provider_broker_dispatch_manifest_type",
                _clean(provider_manifest.get("run_type")),
                "is",
                "provider_market_data_imbalance_broker_dispatch",
                _clean(provider_manifest.get("run_type"))
                == "provider_market_data_imbalance_broker_dispatch",
                "provider imbalance broker-dispatch manifest run_type is not expected",
            ),
            _check(
            ...
            _check(
                "provider_broker_dispatch_adapter_receipt_proof_carried",
                ...
                "provider imbalance broker-dispatch is missing ready adapter receipt proof",
            ),
            _check(
                "provider_broker_dispatch_adapter_receipt_proof_matches_manifest",
                receipt_proofs_match,
                "is",
                True,
                receipt_proofs_match if bundle_provided else True,
                "adapter receipt proof differs between broker-dispatch config and manifest",
            ),
            _check(
                "provider_broker_dispatch_adapter_receipts_valid",
                ...
                "provider imbalance broker-dispatch did not preserve valid required adapter receipts",
            ),
            _check(
                "provider_broker_dispatch_adapter_receipt_fingerprints_current",
                ...
                "adapter receipt files changed after provider broker-dispatch planning",
            ),
            _check(
                "provider_broker_dispatch_capture_fingerprints_current",
                ...
                "provider capture files changed after provider broker-dispatch planning",
            ),
            _check(
                "provider_broker_dispatch_dispatch_roundtrip_adapter_receipt_proof_carried",
                ...
                "provider broker-dispatch is missing final round-trip adapter receipt proof",
            ),
            _check(
                "provider_broker_dispatch_dispatch_roundtrip_adapter_receipt_proof_matches_manifest",
                ...
                "final round-trip adapter receipt proof differs between broker-dispatch config and manifest",
            ),
            _check(
                "provider_broker_dispatch_dispatch_roundtrip_adapter_receipt_proof_matches_runtime",
                ...
                "final round-trip adapter receipt proof differs from broker-dispatch runtime proof",
            ),
            _check(
                "provider_broker_dispatch_dispatch_roundtrip_adapter_receipts_valid",
                ...
                "provider broker-dispatch did not preserve valid final round-trip adapter receipts",
            ),
            _check(
                "provider_broker_dispatch_dispatch_roundtrip_adapter_receipt_fingerprints_current",
                ...
                "final round-trip adapter receipt files changed after provider broker-dispatch",
            ),
            _check(
                "provider_broker_dispatch_dispatch_roundtrip_capture_fingerprints_current",
                dispatch_roundtrip_receipt_status[
                ...
                "final round-trip provider capture files changed after provider broker-dispatch",
            ),
            _check(
...
def _checks(
    ...
    rows.append(
        _check(
            "provider_broker_dispatch_provider_capture_commands_carried",
            ...
            "provider imbalance broker-dispatch is missing capture-bundle provider command proof",
        )
    )
    rows.append(
        _check(
            "provider_broker_dispatch_provider_capture_commands_match_session",
            bundle_provider_capture_command_count,
            "matches",
            provider_capture_command_count,
            bundle_provider_capture_commands_match_session if bundle_provided else True,
            "provider imbalance broker-dispatch command proof no longer matches the session packet",
        )
    )
    rows.append(
        _check(
            "provider_broker_dispatch_adapter_execution_contract_carried",
            _adapter_contract_metadata_text(provider_summary),
            "is_not",
            "",
            adapter_contract_carried if bundle_provided else True,
            "provider imbalance broker-dispatch is missing credential-safe adapter execution contract metadata",
        )
    )
    rows.append(
        _check(
            "provider_broker_dispatch_adapter_execution_contract_matches_evidence",
            _adapter_contract_metadata_text(provider_summary),
            "matches",
            "live evidence",
            ...
            "provider imbalance broker-dispatch adapter execution contract no longer matches live evidence",
        )
    )
    rows.append(
        _check(
            "provider_broker_dispatch_provider_profile_carried",
            _first_text(provider_summary, "provider_profile_sha256"),
            "has",
            "provider profile",
            provider_profile_carried,
            "provider imbalance broker-dispatch is missing provider-profile proof",
        )
    )
    rows.append(
        _check(
            "provider_broker_dispatch_provider_profile_matches_session",
            _first_text(provider_summary, "provider_profile_sha256"),
            "matches",
            "live session",
            _first_bool(provider_summary, "provider_profile_matches_session"),
            "provider imbalance broker-dispatch provider-profile proof no longer matches the live session packet",
        )
    )
    rows.append(
        _check(
            "provider_broker_dispatch_provider_profile_matches_bundle",
            ...
            "provider imbalance broker-dispatch provider-profile proof no longer matches the capture bundle",
        )
    )
    rows.append(
        _check(
            "provider_broker_dispatch_adapter_provider_profile_matches_evidence",
            ...
            "provider imbalance broker-dispatch adapter contract provider-profile SHA no longer matches live evidence",
        )
    )
    rows.append(
        _check(
            "provider_broker_dispatch_synthetic_sidecar_proof_carried",
            ...
            "provider imbalance broker-dispatch is missing synthetic rehearsal sidecar proof",
        )
    )
    rows.append(
        _check(
            "provider_broker_dispatch_synthetic_sidecar_proof_ready",
            synthetic_sidecar_proof_ready,
            "is",
            True,
            synthetic_sidecar_proof_ready if synthetic_sidecar_proof_required else True,
            "provider imbalance broker-dispatch synthetic rehearsal sidecar proof is not ready",
        )
    )
    rows.append(
        _check(
            "provider_broker_dispatch_route_readiness_provider_sidecar_breach_pairs",
    ...
    rows.append(
        _check(
            ...
            "provider imbalance broker-dispatch-send is missing broker-dispatch round-trip provider command proof",
        )
    )
    rows.append(
        _check(
            ...
            "provider imbalance broker-dispatch-send round-trip command proof no longer matches the session packet",
        )
    )
    rows.append(
        _check(
            ...
            "provider imbalance broker-dispatch-send round-trip command proof no longer matches runtime-session proof",
        )
    )
    rows.append(
        _check(
            "provider_broker_dispatch_dispatch_roundtrip_adapter_execution_contract_carried",
            dispatch_adapter_contract_metadata,
            "is_not",
            "",
            dispatch_adapter_contract_carried if dispatch_bundle_provided else True,
            "provider imbalance broker-dispatch-send is missing broker-dispatch round-trip adapter execution contract proof",
        )
    )
    rows.append(
        _check(
            ...
            "provider imbalance broker-dispatch-send round-trip adapter execution contract no longer matches live evidence",
        )
    )
    rows.append(
        _check(
            ...
            "provider imbalance broker-dispatch-send round-trip adapter execution contract no longer matches runtime-session proof",
        )
    )
    rows.append(
        _check(
            "provider_broker_dispatch_dispatch_roundtrip_provider_profile_carried",
            dispatch_provider_profile_metadata,
            "is_not",
            "",
            dispatch_provider_profile_carried if dispatch_bundle_provided else True,
            "provider imbalance broker-dispatch-send is missing broker-dispatch round-trip provider-profile proof",
        )
    )
    rows.append(
        _check(
            "provider_broker_dispatch_dispatch_roundtrip_provider_profile_matches_session",
            ...
            "provider imbalance broker-dispatch-send round-trip provider-profile proof no longer matches live session",
        )
    )
    rows.append(
        _check(
            "provider_broker_dispatch_dispatch_roundtrip_provider_profile_matches_bundle",
            ...
            "provider imbalance broker-dispatch-send round-trip provider-profile proof no longer matches capture bundle",
        )
    )
    rows.append(
        _check(
            ...
            "provider imbalance broker-dispatch-send round-trip adapter contract provider-profile SHA no longer matches evidence",
        )
    )
    rows.append(
        _check(
            ...
            "provider imbalance broker-dispatch-send round-trip provider profile no longer matches runtime-session proof",
        )
    )
    rows.append(
        _check(
            "provider_broker_dispatch_dispatch_roundtrip_synthetic_sidecar_proof_carried",
            ...
            "provider imbalance broker-dispatch-send is missing broker-dispatch round-trip synthetic rehearsal sidecar proof",
        )
    )
    rows.append(
        _check(
            "provider_broker_dispatch_dispatch_roundtrip_synthetic_sidecar_proof_ready",
            ...
            "provider imbalance broker-dispatch-send broker-dispatch round-trip synthetic rehearsal sidecar proof is not ready",
        )
    )
    rows.append(
        _check(
            "provider_broker_dispatch_dispatch_roundtrip_route_readiness_provider_sidecar_breach_pairs",
            dispatch_route_sidecar_breach_pairs,
            "<=",
            0,
            dispatch_route_sidecar_breach_pairs <= 0 if dispatch_route_sidecar_gate_active else True,
            (
                "provider imbalance broker-dispatch-send carries breached "
                "broker-dispatch round-trip route-readiness sidecar proof"
            ),
        )
    )
        ...
        [
            {
                ...
                "provider_broker_dispatch_manifest_run_type": _clean(
...
def _runbook_markdown(summary: pd.Series, checks: pd.DataFrame, action_queue: pd.DataFrame) -> str:
    dispatch_roundtrip_receipt_line = (
        ...
        if not bool(summary["provider_broker_dispatch_roundtrip_wrapper_provided"])
            ...
            if not bool(summary["dispatch_roundtrip_adapter_receipts_required"])
                ...
                "broker-dispatch manifest match: "
    ...
    lines = [
        ...
        f"- Adapter receipt proof: {'ready' if bool(summary['adapter_receipt_proof_ready']) else 'blocked'} ({summary['adapter_receipt_fingerprint_match_count']}/{summary['adapter_receipt_required_count']} sealed; broker-dispatch manifest match: {'yes' if bool(summary['adapter_receipt_proof_matches_manifest']) else 'no'})",
        dispatch_roundtrip_receipt_line,
...
def _recommendation_for_check(check: str) -> str:
    ...
    if check.startswith("provider_broker_dispatch_route_readiness_provider_sidecar"):
        return "review_provider_route_readiness_sidecar_proof_before_broker_dispatch_send"
    if check.startswith("provider_broker_dispatch"):
...
def _inferred_upstream_dispatch_roundtrip_dirs(
    ...
    provider_dispatch_roundtrip_dir = _first_existing_path(
        ...
        _path_from_text(broker_dispatch_inputs.get("upstream_provider_dispatch_roundtrip_dir")),
    )
    dispatch_roundtrip_dir = _first_existing_path(
        _path_from_text(_first_text(provider_summary, "upstream_dispatch_roundtrip_dir")),
        _path_from_text(broker_dispatch_inputs.get("upstream_dispatch_roundtrip_dir")),
    )
    return provider_dispatch_roundtrip_dir, dispatch_roundtrip_dir
