...
def _prechecks(
    ...
    return pd.DataFrame(
        [
            ...
            _check(
                "provider_broker_dispatch_send_dispatch_roundtrip_adapter_receipt_proof_carried",
                ...
                "provider broker-dispatch-send is missing final round-trip adapter receipt proof",
            ),
            _check(
                "provider_broker_dispatch_send_dispatch_roundtrip_adapter_receipt_proof_matches_manifest",
                ...
                "final round-trip adapter receipt proof differs between send config and manifest",
            ),
            _check(
                "provider_broker_dispatch_send_dispatch_roundtrip_adapter_receipt_proof_matches_runtime",
                ...
                "final round-trip adapter receipt proof differs from send runtime proof",
            ),
            _check(
                "provider_broker_dispatch_send_dispatch_roundtrip_adapter_receipts_valid",
                ...
                "provider broker-dispatch-send did not preserve valid final round-trip adapter receipts",
            ),
            _check(
                "provider_broker_dispatch_send_dispatch_roundtrip_adapter_receipt_fingerprints_current",
                ...
                "final round-trip adapter receipt files changed after provider broker-dispatch-send",
            ),
            _check(
                "provider_broker_dispatch_send_dispatch_roundtrip_capture_fingerprints_current",
                dispatch_roundtrip_receipt_status[
                ...
                "final round-trip provider capture files changed after provider broker-dispatch-send",
            ),
            _check(
...
def _checks(
    ...
    rows.append(
        _check(
            "provider_broker_dispatch_send_provider_capture_commands_carried",
            ...
            "provider imbalance broker-dispatch-send is missing capture-bundle provider command proof",
        )
    )
    rows.append(
        _check(
            "provider_broker_dispatch_send_provider_capture_commands_match_session",
            bundle_provider_capture_command_count,
            "matches",
            provider_capture_command_count,
            bundle_provider_capture_commands_match_session if bundle_provided else True,
            "provider imbalance broker-dispatch-send command proof no longer matches the session packet",
        )
    )
    rows.append(
        _check(
            "provider_broker_dispatch_send_adapter_execution_contract_carried",
            _adapter_contract_metadata_text(provider_summary),
            "is_not",
            "",
            adapter_contract_carried if bundle_provided else True,
            "provider imbalance broker-dispatch-send is missing credential-safe adapter execution contract metadata",
        )
    )
    rows.append(
        _check(
            "provider_broker_dispatch_send_adapter_execution_contract_matches_evidence",
            _adapter_contract_metadata_text(provider_summary),
            "matches",
            "live evidence",
            ...
            "provider imbalance broker-dispatch-send adapter execution contract no longer matches live evidence",
        )
    )
    rows.append(
        _check(
            "provider_broker_dispatch_send_provider_profile_carried",
            _first_text(provider_summary, "provider_profile_sha256"),
            "has",
            "provider profile",
            provider_profile_carried,
            "provider imbalance broker-dispatch-send is missing provider-profile proof",
        )
    )
    rows.append(
        _check(
            "provider_broker_dispatch_send_provider_profile_matches_session",
            _first_text(provider_summary, "provider_profile_sha256"),
            "matches",
            "live session",
            _first_bool(provider_summary, "provider_profile_matches_session"),
            "provider imbalance broker-dispatch-send provider-profile proof no longer matches the live session packet",
        )
    )
    rows.append(
        _check(
            "provider_broker_dispatch_send_provider_profile_matches_bundle",
            ...
            "provider imbalance broker-dispatch-send provider-profile proof no longer matches the capture bundle",
        )
    )
    rows.append(
        _check(
            "provider_broker_dispatch_send_adapter_provider_profile_matches_evidence",
            ...
            "provider imbalance broker-dispatch-send adapter contract provider-profile SHA no longer matches live evidence",
        )
    )
    rows.append(
        _check(
            "provider_broker_dispatch_send_synthetic_sidecar_proof_carried",
            ...
            "provider imbalance broker-dispatch-send is missing synthetic rehearsal sidecar proof",
        )
    )
    rows.append(
        _check(
            "provider_broker_dispatch_send_synthetic_sidecar_proof_ready",
            synthetic_sidecar_proof_ready,
            "is",
            True,
            synthetic_sidecar_proof_ready if synthetic_sidecar_proof_required else True,
            "provider imbalance broker-dispatch-send synthetic rehearsal sidecar proof is not ready",
        )
    )
    rows.append(
        _check(
            "provider_broker_dispatch_send_route_readiness_provider_sidecar_breach_pairs",
            route_sidecar_breach_pairs,
            "<=",
            0,
            route_sidecar_breach_pairs <= 0 if route_sidecar_gate_active else True,
            (
                "provider imbalance broker-dispatch-send carries breached route-readiness "
    ...
    rows.append(
        _check(
            ...
            "provider broker-dispatch-send does not carry a complete active-lineage selection contract",
        )
    )
    rows.append(
        _check(
            ...
            "provider broker-dispatch-send lineage selection contract differs across summary, config, and manifest",
        )
    )
    ...
    rows.append(
        _check(
            ...
            "provider imbalance broker-dispatch-ack is missing send-retained round-trip provider command proof",
        )
    )
    rows.append(
        _check(
            ...
            "provider imbalance broker-dispatch-ack round-trip command proof no longer matches the session packet",
        )
    )
    rows.append(
        _check(
            ...
            "provider imbalance broker-dispatch-ack round-trip command proof no longer matches runtime-session proof",
        )
    )
    rows.append(
        _check(
            ...
            "provider imbalance broker-dispatch-ack is missing send-retained round-trip adapter execution contract proof",
        )
    )
    rows.append(
        _check(
            ...
            "provider imbalance broker-dispatch-ack round-trip adapter execution contract no longer matches live evidence",
        )
    )
    rows.append(
        _check(
            ...
            "provider imbalance broker-dispatch-ack round-trip adapter execution contract no longer matches runtime-session proof",
        )
    )
    rows.append(
        _check(
            "provider_broker_dispatch_send_dispatch_roundtrip_provider_profile_carried",
            dispatch_provider_profile_metadata,
            "is_not",
            "",
            dispatch_provider_profile_carried if dispatch_bundle_provided else True,
            "provider imbalance broker-dispatch-ack is missing send-retained round-trip provider-profile proof",
        )
    )
    rows.append(
        _check(
            "provider_broker_dispatch_send_dispatch_roundtrip_provider_profile_matches_session",
            ...
            "provider imbalance broker-dispatch-ack round-trip provider-profile proof no longer matches live session",
        )
    )
    rows.append(
        _check(
            "provider_broker_dispatch_send_dispatch_roundtrip_provider_profile_matches_bundle",
            ...
            "provider imbalance broker-dispatch-ack round-trip provider-profile proof no longer matches capture bundle",
        )
    )
    rows.append(
        _check(
            ...
            "provider imbalance broker-dispatch-ack round-trip adapter contract provider-profile SHA no longer matches evidence",
        )
    )
    rows.append(
        _check(
            ...
            "provider imbalance broker-dispatch-ack round-trip provider profile no longer matches runtime-session proof",
        )
    )
    rows.append(
        _check(
            "provider_broker_dispatch_send_dispatch_roundtrip_synthetic_sidecar_proof_carried",
            ...
            "provider imbalance broker-dispatch-ack is missing send-retained round-trip synthetic rehearsal sidecar proof",
        )
    )
    rows.append(
        _check(
            "provider_broker_dispatch_send_dispatch_roundtrip_synthetic_sidecar_proof_ready",
            ...
            "provider imbalance broker-dispatch-ack send-retained round-trip synthetic rehearsal sidecar proof is not ready",
        )
    )
    rows.append(
        _check(
            "provider_broker_dispatch_send_dispatch_roundtrip_route_readiness_provider_sidecar_breach_pairs",
            dispatch_route_sidecar_breach_pairs,
            "<=",
            0,
            dispatch_route_sidecar_breach_pairs <= 0 if dispatch_route_sidecar_gate_active else True,
            (
                "provider imbalance broker-dispatch-ack carries breached "
                "broker-dispatch-send round-trip route-readiness sidecar proof"
            ),
        )
    )
...
def _next_gate_for_check(check: str, broker_dispatch_ack: BrokerDispatchAckReport | None) -> str:
    if check.startswith("provider_broker_dispatch_send_route_readiness_provider_lineage"):
        return "review-provider-market-data-imbalance-route-readiness"
    if check.startswith("provider_broker_dispatch_send_dispatch_roundtrip_route_readiness_provider_sidecar"):
        return "review-provider-market-data-imbalance-route-readiness"
    if check.startswith("provider_broker_dispatch_send_route_readiness_provider_sidecar"):
        return "review-provider-market-data-imbalance-route-readiness"
...
def _recommendation_for_check(check: str) -> str:
    if check.startswith("provider_broker_dispatch_send_route_readiness_provider_lineage"):
        return "review_provider_lineage_selection_contract_before_broker_dispatch_ack"
    if check.startswith("provider_broker_dispatch_send_dispatch_roundtrip_route_readiness_provider_sidecar"):
        return "review_provider_roundtrip_route_readiness_sidecar_proof_before_broker_dispatch_ack"
    if check.startswith("provider_broker_dispatch_send_route_readiness_provider_sidecar"):
        return "review_provider_route_readiness_sidecar_proof_before_broker_dispatch_ack"
    if check.startswith("provider_broker_dispatch_send"):
