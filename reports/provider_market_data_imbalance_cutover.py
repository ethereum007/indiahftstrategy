...
def _prechecks(
    ...
    return pd.DataFrame(
        [
            ...
            _check(
                "provider_broker_readiness_manifest_readable",
                provider_manifest_error or "ok",
                "is",
                "ok",
                not provider_manifest_error,
                provider_manifest_error or "provider imbalance broker-readiness manifest could not be read",
            ),
            _check(
                "provider_broker_readiness_manifest_type",
                _clean(provider_manifest.get("run_type")),
                "is",
                "provider_market_data_imbalance_broker_readiness",
                _clean(provider_manifest.get("run_type"))
                == "provider_market_data_imbalance_broker_readiness",
                "provider imbalance broker-readiness manifest run_type is not expected",
            ),
            _check(
            ...
            _check(
                "provider_broker_readiness_adapter_receipt_proof_carried",
                ...
                "provider imbalance broker readiness is missing ready adapter receipt proof",
            ),
            _check(
                "provider_broker_readiness_adapter_receipt_proof_matches_manifest",
                receipt_proofs_match,
                "is",
                True,
                receipt_proofs_match if bundle_provided else True,
                "adapter receipt proof differs between broker-readiness config and manifest",
            ),
            _check(
                "provider_broker_readiness_adapter_receipts_valid",
                ...
                "provider imbalance broker readiness did not preserve valid required adapter receipts",
            ),
            _check(
                "provider_broker_readiness_adapter_receipt_fingerprints_current",
                ...
                "adapter receipt files changed after provider broker-readiness review",
            ),
            _check(
                "provider_broker_readiness_capture_fingerprints_current",
                ...
                "provider capture files changed after provider broker-readiness review",
            ),
            _check(
                "provider_broker_readiness_dispatch_roundtrip_adapter_receipt_proof_carried",
                ...
                "provider broker readiness is missing final round-trip adapter receipt proof",
            ),
            _check(
                "provider_broker_readiness_dispatch_roundtrip_adapter_receipt_proof_matches_manifest",
                ...
                "final round-trip adapter receipt proof differs between broker-readiness config and manifest",
            ),
            _check(
                "provider_broker_readiness_dispatch_roundtrip_adapter_receipt_proof_matches_runtime",
                ...
                "final round-trip adapter receipt proof differs from broker-readiness runtime proof",
            ),
            _check(
                "provider_broker_readiness_dispatch_roundtrip_adapter_receipts_valid",
                ...
                "provider broker readiness did not preserve valid final round-trip adapter receipts",
            ),
            _check(
                "provider_broker_readiness_dispatch_roundtrip_adapter_receipt_fingerprints_current",
                ...
                "final round-trip adapter receipt files changed after provider broker readiness",
            ),
            _check(
                "provider_broker_readiness_dispatch_roundtrip_capture_fingerprints_current",
                dispatch_roundtrip_receipt_status[
                ...
                "final round-trip provider capture files changed after provider broker readiness",
            ),
            _check(
...
def _checks(
    ...
    rows.append(
        _check(
            ...
            "provider broker-readiness does not carry a complete active-lineage selection contract",
        )
    )
    rows.append(
        _check(
            ...
            "provider broker-readiness lineage selection contract differs across summary, config, and manifest",
        )
    )
    dispatch_roundtrip = _dispatch_roundtrip_provenance(provider_config)
        ...
        [
            {
                ...
                "provider_broker_readiness_manifest_run_type": _clean(
...
def _runbook_markdown(summary: pd.Series, checks: pd.DataFrame, action_queue: pd.DataFrame) -> str:
    dispatch_roundtrip_receipt_line = (
        ...
        if not bool(summary["provider_broker_dispatch_roundtrip_wrapper_provided"])
            ...
            if not bool(summary["dispatch_roundtrip_adapter_receipts_required"])
                ...
                "broker-readiness manifest match: "
    ...
    lines = [
        ...
        f"- Adapter receipt proof: {'ready' if bool(summary['adapter_receipt_proof_ready']) else 'blocked'} ({summary['adapter_receipt_fingerprint_match_count']}/{summary['adapter_receipt_required_count']} sealed; broker-readiness manifest match: {'yes' if bool(summary['adapter_receipt_proof_matches_manifest']) else 'no'})",
        dispatch_roundtrip_receipt_line,
...
def _next_gate_for_check(check: str, cutover: CutoverGateReport | None) -> str:
    if check.startswith("provider_broker_readiness_route_readiness_provider_lineage"):
...
def _recommendation_for_check(check: str) -> str:
    if check.startswith("provider_broker_readiness_route_readiness_provider_lineage"):
        return "review_provider_lineage_selection_contract_before_cutover"
    if _is_dispatch_roundtrip_route_sidecar_check(check):
