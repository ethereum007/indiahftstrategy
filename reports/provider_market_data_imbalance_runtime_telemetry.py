...
def _prechecks(
    ...
    return pd.DataFrame(
        [
            ...
            _check(
                "provider_scaleup_manifest_readable",
                provider_manifest_error or "ok",
                "is",
                "ok",
                not provider_manifest_error,
                provider_manifest_error or "provider imbalance scale-up manifest could not be read",
            ),
            _check(
                "provider_scaleup_manifest_type",
                _text(provider_manifest.get("run_type")),
                "is",
                "provider_market_data_imbalance_scaleup_plan",
                _text(provider_manifest.get("run_type"))
                == "provider_market_data_imbalance_scaleup_plan",
                "provider imbalance scale-up manifest run_type is not expected",
            ),
            _check(
            ...
            _check(
                "provider_scaleup_adapter_receipt_proof_carried",
                ...
                "provider imbalance scale-up is missing ready adapter receipt proof",
            ),
            _check(
                "provider_scaleup_adapter_receipt_proof_matches_manifest",
                receipt_proofs_match,
                "is",
                True,
                receipt_proofs_match if bundle_provided else True,
                "adapter receipt proof differs between scale-up config and manifest",
            ),
            _check(
                "provider_scaleup_adapter_receipts_valid",
                ...
                "provider imbalance scale-up did not preserve valid required adapter receipts",
            ),
            _check(
                "provider_scaleup_adapter_receipt_fingerprints_current",
                ...
                "adapter receipt files changed after provider scale-up planning",
            ),
            _check(
                "provider_scaleup_capture_fingerprints_current",
                ...
                "provider capture files changed after provider scale-up planning",
            ),
            _check(
...
def _checks(
    ...
    rows.append(
        _check(
            "provider_scaleup_adapter_execution_contract_carried",
            _adapter_contract_metadata_text(provider_summary),
            "is_not",
            "",
            adapter_contract_carried if bundle_provided else True,
            "provider imbalance scale-up is missing credential-safe adapter execution contract proof",
        )
    )
    rows.append(
        _check(
            "provider_scaleup_adapter_execution_contract_matches_evidence",
            _adapter_contract_metadata_text(provider_summary),
            "matches",
            "live evidence",
            _first_bool(provider_summary, "adapter_contract_metadata_matches_evidence") if bundle_provided else True,
            "provider imbalance scale-up adapter execution contract no longer matches live evidence",
        )
    )
    rows.append(
        _check(
            "provider_scaleup_provider_profile_carried",
            _first_text(provider_summary, "provider_profile_sha256"),
            "has",
            "provider profile",
            provider_profile_carried,
            "provider imbalance scale-up is missing provider-profile proof",
        )
    )
    rows.append(
        _check(
            "provider_scaleup_provider_profile_matches_session",
            _first_text(provider_summary, "provider_profile_sha256"),
            "matches",
            "live session",
            _first_bool(provider_summary, "provider_profile_matches_session"),
            "provider imbalance scale-up provider-profile proof no longer matches the live session packet",
        )
    )
    rows.append(
        _check(
            "provider_scaleup_provider_profile_matches_bundle",
            ...
            "provider imbalance scale-up provider-profile proof no longer matches the capture bundle",
        )
    )
    rows.append(
        _check(
            "provider_scaleup_adapter_provider_profile_matches_evidence",
            ...
            "provider imbalance scale-up adapter contract provider-profile SHA no longer matches live evidence",
        )
    )
    rows.append(
        _check(
            "provider_scaleup_synthetic_sidecar_proof_carried",
            sidecar_proof_count,
            "==",
            synthetic_dataset_count,
            sidecar_proof_count_matches if sidecar_proof_required else True,
            "provider imbalance scale-up synthetic folds are missing rehearsal sidecar proof",
        )
    )
    rows.append(
        _check(
            "provider_scaleup_synthetic_sidecar_proof_ready",
            sidecar_proof_ready,
            "is",
            True,
        ...
        [
            {
                ...
                "provider_scaleup_manifest_run_type": _text(
...
def _config(
    ...
    return {
        ...
        "provider_scaleup_manifest_run_type": _text(provider_manifest.get("run_type")),
        "runtime_telemetry": {
