...
def _checks(
    ...
    rows.append(
        _check(
            ...
            "provider broker-dispatch-ack does not carry a complete active-lineage selection contract",
        )
    )
    rows.append(
        _check(
            ...
            "provider broker-dispatch-ack lineage selection contract differs across summary, config, and manifest",
        )
    )
...
def _next_gate_for_check(
    ...
    if check.startswith("provider_broker_dispatch_ack_route_readiness_provider_lineage"):
...
def _recommendation_for_check(check: str) -> str:
    if check.startswith("provider_broker_dispatch_ack_route_readiness_provider_lineage"):
        return "review_provider_lineage_selection_contract_before_broker_dispatch_roundtrip"
    if check.startswith("provider_broker_dispatch_ack_dispatch_roundtrip_route_readiness_provider_sidecar"):
