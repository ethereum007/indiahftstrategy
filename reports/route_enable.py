    ...
    if cutover["cutover_lineage_required"]:
        ...
        if cutover["cutover_broker_readiness_required"]:
            ...
            if cutover[
                ...
                runtime_route_enable_route_enable_identity_sha256 = str(
                    cutover[
                        (
                            "cutover_runtime_telemetry_broker_readiness_"
                ...
                current_route_enable_route_enable_identity_sha256 = str(
                    cutover[
                        (
                ...
                checks.extend(
                    [
                        _check(
                            ...
                            runtime_route_enable_route_enable_identity_sha256,
                            "present",
                            True,
                            bool(
                                runtime_route_enable_route_enable_identity_sha256
                            ...
                            (
                                "cutover runtime route-enable route-enable "
                        ...
                        _check(
                            ...
                            (
                                "cutover runtime route-enable route-enable "
                                "route contract identity differs from current "
                                "broker readiness"
                            ),
                        ),
                        _check(
                            ...
                            (
                                "cutover route-enable route-enable route "
                                "contract identity no longer matches the "
            ...
            if cutover[
                ...
                runtime_route_enable_route_enable_route_enable_identity_sha256 = str(
                    cutover[
                        (
                            "cutover_runtime_telemetry_broker_readiness_"
                ...
                current_route_enable_route_enable_route_enable_identity_sha256 = str(
                    cutover[
                        (
                            "cutover_current_runtime_route_enable_route_enable_"
                            "route_enable_route_contract_identity_sha256"
                ...
                checks.extend(
                    [
                        _check(
                            ...
                            runtime_route_enable_route_enable_route_enable_identity_sha256,
                            "present",
                            True,
                            bool(
                                runtime_route_enable_route_enable_route_enable_identity_sha256
                        ...
                        _check(
                            ...
                            (
                                "cutover route-enable route-enable "
                                "route-enable route contract identity no "
                                "longer matches the current recursive "
                                "broker-readiness source"
                            ),
                        ),
                    ]
...
def _component(check: str) -> str:
    ...
    if (
        ...
        or (
            "runtime_route_enable_route_enable_route_contract_identity"
            in check
        )
        or (
...
def _runbook_markdown(summary_row: pd.Series, action_queue: pd.DataFrame) -> str:
    ...
    lines = [
        ...
        f"- Broker route-enable route-enable route contract identity active: {'yes' if _to_bool(summary_row.get('cutover_runtime_route_enable_route_enable_route_contract_identity_active')) else 'no'}",
        f"- Broker route-enable route-enable route contract identity digest: {_code(summary_row.get('cutover_runtime_telemetry_broker_readiness_route_enable_route_enable_route_contract_identity_sha256'))}",
        f"- Current broker route-enable route-enable route contract identity digest: {_code(summary_row.get('cutover_current_runtime_route_enable_route_enable_route_contract_identity_sha256'))}",
        f"- Broker route-enable route-enable route contract identity matches current: {'yes' if _to_bool(summary_row.get('cutover_runtime_route_enable_route_enable_route_contract_identity_matches_current')) else 'no'}",
        f"- Broker route-enable route-enable route-enable route contract identity active: {'yes' if _to_bool(summary_row.get('cutover_runtime_route_enable_route_enable_route_enable_route_contract_identity_active')) else 'no'}",
        f"- Broker route-enable route-enable route-enable route contract identity digest: {_code(summary_row.get('cutover_runtime_telemetry_broker_readiness_route_enable_route_enable_route_enable_route_contract_identity_sha256'))}",
        f"- Current broker route-enable route-enable route-enable route contract identity digest: {_code(summary_row.get('cutover_current_runtime_route_enable_route_enable_route_enable_route_contract_identity_sha256'))}",
        f"- Broker route-enable route-enable route-enable route contract identity matches current: {'yes' if _to_bool(summary_row.get('cutover_runtime_route_enable_route_enable_route_enable_route_contract_identity_matches_current')) else 'no'}",
