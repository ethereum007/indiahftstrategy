...
class ImbalanceLaunchPipelineConfig:
...
@dataclass(frozen=True)
class ImbalanceLaunchPipelineReport:
    order_plan: ImbalanceOrderPlanReport | None
...
def write_imbalance_launch_pipeline(
    promotion_dir: str | Path,
    *,
    output_dir: str | Path,
    config: ImbalanceLaunchPipelineConfig | None = None,
) -> ImbalanceLaunchPipelineReport:
    config = config or ImbalanceLaunchPipelineConfig()
    ...
    order_plan = write_imbalance_order_plan(
        promotion,
    ...
    if _order_plan_ready(order_plan):
        ...
        staging = write_staged_orders(
            order_plan_dir / "imbalance_order_candidates.csv",
...
def _order_plan_ready(report: ImbalanceOrderPlanReport) -> bool:
    ...
    *,
    ...
    return pd.DataFrame(
        [
            {
                "ready": ready,
                "strategy": str(order_row.get("strategy", IMBALANCE_STRATEGY)),
